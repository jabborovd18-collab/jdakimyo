...
export async function sertifikatPDFYuklab(s) {
  ...
  if (isMavsumiy) {
    ...
    let alchemIqRasm = null
    let jdaRasm = null
    try {
      const [alchB, jdaB] = await Promise.all([
        baytlar('/images/hamkorlik/alchemiq-logo.jpg'),
        baytlar('/images/hamkorlik/jdakimyo-neon-logo.jpg'),
      ])
      alchemIqRasm = await doc.embedJpg(alchB)
      jdaRasm = await doc.embedJpg(jdaB)
    } catch {
      // Agar rasm yuklanmasa vektorli/matnli ko'rinish ishlaydi
    }
    ...
    if (alchemIqRasm) {
      sahifa.drawImage(alchemIqRasm, {
        x: 45, y: H - 195, width: 160, height: 140,
      })
    } else {
      sahifa.drawRectangle({
    ...
    if (jdaRasm) {
      sahifa.drawImage(jdaRasm, {
        x: W - 145, y: H - 75, width: 105, height: 52,
      })
    } else {
