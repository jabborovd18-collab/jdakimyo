...
export async function sertifikatPDFYuklab(s, customRank = null) {
  ...
  if (isMavsumiy) {
    ...
    let imzo = null
    try {
      const [alchB, jdaB, imzoB] = await Promise.all([
      ...
      jdaLogo = await doc.embedJpg(jdaB)
    ...
    sahifa.drawText('RASMIY DTM SINOVI', {
      x: 55, y: 110, size: 8.5, font: qalin,
      color: rgb(0.85, 0.68, 0.25),
    });

    // 3. O'ng yuqorida JDA KIMYO logotipi
    if (jdaLogo) {
      sahifa.drawImage(jdaLogo, {
        x: W - 180, y: H - 80, width: 120, height: 60,
      });
    }

    // 4. SERTIFIKAT Sarlavhasi
    ...
    // 5. Qatnashuvchi ismi
    const name = toza(s.fullName || 'Ishtirokchi');
    let nameSize = 22;
