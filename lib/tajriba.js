  ...
  const [reaksiyalar, deflar] = await Promise.all([
    ...
    prisma.labItemDef.findMany({
      ...
      select: {
        ...
        daraja: true, sarflanadi: true, xom: true,
      },
    }),
  ])
...
/** Inventarni kalit → soni ko'rinishida beradi */
async function inventarniOl(labId) {
  const items = await prisma.labItem.findMany({
    where: { labId },
    select: { kalit: true, soni: true },
  })
  return new Map(items.map((i) => [i.kalit, i.soni]))
}
 ...
 */
function nimaYetishmaydi(reaksiya, inventar, daraja, defBoyicha) {
  const kamchilik = []

  for (const a of reaksiya.chap) {
    const bor = inventar.get(a.kalit) ?? 0
    if (bor < a.koef) {
      kamchilik.push({
        turi: 'reagent',
        kalit: a.kalit,
 ...
 */
export async function tajribaniOtkaz(userId, kalitlar, reactionId = null) {
  ...
  const natija = await urinib(() => prisma.$transaction(async (tx) => {
    ...
    const olinganXP = TAJRIBA_XP + (birinchi ? KASHFIYOT_XP : 0)
    ...
    for (const a of reaksiya.chap) {
      const kamaydi = await tx.labItem.updateMany({
        where: { labId: lab.id, kalit: a.kalit, soni: { gte: a.koef } },
        data: { soni: { decrement: a.koef } },
      })
      if (kamaydi.count === 0) {
        throw new LabXatosi(`${a.kalit} yetarli emas (${a.koef} ta kerak)`)
      }
    }

    for (const k of sarfJihozlar) {
      const kamaydi = await tx.labItem.updateMany({
        where: { labId: lab.id, kalit: k, soni: { gte: 1 } },
        data: { soni: { decrement: 1 } },
      })
      if (kamaydi.count === 0) {
    ...
    await tx.labItem.deleteMany({ where: { labId: lab.id, soni: { lte: 0 } } })

    for (const a of mahsulotlar) {
      await tx.labItem.upsert({
        where: { labId_kalit: { labId: lab.id, kalit: a.kalit } },
        create: { labId: lab.id, kalit: a.kalit, soni: a.koef },
        update: { soni: { increment: a.koef } },
      })
    }
