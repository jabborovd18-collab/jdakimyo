 ...
 */
function talabniHisobla(reaksiya, defBoyicha, xarita = null) {
...
function nimaYetishmaydi(reaksiya, inventar, daraja, defBoyicha, sozlama = {}) {
  const { kerakliMiqdor = null, xarita = null, variantlar = null } = sozlama
  const kamchilik = []

  for (const t of talabniHisobla(reaksiya, defBoyicha, xarita)) {
    ...
    if (!xarita && variantlar) {
      for (const v of variantlar.get(t.kalit) ?? []) {
        if (defBoyicha.get(v)?.cheksiz) { bor = Infinity; break }
...
export async function mumkinTajribalar(userId, chegara = 40) {
  ...
  const mumkin = []
  for (const r of yaroqli) {
    if (nimaYetishmaydi(r, inventar, lab.daraja, defBoyicha, { variantlar }).length > 0) continue
...
export async function tajribaniOtkaz(userId, kalitlar, reactionId = null, miqdorlar = null) {
  ...
  const talab = talabniHisobla(reaksiya, defBoyicha, xarita)
