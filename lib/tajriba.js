...
import { koefdanMiqdor, miqdorniFormatla, yetadimi, birlikdanHajmga } from './lab-birlik'
...
import { eritmaErituvchisi, erituvchiBahosi, SUV } from './lab-erituvchi'
import { idishmi, idishSigimi, idishXavfi, pastHaroratniOqi } from './lab-idish'
import { ERKIN_ARALASHMA_MATRITSASI } from '../data/erkin-aralashma-matritsasi.js'

/** Bir tajribada ko'pi bilan shuncha xil reagent */
const MAX_REAGENT = 6

/**
 * Shu chegaradan boshlab `temperature` maydoni "isitish kerak" deb
 * o'qiladi. 60 °C — spirtovka siklida qaynash effekti boshlanadigan
 * nuqta bilan bir xil (3d/lib/jihoz/yordamchi.js, qaynashniYangila).
 * Pastroq qiymatlar ("25°C", "xona harorati") darvoza emas.
 */
const ISITISH_CHEGARASI = 60
...
async function keshniTayyorla() {
  const [reaksiyalar, deflar] = await Promise.all([
    ...
    prisma.labItemDef.findMany({
      ...
      select: {
        ...
        asos: true, cheksiz: true, cheksizAgar: true,
      },
    }),
  ])
  ...
  // Asos kalit → uning variantlari. Jo'mrak suvi inventarda alohida
  // yozuv, lekin reaksiya "H₂O" so'raydi; "mumkin tajribalar" ro'yxati
  // esa foydalanuvchi hali hech nima tanlamagan paytda tuziladi, ya'ni
  // asosning O'RNIGA nima borligini shu indeksdan bilish kerak.
  const variantlar = new Map()
  for (const d of deflar) {
    if (!d.asos) continue
    if (!variantlar.has(d.asos)) variantlar.set(d.asos, [])
    variantlar.get(d.asos).push(d.kalit)
  }
  ...
  return { yaroqli, toplamBoyicha, defBoyicha, variantlar }
}
...
/**
 * Tenglama bazasi aniq moslik bermaganda ikki eritmaning ko'rinadigan
 * holatini aytadi. Mahsulot va sarf bu yerda chiqarilmaydi: koeffitsient
 * noma'lum bo'lsa inventarga modda qo'shish yoki undan ayirish soxta kimyo
 * bo'lardi.
 */
export function erkinAralashmaniBahola(kalitlar = []) {
  const reagentlar = [...new Set((kalitlar || []).map((kalit) => String(kalit || '').trim()).filter(Boolean))]
  const natija = {
    turi: 'ozgarishsiz_aralashma',
    reaksiyaBor: false,
    sarflanadimi: false,
    reagentlar,
    kuzatuv: "Eritmalar aralashadi, lekin jadvaldagi aniq reaksiya yoki cho'kma belgisi topilmadi.",
  }
  if (reagentlar.length !== 2) return natija

  const [birinchi, ikkinchi] = reagentlar
  const kislota = ERKIN_ARALASHMA_MATRITSASI.kislotalar
  const asos = ERKIN_ARALASHMA_MATRITSASI.asoslar
  if ((kislota.includes(birinchi) && asos.includes(ikkinchi)) || (kislota.includes(ikkinchi) && asos.includes(birinchi))) {
    return {
      ...natija,
      turi: 'neytrallanish',
      reaksiyaBor: true,
      kuzatuv: "Neytrallanish boradi: eritma odatda rangsiz qoladi, idish biroz isiydi.",
    }
  }

  const ionlar = [
    ...(ERKIN_ARALASHMA_MATRITSASI.ionlar[birinchi] || []),
    ...(ERKIN_ARALASHMA_MATRITSASI.ionlar[ikkinchi] || []),
  ]
  const chokma = ERKIN_ARALASHMA_MATRITSASI.chokmalar.find((qoida) =>
    qoida.ionlar.every((ion) => ionlar.includes(ion)),
  )
  if (!chokma) return natija

  return {
    ...natija,
    turi: 'chokma',
    reaksiyaBor: true,
    kuzatuv: chokma.kuzatuv,
  }
}
 ...
 */
function talabniHisobla(reaksiya, defBoyicha, xarita = null, cheksizlar = null) {
  return reaksiya.chap.map((a) => {
    // `kalit` — tenglamadagi asos ("H₂O"). `inventarKaliti` — foydalanuvchi
    // aynan nimani tanlagani ("H₂O-oddiy"). Sarflash, birlik va "yetadimi"
    // tekshiruvi ikkinchisi bo'yicha ketadi, nisbat esa birinchisi bo'yicha.
    const inventarKaliti = xarita?.get(a.kalit) ?? a.kalit
    const def = defBoyicha.get(inventarKaliti)
    const birlik = def?.birlik ?? 'ml'
    return {
      kalit: a.kalit,
      inventarKaliti,
      koef: a.koef,
      birlik,
      // Cheksizlik foydalanuvchiga bog'liq: distillagich o'rnatgan odam
      // uchun distillangan suv ham tugamaydi.
      cheksiz: cheksizlar ? cheksizlar.has(inventarKaliti) : Boolean(def?.cheksiz),
 ...
 */
function nimaYetishmaydi(reaksiya, inventar, daraja, defBoyicha, sozlama = {}) {
  const { kerakliMiqdor = null, xarita = null, variantlar = null, cheksizlar = null } = sozlama
  const kamchilik = []

  for (const t of talabniHisobla(reaksiya, defBoyicha, xarita, cheksizlar)) {
    ...
    // Cheksiz manba (jo'mrak suvi) hech qachon yetishmaydi.
    if (t.cheksiz) continue

    // Foydalanuvchi hali hech nima tanlamagan bo'lsa (ro'yxat tuzilayotgan
    // payt) asosning O'RNIGA variant turgan bo'lishi mumkin: reaksiya
    // "H₂O" so'raydi, inventarda esa jo'mrak suvi bor. Shuning uchun
    // eng ko'p miqdorli variant hisobga olinadi.
    let bor = inventar.get(t.inventarKaliti) ?? 0
    if (!xarita && variantlar) {
      for (const v of variantlar.get(t.kalit) ?? []) {
        if (cheksizlar ? cheksizlar.has(v) : defBoyicha.get(v)?.cheksiz) { bor = Infinity; break }
        bor = Math.max(bor, inventar.get(v) ?? 0)
      }
    }
    ...
    if (!yetadimi(bor, kerak)) {
      kamchilik.push({
        ...
        bor: Number.isFinite(bor) ? miqdorniFormatla(bor, t.birlik) : 'cheksiz',
      })
    }
  }
...
export async function mumkinTajribalar(userId, chegara = 40) {
  ...
  const { yaroqli, defBoyicha, variantlar } = await keshniOl()
  const inventar = await inventarniOl(lab.id)
  const cheksizlar = cheksizKalitlar([...defBoyicha.values()], inventar.keys())
  ...
  const mumkin = []
  for (const r of yaroqli) {
    if (nimaYetishmaydi(r, inventar, lab.daraja, defBoyicha, { variantlar, cheksizlar }).length > 0) continue
 ...
 * @param {number|null=} harorat — idishning hozirgi harorati (°C).
 *        3D laboratoriya spirtovka siklidan yuboradi; 2D yubormaydi va
 *        shunda tekshiruv o'tkazib yuboriladi — eski xatti-harakat
 *        saqlanadi. Qiymatga ishonish xavfsiz: harorat resurs emas,
 *        uni "aldab" yuborishdan foydalanuvchi mahsulot yutmaydi —
 *        faqat o'ziga tegishli o'quv natijasi o'zgaradi.
 * @returns natija, yoki {tanlov: [...]} — tanlash kerakligini bildiradi
 */
export async function tajribaniOtkaz(userId, kalitlar, reactionId = null, miqdorlar = null, idish = null, harorat = null) {
  ...
  // Variant → asos xaritasi.
  //
  // Jo'mrak suvi ("H₂O-oddiy") inventarda alohida yozuv, lekin reaksiya
  // tenglamada "H₂O" deb yozilgan. Qidiruv asos bo'yicha ketadi, sarflash
  // esa foydalanuvchining HAQIQIY kaliti bo'yicha — aks holda jo'mrak
  // suvi tanlansa ham distillangani kamayardi.
  //
  // Ikkalasi birga tanlansa variant ustun turadi: bir tomchi jo'mrak
  // suvi butun eritmani ifloslantiradi, toza suv qo'shish uni orqaga
  // tozalamaydi.
  const xarita = new Map()
  for (const k of tanlangan) {
    const def = defBoyicha.get(k)
    const asos = def?.asos || k
    if (!xarita.has(asos) || def?.asos) xarita.set(asos, k)
  }

  let nomzodlar = toplamBoyicha.get(toplamKaliti([...xarita.keys()])) || []

  // Suv ERITUVCHI bo'lishi mumkin. CuSO₄ avval suvda eritiladi, keyin
  // NaOH qo'shiladi — idishda uch kalit bor, tenglamada esa ikkitasi
  // (CuSO₄ + 2NaOH). Aniq to'plam topilmasa, suv "muhit" deb qaraladi
  // va usiz qayta izlanadi.
  //
  // Tartib muhim: AVVAL aniq to'plam. "2Na + 2H₂O" kabi reaksiyalarda
  // suv haqiqiy reagent va u yo'qotilmasligi kerak — aniq moslik ustun.
  // Suv baribir sarflanmaydi: sarflash `talab` (tenglamaning chap
  // tomoni) bo'yicha ketadi, erituvchi bahosi esa `tanlangan` dan
  // hisoblanadi, ya'ni jo'mrak suvi jarimasi saqlanadi.
  if (nomzodlar.length === 0 && xarita.has(SUV)) {
    const suvsiz = [...xarita.keys()].filter((k) => k !== SUV)
    if (suvsiz.length > 0) {
      nomzodlar = toplamBoyicha.get(toplamKaliti(suvsiz)) || []
    }
  }

  if (nomzodlar.length === 0) {
    // 3D va 2D clientlar `kirishmadi` kodidan idishning rangini va xato
    // holatini belgilashda foydalanadi. Matritsa keyingi UI taklifi uchun
    // sof yordamchi bo'lib qoladi; mavjud HTTP shartnomasi buzilmaydi.
    throw new LabXatosi(
  ...
  // Inventar talabdan OLDIN o'qiladi: cheksizlik foydalanuvchining
  // nimaga egaligiga bog'liq (distillagich bo'lsa suv tugamaydi), talab
  // esa qaysi modda cheksizligini bilishi kerak.
  const inventar = await inventarniOl(lab.id)
  const cheksizlar = cheksizKalitlar([...defBoyicha.values()], inventar.keys())

  const talab = talabniHisobla(reaksiya, defBoyicha, xarita, cheksizlar)
  ...
  for (const t of talab) {
    // Client miqdorni O'ZI TANLAGAN kalit bilan yuboradi ("H₂O-oddiy"),
    // nisbat esa asos bo'yicha hisoblanadi ("H₂O").
    const xom = miqdorlar ? Number(miqdorlar[t.inventarKaliti] ?? miqdorlar[t.kalit]) : NaN
    quyilgan[t.kalit] = Number.isFinite(xom) && xom > 0 ? xom : t.miqdor
  }

  const baho = nisbatniBaho(quyilgan, talab)

  // Erituvchi bahosi. Idishga qaysi suv quyilgani natijaga ta'sir qiladi:
  // jo'mrak suvi cho'ktirish tahlilini buzadi, distillangan suv esa
  // elektrolizni umuman boshlamaydi (unda erkin ion yo'q).
  const erituvchi = eritmaErituvchisi(tanlangan)
  const erituvchiBaho = erituvchiBahosi(reaksiya, erituvchi?.xossa ?? null)

  // Idish sig'imi. Client qaysi idishda ishlaganini yuboradi; yubormasa
  // tekshiruv o'tkazib yuboriladi (2D laboratoriyada idish tanlanmaydi).
  //
  // Ilgari bunday tekshiruv UMUMAN yo'q edi: sig'im faqat 3D modellarida
  // yashardi va serverdan ko'rinmasdi, ya'ni 25 ml probirkaga 500 ml
  // quyish mumkin edi.
  if (idish && idishmi(idish)) {
    const sigim = idishSigimi(idish)
    const jamiHajm = talab.reduce(
      (yigindi, t) => yigindi + birlikdanHajmga(quyilgan[t.kalit] ?? 0, t.birlik),
      0,
    )
    if (jamiHajm > sigim + 0.001) {
      const nom = defBoyicha.get(idish)?.nom ?? idish
      throw new LabXatosi(
        `${nom} ga ${sigim} ml sig'adi, siz ${jamiHajm.toFixed(1)} ml quydingiz — ` +
          "idish to'lib toshdi. Kattaroq idish oling yoki kamroq quying.",
      )
    }
  }

  if (erituvchiBaho.toxtatadimi) {
    // Reaksiya boshlanmadi — hech narsa sarflanmaydi. Bu xato emas,
    // natija: "nima uchun bo'lmadi" degan savolga javob izohda.
    throw new LabXatosi(erituvchiBaho.izoh)
  }

  // Harorat darvozasi. Bazadagi `temperature` reaksiya borishi uchun
  // kerakli haroratni aytadi ("80–100 °C"); idish hali sovuq bo'lsa
  // reaksiya BOSHLANMAYDI va hech narsa sarflanmaydi.
  //
  // Faqat client harorat YUBORGANDA ishlaydi: 2D laboratoriya yubormaydi
  // va u yerda eski xatti-harakat (bir bosishda tayyor) saqlanadi.
  // Chegara XONA_CHEGARASI dan past bo'lsa ham tekshirilmaydi — "25°C"
  // xona sharoiti degani, uni darvoza qilish har reaksiyani to'sardi.
  if (Number.isFinite(harorat)) {
    const kerakHarorat = pastHaroratniOqi(reaksiya.temperature)
    if (kerakHarorat >= ISITISH_CHEGARASI && harorat < kerakHarorat) {
      throw new LabXatosi(
        `Bu reaksiya ${reaksiya.temperature} da boradi, idish esa hozir ` +
          `${Math.round(harorat)} °C. Spirtovkada qizdirib, keyin qayta uriring — ` +
          'hech narsa sarflanmadi.',
        'sovuq',
      )
    }
  }

  const kamchilik = nimaYetishmaydi(reaksiya, inventar, lab.daraja, defBoyicha, {
    kerakliMiqdor: quyilgan,
    xarita,
    cheksizlar,
  })
  ...
  // Idish reaksiyadan omon chiqadimi — tranzaksiyadan oldin hisoblanadi,
  // chunki natijaga ham, javobga ham kerak.
  const idishBahosi =
    idish && idishmi(idish)
      ? idishXavfi(
          reaksiya,
          [...reaksiya.chap.map((a) => a.kalit), ...reaksiya.ong.map((a) => a.kalit)],
          idish,
        )
      : { buziladimi: false, sabab: null }
  const idishBuzildi = idishBahosi.buziladimi
  ...
  const natija = await urinib(() => prisma.$transaction(async (tx) => {
    ...
    const asosiyXP = Math.max(
      1,
      Math.round(TAJRIBA_XP * xpKoeffitsiyenti(baho.holat) * erituvchiBaho.xpKoef),
    )
    ...
    for (const t of talab) {
      // Cheksiz manba kamaymaydi — jo'mrakni "sarflab" bo'lmaydi.
      if (t.cheksiz) continue

      const yetdi = await inventarSarfla(
        tx, lab.id, t.inventarKaliti, quyilgan[t.kalit], t.birlik,
      )
      if (!yetdi) {
        const nom = defBoyicha.get(t.inventarKaliti)?.nom ?? t.inventarKaliti
        throw new LabXatosi(
    ...
    for (const a of mahsulotlar) {
      ...
      const chiqdi = koefdanMiqdor(a.koef, birlik) * baho.unum * erituvchiBaho.unumKoef
      await inventarQosh(tx, lab.id, a.kalit, chiqdi, birlik)
    }

    // Idish reaksiyaga chidadimi. Ftorid kislota shishani eritadi,
    // 500 °C dan yuqori harorat uni yumshatadi, qizdirilgan ishqor
    // yemiradi. Chinni va kvarts chidaydi.
    //
    // Idish MAHSULOTDAN KEYIN sarflanadi: reaksiya baribir bo'ldi,
    // faqat idish undan omon chiqmadi.
    if (idishBuzildi) {
      await inventarSarfla(tx, lab.id, idish, 1, 'dona')
      await boshYozuvlarniOchir(tx, lab.id)
    }
  ...
  return {
    reaksiya: {
      ...
      // Erituvchi izohi kuzatuvga QO'SHILADI, o'rniga yozilmaydi:
      // reaksiyaning o'zi baribir bo'ldi, faqat natija toza chiqmadi.
      observations: erituvchiBaho.kuzatuv
        ? `${reaksiya.observations} ${erituvchiBaho.kuzatuv}`
    ...
    sarflandi: talab.map((a) => {
      const birlik = a.birlik
      return {
        // Foydalanuvchi nimani ishlatgan bo'lsa, o'shani ko'rsatamiz:
        // "H₂O sarflandi" deyish jo'mrak suvi quyilganda yolg'on bo'lardi.
        kalit: a.inventarKaliti,
        nom: defBoyicha.get(a.inventarKaliti)?.nom ?? a.inventarKaliti,
        soni: a.koef,
        birlik,
        cheksiz: a.cheksiz,
    ...
    idish: idish
      ? {
          kalit: idish,
          nom: defBoyicha.get(idish)?.nom ?? idish,
          sigim: idishmi(idish) ? idishSigimi(idish) : null,
          buzildi: idishBuzildi,
          sabab: idishBahosi.sabab,
        }
      : null,
    // Qaysi suv ishlatilgani va u natijaga qanday ta'sir qilgani
    erituvchi: erituvchi
      ? {
          kalit: erituvchi.kalit,
          nom: erituvchi.xossa.nom,
          toza: erituvchi.xossa.toza,
          ...erituvchiBaho,
        }
      : null,
    ...natija,
