...
function toza(matn) {
  ...
  // Control characterlar va \text variatsiyalari
  s = s.replace(/[\t\\]?ext\{([^}]+)\}/gi, "$1");
  s = s.replace(/\\(?:text|mathrm|mathbf|mathit|textbf)\{([^}]+)\}/gi, "$1");
...
async function baytlar(manzil) {
  if (typeof window === "undefined") {
    ...
    const toliq = path.join(process.cwd(), "public", manzil.replace(/^\//, ""));
    return fs.readFile(toliq);
  }
...
export async function masalaPdfYukla({
  ...
  if (typeof window === "undefined" || !avtoYuklabOlish) {
    return pdfBaytlar;
  }
