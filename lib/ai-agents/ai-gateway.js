...
async function geminiChaqir(nomzod, vazifa) {
  ...
  const url = `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(nomzod.model)}:generateContent?key=${encodeURIComponent(tozaKalit)}`;
...
export async function aiModelChaqir(prompt, {
  ...
  const tafsilot = oxirgiXato?.message ? ` (${String(oxirgiXato.message).slice(0, 150)})` : "";
