...
export const AI_MODEL_REYESTRI = Object.freeze({
  ...
  gemini: Object.freeze({ asosiy: "gemini-3.6-flash", zaxira: "gemini-3.5-flash" }),
  openrouter: Object.freeze({
    matn: "google/gemma-4-31b-it:free",
    rasm: "google/gemma-4-31b-it:free",
  }),
});
 ...
 */
export function aiMasalaNatijasiniTekshir(natija) {
...
async function openrouterChaqir(nomzod, vazifa) {
  ...
  const response = await vaqtChegaraliFetch("https://openrouter.ai/api/v1/chat/completions", {
    ...
    body: JSON.stringify({
      model: nomzod.model, messages: xabarlar, temperature: 0.2, max_tokens: vazifa.tokenChegarasi,
      tools: vazifa.vositalar?.length ? vazifa.vositalar : undefined,
      tool_choice: vazifa.vositalar?.length ? vazifa.vositaTanlovi : undefined,
    }),
  }, vazifa.vaqtMs);
...
let geminiMavjudModellarKesh = null;
let geminiKeshVaqti = 0;

async function geminiHaqiqiyModelOl(kalit, talabQilinganModel, xatoXabari = "") {
  const hozir = Date.now();
  // Agar Google xato xabarida to'g'ridan-to'g'ri yangi tavsiya etilgan modelni aytgan bo'lsa (masalan gemini-3.6-flash),
  // uni birinchi navbatda olamiz va keshga yozamiz:
  const tavsiya = String(xatoXabari || "").match(/(?:update your code to use|use)\s+(?:models\/)?([a-zA-Z0-9_.-]+)/i)?.[1];
  if (tavsiya) {
    geminiMavjudModellarKesh = [tavsiya];
    geminiKeshVaqti = hozir;
    return tavsiya;
  }

  if (geminiMavjudModellarKesh && (hozir - geminiKeshVaqti < 3600_000)) {
    return geminiMavjudModellarKesh[0] || talabQilinganModel;
  }
  try {
    ...
    const eskirganlar = ["1.5", "2.0", "2.5", "legacy", "deprecated"];
    const modellar = (data?.models || [])
      .filter((m) => Array.isArray(m.supportedGenerationMethods) && m.supportedGenerationMethods.includes("generateContent"))
      .map((m) => String(m.name || "").replace(/^models\//, ""))
      .filter((m) => m && !eskirganlar.some((e) => m.includes(e)));
    if (modellar.length > 0) {
      geminiMavjudModellarKesh = modellar;
      geminiKeshVaqti = hozir;
      const flashModel = modellar.find((m) => m === "gemini-3.6-flash")
        || modellar.find((m) => m.includes("3.6") && m.includes("flash"))
        || modellar.find((m) => m.includes("3.5") && m.includes("flash"))
        || [...modellar].reverse().find((m) => m.includes("flash") && !m.includes("8b") && !m.includes("exp"))
        || modellar[0];
      return flashModel || talabQilinganModel;
    }
  } catch (err) {
...
async function geminiChaqir(nomzod, vazifa) {
  ...
  let modelNomi = geminiMavjudModellarKesh?.[0] || nomzod.model;
  const qismlar = [];
  if (vazifa.rasmBase64) {
    ...
    qismlar.push({
      inline_data: { mime_type: mime, data: tozaData },
    });
  }
  ...
  if (!response.ok && (response.status === 404 || response.status === 400)) {
    const xatoData = await response.clone().json().catch(() => null);
    const xatoXabari = xatoData?.error?.message || "";
    geminiMavjudModellarKesh = null;
    const yangiModel = await geminiHaqiqiyModelOl(tozaKalit, modelNomi, xatoXabari);
...
export async function aiModelChaqir(prompt, {
  ...
  // Rasm tahlili multimodal OCR va chuqur kimyoviy hisobni talab qiladi;
  // shuning uchun bitta urinish kamida 48 soniya, umumiy vaqt kamida 55 soniya beriladi.
  const urinishVaqtiMs = rasmBase64 ? Math.max(sozlama.urinishVaqtiMs, 48_000) : sozlama.urinishVaqtiMs;
  const umumiyVaqtMs = rasmBase64 ? Math.max(sozlama.umumiyVaqtMs, 55_000) : sozlama.umumiyVaqtMs;
  const muhit = muhitSozlamalariOl();
  ...
  for (let indeks = 0; indeks < nomzodlar.length; indeks++) {
    ...
    try {
      let xabarlar = faolVositalar.length ? openAiXabarlari(prompt, systemPrompt) : null;
      ...
      for (let aylana = 0; aylana < 4; aylana++) {
        javob = await nomzodniChaqir(nomzod, {
          ...
          vaqtMs: Math.min(urinishVaqtiMs, qolganVaqt),
          xabarlar,
      ...
      if (jsonRejim) {
        const tekshiruv = aiMasalaNatijasiniTekshir(natija);
        if (!tekshiruv.yaroqli) {
          throw new ProvayderXatosi(`Provayder javobi yaroqsiz: ${tekshiruv.sabab}`, { kod: "FORMAT_XATOSI" });
        }
      }
      ...
      const xatoKodi = error?.status
        ? `${error.status}:${String(error?.message || error?.kod || "XATO")}`.slice(0, 75)
        : (error?.kod || "NOMALUM");
      urinishXatolari.push({
        provayder: nomzod.provayder,
        model: nomzod.model,
        xabar: String(error?.message || error?.kod || "XATO").slice(0, 250),
      });
      const hodisa = {
        provider: nomzod.provayder,
        model: nomzod.model,
      ...
      console.warn("[AI Gateway urinish xatosi]", { ...hodisa, xato: error?.message, urinish: indeks + 1 });
    }
  }

  const vaqtTugadimi = Date.now() - boshlanganVaqt >= umumiyVaqtMs || oxirgiXato?.kod === "VAQT_TUGADI";
  const tafsilot = urinishXatolari.length
    ? ` (${urinishXatolari.map((x) => `${x.provayder}: ${x.xabar}`).join(" | ")})`
    : (oxirgiXato?.message ? ` (${String(oxirgiXato.message).slice(0, 150)})` : "");
  throw new AiGatewayXatosi(
    vaqtTugadimi
      ? "AI belgilangan vaqtda javob bermadi. So'rovni qisqartirib yoki qayta yuborib ko'ring."
      : `AI provayderlari hozir javob bera olmadi${tafsilot}.`,
