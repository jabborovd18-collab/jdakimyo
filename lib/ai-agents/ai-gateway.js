...
async function geminiHaqiqiyModelOl(kalit, talabQilinganModel, xatoXabari = "") {
  ...
  try {
    const res = await vaqtChegaraliFetch(
      ...
      { method: "GET", headers: { "x-goog-api-key": tozaKalit } },
      5000,
    );
...
async function geminiChaqir(nomzod, vazifa) {
  ...
  const sovuqChaqir = async (ishlatiladiganModel) => {
    const url = `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(ishlatiladiganModel)}:generateContent`;
...
export async function aiModelChaqir(prompt, {
  ...
  for (let indeks = 0; indeks < nomzodlar.length; indeks++) {
    ...
    try {
      ...
      // Gemini fikrlash tokenlari qisqa suhbatning 800 tokenini yutib yuboradi;
      // boshqa provayderlarning kvotasi esa o'zgarishsiz qoladi.
      let joriyTokenChegarasi = nomzod.provayder === "gemini" && yonalish === "tezkor" && !rasmBase64
        ? Math.max(tokenChegarasi, 2_000)
        : tokenChegarasi;
      for (let aylana = 0; aylana < 4; aylana++) {
        ...
        if (tokenChegarasidaKesildimi(javob.finishReason)) {
          ...
          if (nomzod.provayder === "gemini" && yonalish === "tezkor" && !rasmBase64
            && !kesilishQaytaUrinildi && joriyTokenChegarasi < 4_000 && qaytaUrinishUchunVaqt >= 6_000) {
            kesilishQaytaUrinildi = true;
            joriyTokenChegarasi = 4_000;
            continue;
          }
...
export async function aiProvayderKorigi({ runtimeSozlama = null, prompt = "H2O formulasi nimani anglatadi? Bir jumlada javob bering." } = {}) {
  ...
  return Promise.all(nomzodlar.map(async (nomzod) => {
    ...
    try {
      const rasmSinovi = nomzod.provayder === "gemini" || nomzod.alias === "openrouterRasm";
      const javob = await nomzodniChaqir(nomzod, {
        prompt: `Sinov savoli: ${String(prompt || "").slice(0, 500)}`,
        // DeepSeek faqat JSON obyekt rejimini biladi; sxema matni bo'lmasa
        // sog'lom kalit noto'g'ri format xatosi sifatida ko'rinadi.
        systemPrompt: `Kimyo savoliga qisqa javob bering. Faqat shu JSON sxemaga mos obyekt qaytaring: ${JSON.stringify(AI_SUHBAT_JSON_SXEMASI)}`,
        jsonRejim: true,
