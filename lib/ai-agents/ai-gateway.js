...
export const AI_MODEL_REYESTRI = Object.freeze({
  ...
  groq: Object.freeze({ tezkor: "llama-3.1-8b-instant", asosiy: "llama-3.3-70b-versatile" }),
  gemini: Object.freeze({ asosiy: "gemini-2.0-flash", zaxira: "gemini-1.5-flash" }),
...
async function geminiHaqiqiyModelOl(kalit, talabQilinganModel) {
  ...
  try {
    ...
    if (modellar.length > 0) {
      ...
      const flashModel = modellar.find((m) => m.includes("flash") && !m.includes("8b") && !m.includes("exp"))
        || modellar.find((m) => m.includes("flash"))
        || modellar[0];
...
async function geminiChaqir(nomzod, vazifa) {
  ...
  if (response.status === 404) {
    ...
    const yangiModel = await geminiHaqiqiyModelOl(tozaKalit, modelNomi);
  ...
  for (let indeks = 0; indeks < nomzodlar.length; indeks++) {
    ...
    try {
      ...
      urinishXatolari.push({
        ...
        model: nomzod.model,
        xabar: String(error?.message || error?.kod || "XATO").slice(0, 100),
      });
      const hodisa = {
        provider: nomzod.provayder,
        model: nomzod.model,
