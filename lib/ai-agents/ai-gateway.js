...
export async function aiModelChaqir(prompt, {
  ...
  // Rasm OCRi uchun uchta fallbackning har biriga navbat yetishi kerak. Bitta
  // urinishga butun route vaqtini berish birinchi provayderni amalda yagona qiladi.
  const urinishVaqtiMs = rasmBase64 ? Math.max(sozlama.urinishVaqtiMs, 24_000) : sozlama.urinishVaqtiMs;
  const umumiyVaqtMs = rasmBase64 ? Math.max(sozlama.umumiyVaqtMs, 75_000) : sozlama.umumiyVaqtMs;
  // Gemini 3 fikrlash tokenlari ham chiqish limitiga kiradi. 2 500 tokenli
  // oddiy yo'l OCR + hisob + JSONni kesib qo'ygani uchun rasmga alohida zaxira beriladi.
  const tokenChegarasi = rasmBase64 ? Math.max(sozlama.tokenChegarasi, 8_000) : sozlama.tokenChegarasi;
  const urinishChegarasi = rasmBase64 ? Math.max(sozlama.urinishChegarasi, 3) : sozlama.urinishChegarasi;
  const muhit = muhitSozlamalariOl();
