...
export async function multiAgentMasalaYech({
  ...
  const tenglamaVositaKerak = /\b(?:tengla|tenglashtir|koeffitsiyent|redoks)\w*/i.test(tozaMatn);
  const vositalarFaol = aiYonalish.id === "murakkab"
    || ["olimpiada", "organik", "eritmalar"].includes(masalaTuri)
    || tenglamaVositaKerak;
  const vositaMajburiy = aiYonalish.id === "murakkab" || masalaTuri === "olimpiada" || tenglamaVositaKerak;
  const vositaIzlari = [];
