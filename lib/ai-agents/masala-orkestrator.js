...
//
// JDA KIMYO — KO'P AGENTLI KIMYOVIY MASALALAR ORKESTRATORI (v4.0.0)
//
// Ushbu modul 4 ta ixtisoslashgan AI agent zanjiridan iborat:
// 1. Tahlilchi Agent (Shartni o'qish, Berilgan/Topish kerak va Masala turini aniqlash)
// 2. Kimyogar Agent (Reaksiya tenglamalari, molyar massalar, stexiometriya va mollar)
// 3. Matematik & Auditor Agent (Hisob-kitoblarni tekshirish, birliklar mutanosibligi)
// 4. Pedagog & KaTeX Formatter Agent (O'zbekcha darslik formati, KaTeX, Pearson kresti)
//
// Provayderlar: Groq (GPT-OSS-120B, Qwen-3.8-27B) + OpenRouter + Gemini 2.0/1.5
...
/**
 * Umumiy AI chaqiruvchi funksiya (Groq -> OpenRouter -> Gemini zaxira zanjiri)
 */
export async function aiModelChaqir(prompt, { systemPrompt = '', jsonRejim = true, rasmBase64 = null } = {}) {
  const keys = getEnvKeys();

  // 1. GROQ ORQALI CHAQIRISH (Eng tezkor, 120B kuchli model)
  ...
  // 2. OPENROUTER ORQALI CHAQIRISH (Bepul modellar zanjiri)
  if (keys.openrouter && !rasmBase64) {
  ...
  // 3. GEMINI ORQALI CHAQIRISH (Ayniqsa rasmlar va multimodal uchun)
  if (keys.gemini) {
    const geminiModellar = ['gemini-2.0-flash', 'gemini-1.5-flash'];
    for (const m of geminiModellar) {
      try {
        ...
        const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${m}:generateContent?key=${keys.gemini.trim()}`, {
          method: 'POST',
...
1-AGENT (Tahlilchi):
- Masala matnidan yoki ilova qilingan rasmdan barcha sonli qiymatlar va fizik kattaliklarni "berilgan" va "topishKerak" bo'limlariga ajratadi.
...
Agar masala eritmalar aralashmasi (2 xil foizli eritma aralashtirilib yangi foiz olish) bo'lsa, "krestSxemasi" obyektini to'ldiring:
{
  "mavjud": true,
  "w1": 40,
  "w2": 10,
...
SOF JSON FORMATIDA QAYTARING (hech qanday markdown \`\`\`json tegisiz):
{
  ...
  const foydalanuvchiPrompti = rasm
    ? `Ilova qilingan rasmdagi kimyo masalasini sinchkovlik bilan tahlil qiling va ${rejim} rejimida yeching.`
 ...
 * O'quvchi yechim chiqqandan keyin tushunmagan joyini so'rasa
 */
export async function aiRepetitorChat({ masalaMatni, yechim, savol, tarix = [] }) {
