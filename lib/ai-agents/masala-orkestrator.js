...
import { AI_SUHBAT_JSON_SXEMASI, AI_YECHIM_JSON_SXEMASI } from "./ai-javob-sxema.js";
import { aiKesh } from "./ai-cache.js";
import { aiSozlamaniOl } from "./ai-config.js";
import { DETERMINISTIK_VOSITA_SCHEMALARI, aiYechiminiDeterministikTekshir, deterministikKontekstTuz, deterministikVositaniBajar } from "./deterministik-kimyo.js";
import { xavfsizlikTekshir } from "./ai-security.js";
import { aiYonalishniAniqlash, masalaTuriniAniqlash } from "./ai-yonalish.js";
import { aiVazifaSiyosatiniTuz } from "./ai-vazifa-siyosati.js";
...
import { sokratikKorsatmaTuz, sokratikRejimniAniqla } from "./sokratik-repetitor.js";
import { chatVizualiniYarat } from "../chat-vizual.js";
...
- "tushuntirish", "mantiq" va "yakuniyJavob" matnidagi barcha \\ce, \\times, indeksli belgilar va boshqa LaTeX ifodalarini $...$ ichiga oling; alohida katta formula uchun $$...$$ ishlating. Har bir delimiter jufti alohida va to'liq yopilsin.
- Kimyoviy atamalar va birliklarni izchil o'zbekcha yozing; oraliq hisobni erta yaxlitlamang, yakuniy sonni masala aniqligiga mos yaxlitlang.`;
...
const MURAKKAB_YECHIM_PROTOKOLI = `

MURAKKAB YECHIM PROTOKOLI — BU UCH QADAMNI O'TKAZIB YUBORMANG:
A. Kimyoviy model: parallel va ketma-ket reaksiyalarni, tenglashtirilgan koeffitsiyentlar hamda OVR elektron balansini yozing.
B. Matematik apparat: aralashma komponentlari uchun x, y (zarur bo'lsa z) mollarni belgilang, massa/mol/gaz hajmi tenglamalar sistemasini tuzing va qadamma-qadam yeching.
C. Sanity check: kirgan-chiqqan massa balansini, cheklovchi reagentni va fizik ma'noga ega bo'lmagan ildizlarni tekshiring. Yakuniy JSONga "sanityTekshiruvi" obyektini qo'shing: massaBalansi, cheklovchiReagent, xulosa.`;

const DETERMINISTIK_VOSITA_QOIDASI = `

DETERMINISTIK VOSITALAR QOIDASI:
- Murakkab sonli hisobni o'zingizdan to'qimang. Avval mos server vositasini chaqiring: sistema uchun kramer_yech, elektroliz uchun faradey_massasi, pH uchun ph_hisobla, formula uchun molyar_massa_hisobla, normal sharoit gazlari uchun gaz_hisobla, cho'kma muvozanati uchun ks_hisobla.
- Yonish analizi uchun organik_formula_top, Henderson-Hasselbalch buferi uchun bufer_ph, eritma nisbatlari uchun pearson_kresti vositasidan foydalaning.
- Tenglama koeffitsiyentlari, redoks yoki "tenglashtir" so'ralganda avval reaksiya_tengla vositasini chaqiring; uning tenglashtirilgan tenglamasini o'zgartirmang.
- Vositadan qaytgan sonlar yakuniy haqiqatdir. Ularni o'zgartirmang, faqat birliklari va kimyoviy ma'nosini o'quvchiga tushuntiring.
- Vositani chaqirgandan keyingina sof JSON yechimni qaytaring.`;
...
export async function multiAgentMasalaYech({
  ...
  const matndanAniqlanganTur = masalaTuriniAniqlash(tozaMatn);
  const dastlabkiMasalaTuri = rasm && matndanAniqlanganTur === "suhbat" ? "umumiy" : matndanAniqlanganTur;
  const aiYonalish = aiYonalishniAniqlash({
    matn: tozaMatn,
    rasm,
    masalaTuri: dastlabkiMasalaTuri,
    tanlov: ishlashYonalishi,
  });
  const { masalaTuri, vositalarFaol, vositaMajburiy } = aiVazifaSiyosatiniTuz({
    rasm,
    matndanAniqlanganTur,
    yonalishId: aiYonalish.id,
    matn: tozaMatn,
  });
  ...
  const vositaIzlari = [];
  const vositaBajaruvchi = ({ nom, argumentlar }) => {
    const natija = deterministikVositaniBajar({ nom, argumentlar });
    vositaIzlari.push({ nom, natija });
    return natija;
  };
...
SOF JSON FORMATIDA QAYTARING:
{
  ...
  if (aiYonalish.id === "murakkab") {
    tizimPrompti += MURAKKAB_YECHIM_PROTOKOLI;
  }
  if (vositalarFaol) {
    tizimPrompti += DETERMINISTIK_VOSITA_QOIDASI;
  }
...
3. "boshqaMasalalar" massivida rasmdagi qolgan yechilmagan masalalar nomlari/raqamlarini ro'yxat qiling (masalan: ["13-masala", "14-masala"]). Agar boshqa masala bo'lmasa, bo'sh massiv [] qoldiring.`
  ...
  const natija = await aiModelChaqir(foydalanuvchiPrompti, {
    ...
    vositalar: vositalarFaol ? DETERMINISTIK_VOSITA_SCHEMALARI : [],
    vositaBajaruvchi: vositalarFaol ? vositaBajaruvchi : null,
    vositaTanlovi: vositaMajburiy ? "required" : "auto",
    jsonSxema: masalaTuri === "suhbat" ? AI_SUHBAT_JSON_SXEMASI : AI_YECHIM_JSON_SXEMASI,
    kutilganJavobTuri: masalaTuri === "suhbat" ? "suhbat" : "yechim",
  });
  ...
  if (tozaNatija && tozaNatija.muvaffaqiyatli) {
    ...
    const hakamlik = aiYechiminiDeterministikTekshir(tozaNatija);
    if (deterministik.ishlatildi || hakamlik.tekshirildi || vositaIzlari.length > 0) {
      tozaNatija.serverTekshiruvi = {
        turi: "deterministik_kimyo",
        dalillar: deterministik.dalillar,
        ...hakamlik,
        vositalar: vositaIzlari,
      };
    }
...
export async function aiRepetitorChat({
  ...
  const xavfsizlik = xavfsizlikTekshir(savol);
  if (!xavfsizlik.xavfsiz) {
    return {
      muvaffaqiyatli: false,
      xato: xavfsizlik.sabab,
  ...
  const tozaSavol = xavfsizlik.tozaMatn;
  const sokratikRejim = sokratikRejimniAniqla(tozaSavol);
  const kontekstMatni = xotiraKontekstiniMatnga(xotiraKonteksti);
...
Oddiy matn ichidagi formulani $...$, alohida formulani $$...$$ bilan yozing. Hech qachon $$$$ ishlatmang; keyingi formula uchun avval matn yoki yangi qator yozing.`;
  const sokratikTizimPrompti = sokratikRejim ? `${tizimPrompti}${sokratikKorsatmaTuz()}` : tizimPrompti;

  const prompt = `${kontekstMatni}\n\nO'quvchining savoli: "${tozaSavol}"`;
  const aiYonalish = aiYonalishniAniqlash({
    matn: tozaSavol,
    masalaTuri: "suhbat",
    tanlov: ishlashYonalishi,
  ...
  const javob = await aiModelChaqir(prompt, {
    systemPrompt: sokratikTizimPrompti,
    jsonRejim: false,
    yonalish: aiYonalish.id,
    runtimeSozlama: faolSozlama.config,
  ...
  return {
    ...
    sokratikRejim,
    vizual: chatVizualiniYarat(yechim),
    aiYonalish: {
      id: aiYonalish.id,
      nom: aiYonalish.nom,
      avtomatik: aiYonalish.avtomatik,
