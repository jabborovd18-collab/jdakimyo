...
import { azoniOqi } from "../chem-balance.js";

export { ATOM_MASSALAR, molyarMassaHisobla };
...
function lcmButun(a, b) {
  return Math.abs(a * b) / gcdButun(a, b);
}

function kasrgaYaqinlashtir(qiymat, maxMaxraj = 10000) {
  const ishora = qiymat < 0 ? -1 : 1;
  const nishon = Math.abs(qiymat);
  for (let maxraj = 1; maxraj <= maxMaxraj; maxraj++) {
    const surat = Math.round(nishon * maxraj);
    if (Math.abs(nishon - surat / maxraj) < 1e-8) return { surat: ishora * surat, maxraj };
  }
  return null;
}

/** Gauss eliminatsiyasi bir o'lchamli nol-fazoni butun koeffitsiyentga aylantiradi. */
function nolFazodanButunKoeffitsiyentlar(matritsa) {
  const a = matritsa.map((qator) => [...qator].map(Number));
  const qatorlar = a.length;
  const ustunlar = a[0]?.length || 0;
  const pivotlar = [];
  let pivotQator = 0;

  for (let ustun = 0; ustun < ustunlar && pivotQator < qatorlar; ustun++) {
    let engKatta = pivotQator;
    for (let qator = pivotQator + 1; qator < qatorlar; qator++) {
      if (Math.abs(a[qator][ustun]) > Math.abs(a[engKatta][ustun])) engKatta = qator;
    }
    if (Math.abs(a[engKatta][ustun]) < 1e-10) continue;
    [a[pivotQator], a[engKatta]] = [a[engKatta], a[pivotQator]];
    const pivot = a[pivotQator][ustun];
    a[pivotQator] = a[pivotQator].map((qiymat) => qiymat / pivot);
    for (let qator = 0; qator < qatorlar; qator++) {
      if (qator === pivotQator) continue;
      const koeffitsiyent = a[qator][ustun];
      if (Math.abs(koeffitsiyent) < 1e-10) continue;
      a[qator] = a[qator].map((qiymat, indeks) => qiymat - koeffitsiyent * a[pivotQator][indeks]);
    }
    pivotlar.push(ustun);
    pivotQator++;
  }

  const erkinlar = Array.from({ length: ustunlar }, (_, indeks) => indeks).filter((indeks) => !pivotlar.includes(indeks));
  // Bir nechta erkin noma'lumda kimyoviy jihatdan yagona eng kichik yechim yo'q.
  if (erkinlar.length !== 1) return null;
  const yechim = Array(ustunlar).fill(0);
  yechim[erkinlar[0]] = 1;
  for (let qator = pivotlar.length - 1; qator >= 0; qator--) {
    const pivotUstun = pivotlar[qator];
    yechim[pivotUstun] = -a[qator].reduce((yigindi, qiymat, ustun) => (
      ustun === pivotUstun ? yigindi : yigindi + qiymat * yechim[ustun]
    ), 0);
  }
  const kasrlar = yechim.map((qiymat) => kasrgaYaqinlashtir(qiymat));
  if (kasrlar.some((kasr) => !kasr)) return null;
  const umumiyMaxraj = kasrlar.reduce((lcm, kasr) => lcmButun(lcm, kasr.maxraj), 1);
  let butunlar = kasrlar.map((kasr) => kasr.surat * (umumiyMaxraj / kasr.maxraj));
  const umumiyBoluvchi = butunlar.reduce((gcd, son) => gcdButun(gcd, son), 0);
  butunlar = butunlar.map((son) => son / umumiyBoluvchi);
  if (butunlar.every((son) => son < 0)) butunlar = butunlar.map((son) => -son);
  return butunlar.every((son) => Number.isInteger(son) && son > 0) ? butunlar : null;
}

function tenglamaTomoniniAjrat(tomon) {
  if (Array.isArray(tomon)) return tomon.map((formula) => String(formula || "").trim()).filter(Boolean);
  const matn = String(tomon || "").trim();
  if (!matn) return null;
  // Bo'shliqli yozuv ion zaryadidagi '+' belgisini modda ajratgichi deb olmaslik uchun ustun.
  const qismlar = /\s+\+\s+/.test(matn) ? matn.split(/\s+\+\s+/) : matn.split(/\s*\+\s*/);
  return qismlar.map((formula) => formula.trim()).filter(Boolean).length === qismlar.length ? qismlar.map((formula) => formula.trim()) : null;
}

function tenglamaniAjrat({ tenglama = null, chapTomon = null, ongTomon = null } = {}) {
  if (chapTomon !== null || ongTomon !== null) {
    const chap = tenglamaTomoniniAjrat(chapTomon);
    const ong = tenglamaTomoniniAjrat(ongTomon);
    return chap?.length && ong?.length ? { chap, ong } : null;
  }
  const matn = String(tenglama || "").trim();
  const mosliklar = [...matn.matchAll(/(?:⇌|⇄|⇋|↔|⟷|<->|<=>|→|⟶|⇒|⟹|➔|➜|->|=>|=)/g)];
  if (mosliklar.length !== 1) return null;
  const moslik = mosliklar[0];
  const chap = tenglamaTomoniniAjrat(matn.slice(0, moslik.index));
  const ong = tenglamaTomoniniAjrat(matn.slice((moslik.index || 0) + moslik[0].length));
  return chap?.length && ong?.length ? { chap, ong } : null;
}

function azoTarkibiniOl(xom) {
  const azo = azoniOqi(xom);
  return {
    formula: String(xom).trim().replace(/^\d+\s*/, ""),
    atomlar: Object.fromEntries(Object.entries(azo.atomlar).map(([element, son]) => [element, son / azo.koeffitsient])),
    zaryad: azo.zaryad / azo.koeffitsient,
  };
}

const ODATIY_OKSIDLANISH_DARAJALARI = Object.freeze({
  H: 1, O: -2, F: -1, Cl: -1, Br: -1, I: -1, Li: 1, Na: 1, K: 1, Rb: 1, Cs: 1,
  Mg: 2, Ca: 2, Sr: 2, Ba: 2, Al: 3, Zn: 2, Ag: 1, S: 6, N: 5,
});

function oksidlanishDarajalari(azo) {
  const elementlar = Object.keys(azo.atomlar);
  if (elementlar.length === 1 && azo.zaryad === 0) return { [elementlar[0]]: 0 };
  const natija = {};
  for (const element of elementlar) {
    // N va S birikmaga qarab bir nechta daraja oladi; ular qolgan atomlar
    // orqali qayta hisoblanadi. Jadval ularni faqat metall darajasini chiqarish
    // uchun tayanch sifatida saqlaydi.
    if (ODATIY_OKSIDLANISH_DARAJALARI[element] !== undefined && !["N", "S"].includes(element)) {
      natija[element] = ODATIY_OKSIDLANISH_DARAJALARI[element];
      continue;
    }
    const qolganlar = elementlar.filter((belgi) => belgi !== element);
    if (!qolganlar.every((belgi) => ODATIY_OKSIDLANISH_DARAJALARI[belgi] !== undefined)) continue;
    const qolganZaryad = qolganlar.reduce((yigindi, belgi) => (
      yigindi + azo.atomlar[belgi] * ODATIY_OKSIDLANISH_DARAJALARI[belgi]
    ), 0);
    natija[element] = (azo.zaryad - qolganZaryad) / azo.atomlar[element];
  }
  return natija;
}

function redoksTahliliniYarat(chap, ong, koeffitsiyentlar) {
  const chapDarajalar = chap.map(oksidlanishDarajalari);
  const ongDarajalar = ong.map(oksidlanishDarajalari);
  const oksidlanish = [];
  const qaytarilish = [];
  const elementlar = new Set([...chapDarajalar.flatMap(Object.keys), ...ongDarajalar.flatMap(Object.keys)]);

  for (const element of elementlar) {
    const chapQiymatlar = [...new Set(chapDarajalar.map((daraja) => daraja[element]).filter((daraja) => daraja !== undefined))];
    const ongQiymatlar = [...new Set(ongDarajalar.map((daraja) => daraja[element]).filter((daraja) => daraja !== undefined))];
    const umumiy = chapQiymatlar.filter((daraja) => ongQiymatlar.includes(daraja));
    const chapFarqli = chapQiymatlar.filter((daraja) => !umumiy.includes(daraja));
    const ongFarqli = ongQiymatlar.filter((daraja) => !umumiy.includes(daraja));
    let dan;
    let ga;
    if (chapFarqli.length === 1 && ongFarqli.length === 1) {
      [dan] = chapFarqli;
      [ga] = ongFarqli;
    } else if (chapFarqli.length === 0 && ongFarqli.length === 1 && umumiy.length === 1) {
      [dan] = umumiy;
      [ga] = ongFarqli;
    } else if (chapFarqli.length === 1 && ongFarqli.length === 0 && umumiy.length === 1) {
      [dan] = chapFarqli;
      [ga] = umumiy;
    } else continue;
    if (dan === ga) continue;
    const chapAtomlarSoni = chap.reduce((yigindi, azo, indeks) => (
      chapDarajalar[indeks][element] === dan
        ? yigindi + azo.atomlar[element] * koeffitsiyentlar[indeks]
        : yigindi
    ), 0);
    const ongAtomlarSoni = ong.reduce((yigindi, azo, indeks) => (
      ongDarajalar[indeks][element] === ga
        ? yigindi + azo.atomlar[element] * koeffitsiyentlar[chap.length + indeks]
        : yigindi
    ), 0);
    // Bir elementning bir qismi tomoshabin ion bo'lib qolishi mumkin (masalan,
    // nitratning bir qismi Cu(NO3)2 ga o'tadi). Shu sabab elektronlar faqat
    // yangi oksidlanish darajasiga amalda o'tgan eng kichik atom oqimidan olinadi.
    const atomlarSoni = Math.min(chapAtomlarSoni, ongAtomlarSoni);
    const hodisa = { element, dan, ga, elektronlar: Math.abs(ga - dan) * atomlarSoni };
    (ga > dan ? oksidlanish : qaytarilish).push(hodisa);
  }

  const berilgan = oksidlanish.reduce((yigindi, hodisa) => yigindi + hodisa.elektronlar, 0);
  const qabulQilingan = qaytarilish.reduce((yigindi, hodisa) => yigindi + hodisa.elektronlar, 0);
  return {
    turi: oksidlanish.length && qaytarilish.length ? "redoks" : "redoks_aniqlanmadi",
    elektronBalansi: {
      oksidlanish,
      qaytarilish,
      berilganElektronlar: berilgan,
      qabulQilinganElektronlar: qabulQilingan,
      mos: berilgan > 0 && berilgan === qabulQilingan,
    },
  };
}

/**
 * Kimyoviy tenglamani mavjud atom/zaryad parseri asosida Gauss eliminatsiyasi
 * bilan tenglaydi. Bir nechta mustaqil yechim bo'lsa taxmin qilmaydi.
 */
export function reaksiyaTengla(kirish = {}) {
  const tomonlar = tenglamaniAjrat(kirish);
  if (!tomonlar) return null;
  try {
    const chap = tomonlar.chap.map(azoTarkibiniOl);
    const ong = tomonlar.ong.map(azoTarkibiniOl);
    const azolar = [...chap, ...ong];
    const elementlar = [...new Set(azolar.flatMap((azo) => Object.keys(azo.atomlar)))].sort();
    const matritsa = elementlar.map((element) => azolar.map((azo, indeks) => (
      (azo.atomlar[element] || 0) * (indeks < chap.length ? 1 : -1)
    )));
    if (azolar.some((azo) => azo.zaryad !== 0)) {
      matritsa.push(azolar.map((azo, indeks) => azo.zaryad * (indeks < chap.length ? 1 : -1)));
    }
    const koeffitsiyentlar = nolFazodanButunKoeffitsiyentlar(matritsa);
    if (!koeffitsiyentlar) return null;
    const formatla = (azo, indeks) => `${koeffitsiyentlar[indeks] === 1 ? "" : koeffitsiyentlar[indeks]}${azo.formula}`;
    const tenglashtirilganTenglama = `${chap.map(formatla).join(" + ")} -> ${ong.map((azo, indeks) => formatla(azo, chap.length + indeks)).join(" + ")}`;
    const redoks = redoksTahliliniYarat(chap, ong, koeffitsiyentlar);
    return {
      tenglashtirilganTenglama,
      koeffitsiyentlar,
      jamiKoeffitsiyent: koeffitsiyentlar.reduce((yigindi, son) => yigindi + son, 0),
      redoksTuri: redoks.turi,
      elektronBalansi: redoks.elektronBalansi,
    };
  } catch {
    return null;
  }
...
const VOSITA_PARAMETRLARI = Object.freeze({
  ...
  reaksiya_tengla: Object.freeze({
    type: "object",
    additionalProperties: false,
    properties: {
      tenglama: { type: "string", description: "Tenglashtirilmagan tenglama, masalan KMnO4 + HCl -> KCl + MnCl2 + Cl2 + H2O" },
      chapTomon: { type: "array", description: "Chap tomondagi formulalar" },
      ongTomon: { type: "array", description: "O'ng tomondagi formulalar" },
    },
  }),
});
...
export const DETERMINISTIK_VOSITA_SCHEMALARI = Object.freeze([
  ...
  Object.freeze({ type: "function", function: Object.freeze({ name: "reaksiya_tengla", description: "Reaksiya tenglamasini atomlar va zaryad bo'yicha Gauss eliminatsiyasi bilan tenglaydi.", parameters: VOSITA_PARAMETRLARI.reaksiya_tengla }) }),
]);
...
export function deterministikVositaniBajar({ nom, argumentlar = {} } = {}) {
  const vositalar = {
    ...
    reaksiya_tengla: reaksiyaTengla,
  };
