...
class AiQuotaManager {
  ...
  /**
   * Tashqi provayder javob bermasa, oldindan band qilingan o'rin foydalanuvchi
   * hisobida qolmasligi kerak. Faqat bugungi musbat hisobdan bittani qaytaradi.
   */
  async qaytar(userId) {
    if (!userId) return { qaytarildi: false };
    ...
    for (let urinish = 0; urinish < 3; urinish++) {
      try {
        return await prisma.$transaction(async (tx) => {
          const yozuv = await tx.sorovLimit.findUnique({ where: { kalit } });
          if (!yozuv || yozuv.oynaBoshi < bugunUtc() || yozuv.soni <= 0) {
            return { qaytarildi: false };
          }
          const yangilangan = await tx.sorovLimit.update({
            where: { kalit },
            data: { soni: { decrement: 1 } },
            select: { soni: true },
          });
          return { qaytarildi: true, ishlatildi: yangilangan.soni };
        }, { isolationLevel: 'Serializable' });
      } catch (error) {
        if (error?.code === 'P2034' && urinish < 2) continue;
        throw error;
