...
const IZOH_BUYRUQLARI = Object.freeze({
  ce: 1, text: 1, mathrm: 1, mathbf: 1, mathit: 1,
  frac: 2, dfrac: 2, sqrt: 1,
  times: 0, cdot: 0, cdotp: 0, div: 0, approx: 0,
  leq: 0, geq: 0, neq: 0, rightarrow: 0, leftrightarrow: 0,
  to: 0, Delta: 0, omega: 0, nu: 0, rho: 0,
...
function ajratilmaganLatexniBol(matn) {
  // Model ba'zan izohda $...$ ni unutadi. Faqat taniqli buyruq va aniq
  // indeksli belgilarni ajratamiz; oddiy nasrni KaTeXga majburlamaymiz.
  const qolip = /\\[A-Za-z]+|\b[A-Za-z][A-Za-z0-9]?(?:_\{[^{}]+\}|_[0-9]+|\^\{[^{}]+\}|\^[0-9+\-]+)+/g;
  const bolaklar = [];
  let oxiri = 0;
  let moslik;

  while ((moslik = qolip.exec(matn)) !== null) {
    let yakun = qolip.lastIndex;
    if (moslik[0].startsWith("\\")) {
      const buyruq = moslik[0].slice(1);
      if (!(buyruq in IZOH_BUYRUQLARI) || matn[moslik.index - 1] === "\\") continue;
      let yaroqli = true;
      for (let i = 0; i < IZOH_BUYRUQLARI[buyruq]; i += 1) {
        while (/\s/.test(matn[yakun] || "")) yakun += 1;
        const guruh = guruhniOl(matn, yakun);
        if (!guruh) {
          yaroqli = false;
          break;
        }
    ...
    if (moslik.index > oxiri) bolaklar.push({ turi: "matn", matn: matn.slice(oxiri, moslik.index) });
    bolaklar.push({ turi: "formula", matn: latexniNormallashtir(matn.slice(moslik.index, yakun)), blok: false });
    oxiri = yakun;
    qolip.lastIndex = yakun;
  }

  if (oxiri < matn.length) bolaklar.push({ turi: "matn", matn: matn.slice(oxiri) });
  return bolaklar;
}
...
export function latexliMatnniBol(matn = "") {
  ...
  const boshlangich = bolaklar.length ? bolaklar : [{ turi: "matn", matn: toza }];
  return boshlangich.flatMap((bolak) => bolak.turi === "matn" ? ajratilmaganLatexniBol(bolak.matn) : [bolak]);
}
