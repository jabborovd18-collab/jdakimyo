...
import { tgHtmlniChekla, tgKimyoFormatla } from "@/lib/telegram-format";
...
export async function aiRejimniBoshla(chatId, ism = "Foydalanuvchi") {
  ...
  if (!ulanish) {
    ...
    return telegramYubor(
      id,
      `🔗 <b>JDA Kimyo AI uchun hisobni ulang</b>\n\n` +
        `Quiz yechish va botning boshqa oddiy xizmatlari hisobsiz ishlaydi. ` +
        `AI hamda saytdagi bildirishnomalar uchun esa <b>jdakimyo.uz</b> hisobi kerak.\n\n` +
        `<code>/kod</code> yozing va berilgan kodni saytdagi Telegram sozlamasiga kiriting.`,
      { havola: { matn: "Saytda ro'yxatdan o'tish", url: `${SAYT}/register` } }
    );
  }
...
export { tgHtmlniChekla, tgKimyoFormatla } from "@/lib/telegram-format";

function tgYechimMatniniTuz(natija, sarlavha) {
  const yakuniyJavob = tgKimyoFormatla(natija?.yakuniyJavob || "Yechildi", Infinity);
  let javobMatni =
    `${sarlavha}\n\n` +
    `<b>Yakuniy javob:</b> <code>${yakuniyJavob}</code>\n\n`;

  if (natija?.serverTekshiruvi?.ogohlantirishlar?.length > 0) {
    const eslatmalar = natija.serverTekshiruvi.ogohlantirishlar
      .map((ogohlantirish) => `• ${tgKimyoFormatla(ogohlantirish.xabar || "", Infinity)}`)
      .filter(Boolean)
      .join("\n");
    if (eslatmalar) javobMatni += `<b>Server eslatmasi:</b>\n${eslatmalar}\n\n`;
  }

  javobMatni += "<i>To'liq bayonnoma keyingi PDF xabarida yuboriladi.</i>";
  return tgHtmlniChekla(javobMatni);
}
...
export async function shaxsiyAiXabariniBajar({ chatId, xabar, username, ism }) {
  ...
  // Ulanish AI rejimi davomida uzilgan bo'lishi mumkin. Har haqiqiy
  // AI so'rovida tekshirish begona hisobdan kvotasiz foydalanishni to'sadi.
  if (!ulanish?.user) {
    faolAiSuhbatlar.delete(id);
    return telegramYubor(
      id,
      `🔗 <b>JDA Kimyo AI uchun hisobni ulang</b>\n\n` +
        `Quiz yechish va botning boshqa oddiy xizmatlari hisobsiz ishlaydi. ` +
        `AI hamda bildirishnomalar uchun <code>/kod</code> orqali hisobingizni ulang.`,
      { klaviatura: true }
    );
  }
  ...
  try {
    ...
    const javobMatni = tgYechimMatniniTuz(natija, "🧪 <b>JDA Kimyo AI — Masala Yechimi</b>");
    ...
    if (foydalanuvchiId && kvotaQaytariladigan) {
      try {
        await aiQuota.qaytar(foydalanuvchiId);
      } catch (qaytarishXatosi) {
        console.error("[Telegram AI kvota qaytarish xatosi]", qaytarishXatosi?.message);
      }
    }
    if (tempMsg?.natija?.message_id) {
      await telegramOchir(id, tempMsg.natija.message_id);
...
export async function guruhAiXabariniBajar({ chatId, xabar, botUsername = "jdakimyouzbot" }) {
  ...
  try {
    ...
    const javobMatni = tgYechimMatniniTuz(natija, "🧪 <b>JDA Kimyo AI</b>");
