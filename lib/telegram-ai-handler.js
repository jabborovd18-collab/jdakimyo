...
//
// JDA KIMYO AI — TELEGRAM BOT SHAXSIY VA GURUH INTEGRATSIYASI (v5.0 Enterprise)
// Shaxsiy sessiya rejimi, guruhlarda @teg bilan PDF yechim jo'natish, foydalanuvchi hisobini tekshirish.
...
export async function aiRejimniBoshla(chatId, ism = "Foydalanuvchi") {
  ...
  return telegramYubor(id, matn, {
    klaviatura: false,
    qoshimcha: { reply_markup: aiKlaviatura() },
  });
}
...
export async function shaxsiyAiXabariniBajar({ chatId, xabar, username, ism }) {
  ...
  if (!masalaMatni && !rasmBase64) {
    return telegramYubor(id, "Iltimos, masala shartini matn holida yozing yoki rasmini yuboring.");
  }

  // 3. Jarayon boshlanganini bildirish
  await telegramYubor(id, "🧠 <i>JDA Kimyo AI tahlil qilmoqda va yechmoqda...</i>");
  ...
  try {
    ...
    await telegramYubor(id, javobMatni);
    ...
    return telegramYubor(id, "⚠️ Masalani tahlil qilishda kutilmagan xatolik yuz berdi. Iltimos, qayta urinib ko'ring.");
  }
}
...
export async function guruhAiXabariniBajar({ chatId, xabar, botUsername = "jdakimyouzbot" }) {
  ...
  // 2. Matndan bot nomini olib tashlash
  let matn = (xabar.text || xabar.caption || "").trim();
  const botTagRegex = new RegExp(`@${botUsername}\\b`, "gi");
  matn = matn.replace(botTagRegex, "").trim();
