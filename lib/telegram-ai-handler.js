...
function tgKimyoFormatla(matn) {
  ...
  // 0. Barcha JSON unescaped control characterlarni (\t -> \text, \f -> \frac, \r -> \rightarrow) tozalash
  s = s.replace(/[\t\\]?ext\{([^}]+)\}/gi, "$1");
  s = s.replace(/\\(?:text|mathrm|mathbf|textbf|mathit)\{([^}]+)\}/gi, "$1");
  s = s.replace(/[\f\\]?d?rac\{([^}]+)\}\{([^}]+)\}/gi, "($1 / $2)");
  s = s.replace(/\\d?frac\{([^}]+)\}\{([^}]+)\}/gi, "($1 / $2)");
  s = s.replace(/[\r\\]?ightarrow/gi, " → ");

  // 1. Nuqta-vergullar va bo'shliqlar
  s = s.replace(/\{,\}/g, ".");
  s = s.replace(/\\,/g, " ");

  // 2. Strelkalar va reaksiyalar
  s = s.replace(/\\xrightarrow\[([^\]]*)\]\{([^}]+)\}/gi, " —[$1, $2]→ ");
  s = s.replace(/\\xrightarrow\{([^}]+)\}/gi, " —[$1]→ ");
  s = s.replace(/\\rightarrow/gi, " → ");
  s = s.replace(/\\to\b/gi, " → ");
  s = s.replace(/\\leftrightarrow/gi, " ↔ ");
  s = s.replace(/\\Delta/gi, "Δ (t°)");
  s = s.replace(/\\cdot/gi, " · ");
  s = s.replace(/\\times/gi, " × ");
  s = s.replace(/\\uparrow/gi, "↑");
  s = s.replace(/\\downarrow/gi, "↓");

  // 3. Superscript (Ionlar zaryadi va darajalar)
  s = s.replace(/\^\{\s*2-\s*\}/g, "²⁻");
  s = s.replace(/\^\{\s*3-\s*\}/g, "³⁻");
  s = s.replace(/\^\{\s*4-\s*\}/g, "⁴⁻");
  s = s.replace(/\^\{\s*2\+\s*\}/g, "²⁺");
  ...
  // 5. Dollar belgilarini olib tashlash
  s = s.replace(/\$\$/g, "");
  s = s.replace(/\$/g, "");

  // 6. HTML xavfsizligi va Markdown bold
