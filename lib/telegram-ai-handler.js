...
function tgKimyoFormatla(matn) {
  ...
  // Telegram 4096 belgidan oshsa xabarni rad etadi (400 Bad Request)
  if (s.length > 3900) {
...
export async function shaxsiyAiXabariniBajar({ chatId, xabar, username, ism }) {
  ...
  try {
    ...
    if (natija.serverTekshiruvi?.ogohlantirishlar?.length > 0) {
      javobMatni += `⚠️ <b>Server Eslatmasi:</b>\n` +
        natija.serverTekshiruvi.ogohlantirishlar.map((o) => `• ${tgHimoyala(o.xabar)}`).join('\n') + `\n\n`;
    }
