...
/** "14:30" — chatdagi xabar vaqti */
export function soatDaqiqa(value) {
  const d = toDate(value)
  if (!d) return ''
...
/**
 * Toshkent kunining kaliti: "2026-08-11".
 *
 * Chatdagi sana ajratgichi shu kalit o'zgargan joyda chiziladi. Nega
 * kalit kerak: taqqoslashni matn ustida qilish uchun. `getDate()` bilan
 * solishtirilsa, oy oxirida 31 va 1 "boshqa kun" bo'ladi-yu, keyingi
 * oyning 31-kuni yana o'sha songa tushib qoladi.
 */
export function kunKaliti(value) {
  const d = toDate(value)
  if (!d) return ''
  return d.toISOString().slice(0, 10)
}

/** Chatdagi ajratgich matni: "Bugun", "Kecha" yoki "11 avgust 2026" */
export function kunSarlavhasi(value) {
  const kalit = kunKaliti(value)
  if (!kalit) return ''
  if (kalit === kunKaliti(new Date())) return 'Bugun'
  if (kalit === kunKaliti(Date.now() - 86400000)) return 'Kecha'
  return sana(value)
}
