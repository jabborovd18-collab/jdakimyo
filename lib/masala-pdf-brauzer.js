...
async function premiumPdfniOl(sozlamalar) {
  ...
  try {
    ...
    if (!javob.ok) {
      ...
      const sabab = new Error(xato?.xato || `PDF serveri ${javob.status} xato qaytardi.`);
      sabab.statusCode = javob.status;
      throw sabab;
    }
...
export async function masalaPdfBrauzerdaYukla(sozlamalar) {
  try {
    ...
    // Admin PDF kanalini ataylab yopgan bo'lsa, brauzer zaxirasi bu qarorni
    // aylanib o'tmasligi kerak.
    if (xato?.statusCode === 503) throw xato;
