...
/**
 * `temperature` matnidan ENG PAST sonni ajratadi — reaksiya boshlanishi
 * uchun yetarli harorat. "80–100 °C" → 80, bo'sh bo'lsa 0.
 *
 * `haroratniOqi` bilan farqi: u idish chidamini tekshiradi (eng yomon
 * holat — eng yuqori son), bu esa reaksiyaning boshlanish ostonasi
 * (eng past son). Ikkalasi bitta matnni ikki maqsadda o'qiydi, shuning
 * uchun yonma-yon turadi — manba bitta, `Reaction.temperature`.
 */
export function pastHaroratniOqi(matn) {
  const sonlar = String(matn ?? "").match(/\d{2,4}/g);
  if (!sonlar) return 0;
  return Math.min(...sonlar.map(Number));
}
