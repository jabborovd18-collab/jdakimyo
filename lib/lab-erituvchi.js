...
/**
 * Distillagich — bu jihozga ega bo'lgan odam distillangan suvni
 * cheksiz oladi.
 *
 * Yangi buyum O'YLAB TOPILMADI: katalogda `haydash-apparati` allaqachon
 * bor (280 tanga, "ajratish" guruhi) va u aynan shu ish uchun — qaynash
 * haroratiga qarab suyuqliklarni ajratadi. Suvni haydash uning eng
 * oddiy qo'llanishi.
 */
export const DISTILLAGICH = "haydash-apparati";
...
export const ERITUVCHILAR = {
  ...
  [SUV]: {
    ...
    aralashmalar: [],
    cheksiz: false,
    // Distillagich o'rnatilgan bo'lsa — cheksiz. Progressiya shunday
    // chiqadi: jo'mrak suvi (bepul, iflos) → ulushlab sotib olinadigan
    // distillangan suv → apparat va boshqa to'lamaslik.
    cheksizAgar: DISTILLAGICH,
...
/**
 * Erituvchining katalogga yoziladigan xossalari — variant bo'lmagani
 * uchun ham (masalan `H₂O` ning `cheksizAgar` i). Seed skripti mavjud
 * yozuvni shu qiymatlar bilan to'ldiradi.
 */
export function erituvchiKatalogXossalari(kalit) {
  const x = ERITUVCHILAR[kalit];
  if (!x) return null;
  return {
    nom: x.nom,
    asos: x.asos ?? null,
    cheksiz: Boolean(x.cheksiz),
    cheksizAgar: x.cheksizAgar ?? null,
    tavsif: x.izoh ?? null,
  };
}
