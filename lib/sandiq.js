...
import { donadanMiqdor } from './lab-birlik'
import { inventarQosh } from './lab-inventar'
import { LabXatosi, labniOl, taqiqniTekshir, urinib } from './laboratoriya'
import SANDIQLAR from '@/data/laboratoriya/sandiqlar.js'
...
export async function sandiqniOch(userId, sandiqKaliti) {
  ...
  const nomzodlar = await prisma.labItemDef.findMany({
    ...
    select: { kalit: true, nom: true, nodirlik: true, icon: true, sotishNarxi: true, birlik: true },
  })
  ...
  await urinib(() => prisma.$transaction(async (tx) => {
    ...
    // Sandiqdan donalab tushadi, inventarga miqdor bo'lib kiradi.
    for (const [kalit, soni] of yigilgan) {
      const birlik = nomzodlar.find((n) => n.kalit === kalit)?.birlik ?? 'ml'
      await inventarQosh(tx, lab.id, kalit, donadanMiqdor(soni, birlik), birlik)
    }
  }))
