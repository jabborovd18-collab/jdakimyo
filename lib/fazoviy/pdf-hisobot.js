...
export async function generateFazoviyPDF({
  ...
  if (sections.dorbital) {
    ...
    safeText(configStr, { x: infoX, y: topY - 13, size: 9, font: regularFont, color: C.textDark, maxWidth: infoMaxW })

    safeText("Spin holati:", { x: infoX, y: midY + 8, size: 8.5, font: boldFont, color: C.textDark, maxWidth: infoMaxW })
    safeText(dOrb.type === "LS" ? "Past spin" : "Yuqori spin", { x: infoX, y: midY - 5, size: 9, font: regularFont, color: C.textDark, maxWidth: infoMaxW })
    ...
    y = botY - 45
    const caption = `2-rasm. ${cleanText(complex.formula)} uchun ${isOct ? "oktaedrik" : "tetraedrik"} kristall maydon ajralish diagrammasi.`
    const capLines = wrapText(cleanText(caption), italicFont, 8.5, CONTENT_W)
    capLines.forEach((line, i) => {
      page.drawText(line, { x: MARGIN, y: y - i * 11, size: 8.5, font: italicFont, color: C.purpleSoft })
    })
