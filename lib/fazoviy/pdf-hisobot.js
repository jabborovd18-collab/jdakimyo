...
export async function generateFazoviyPDF({
  ...
  if (sections.dorbital) {
    ...
    const isTet = (geometryInfo.ks || complex.coordNumber) === 4 && (geometryInfo.symmetry === "Td" || !geometryInfo.symmetry)
    const hasCustomSplitting = !!(complex.dOrbitalSplitting || geometryInfo.dOrbitalSplitting)
    const customSplit = complex.dOrbitalSplitting || geometryInfo.dOrbitalSplitting
    ...
    if (hasCustomSplitting && customSplit.levels?.length) {
      // Maxsus geometriya d-orbital sathlari (tekis-kvadrat, kvadrat-piramida, trigonal-prizma, chiziqli, dodekaedrik va h.k.)
      const nLevels = customSplit.levels.length
      const stepY = (topY - botY) / Math.max(1, nLevels - 1)
      
      customSplit.levels.forEach((lvl, idx) => {
        const ly = topY - idx * stepY
        const lX = diagX + 12
        const lW = 80
        page.drawLine({ start: { x: lX, y: ly }, end: { x: lX + lW, y: ly }, thickness: 2, color: C.purple })
        page.drawText(lvl.name, { x: lX + lW + 8, y: ly - 3, size: 9, font: boldFont, color: C.purple })
        page.drawText(lvl.energy, { x: lX + 4, y: ly + 4, size: 7, font: regularFont, color: C.purpleSoft })
      })
    } else if (isOct) {
      ...
      // Tetraedrik: t2 (3 ta yuqori), e (2 ta pastki)
      const tOrb1X = diagX + 8
      const tOrb2X = diagX + 48
      const tOrb3X = diagX + 88
      const tOrbW = 32
    ...
    const displayDelta = dOrb.delta1 || dOrb.delta1_cm || dOrb.deltaO || dOrb.deltaT || dOrb.deltaTPR || deltaVal
    const displayDeltaSymbol = dOrb.delta1 ? "Δ1" : dOrb.delta1_cm ? "Δ₁" : dOrb.deltaTPR ? "Δ(TPR)" : deltaSymbol

    page.drawText(displayDeltaSymbol, { x: arX + 8, y: midY + 4, size: 12, font: boldFont, color: C.orangeDeep })
    page.drawText(`= ${displayDelta.toLocaleString()} cm⁻¹`, {
      x: arX + 8, y: midY - 10, size: 8.5, font: regularFont, color: C.orangeDeep
    })
    ...
    safeText(dOrb.configuration || configStr, { x: infoX, y: topY - 13, size: 8.5, font: regularFont, color: C.textDark, maxWidth: infoMaxW })

    safeText("Spin holati:", { x: infoX, y: midY + 8, size: 8.5, font: boldFont, color: C.textDark, maxWidth: infoMaxW })
    safeText(dOrb.type === "LS" ? "Past spin (diamagnit)" : dOrb.type === "HS" ? "Yuqori spin" : (complex.magnetism || "Past spin"), { x: infoX, y: midY - 5, size: 8.5, font: regularFont, color: C.textDark, maxWidth: infoMaxW })

    let cfseNum = isOct ? (-0.4 * (dOrb.tg ?? 6) + 0.6 * (dOrb.eg ?? 0)) : (-0.6 * (dOrb.e ?? 4) + 0.4 * (dOrb.t2 ?? 3))
    const cfseValStr = `${cfseNum.toFixed(2)} ${deltaSymbol}`
    const cfseE = (Math.abs(cfseNum) * displayDelta * 0.012).toFixed(0)

    safeText("CFSE (KMBE):", { x: infoX, y: botY + 5, size: 8.5, font: boldFont, color: C.textDark, maxWidth: infoMaxW })
    safeText(dOrb.delta2 || dOrb.delta2_cm ? `Δ2: ${(dOrb.delta2 || dOrb.delta2_cm).toLocaleString()} cm⁻¹` : cfseValStr, { x: infoX, y: botY - 8, size: 8.5, font: regularFont, color: C.textDark, maxWidth: infoMaxW })
    safeText(dOrb.delta3 ? `Δ3: ${dOrb.delta3.toLocaleString()} cm⁻¹` : `≈ ${cfseE} kJ/mol`, { x: infoX, y: botY - 20, size: 8.5, font: regularFont, color: C.textDark, maxWidth: infoMaxW })

    y = botY - 45
    const caption = `2-rasm. ${cleanText(complex.formula)} uchun ${geometryInfo.name || (isOct ? "oktaedrik" : "tetraedrik")} kristall maydon ajralish diagrammasi.`
    const capLines = wrapText(cleanText(caption), italicFont, 8.5, CONTENT_W)
    capLines.forEach((line, i) => {
      page.drawText(line, { x: MARGIN, y: y - i * 11, size: 8.5, font: italicFont, color: C.purpleSoft })
    })
  ...
  if (sections.spectra) {
    ...
    const uvVisD = complex.uvVis || complex.spectroscopy?.uvVis || (isCoNH3
      ? "λmax ≈ 475 nm  (¹A₁g → ¹T₁g)"
      : isCoCl4
      ? "λmax ≈ 660 nm  (⁴A₂ → ⁴T₁(P))"
      : "λmax ≈ 420 nm")

    const irML = complex.ir || complex.spectroscopy?.ir || (isOct ? "400–600 cm⁻¹" : "280–330 cm⁻¹")
    const irSym = isOct ? "≈ 500 cm⁻¹" : "≈ 310 cm⁻¹"
    const irAsym = isOct ? "≈ 450 cm⁻¹" : "≈ 295 cm⁻¹"
    const irBend = isOct ? "≈ 320 cm⁻¹" : "≈ 120 cm⁻¹"

    const specData = [
      ["UV-Vis (d–d / elektron o'tish)", uvVisD],
      ["UV-Vis (LMCT / zaryad ko'chishi)", complex.spectroscopy?.lmct || "λmax < 300 nm"],
      [`IR (M–${complex.ligand?.donor || "L"} tebranish)`, irML],
      [`Simmetrik cho'zilish (${isOct ? "a₁g" : "a₁"})`, complex.spectroscopy?.symmetric || irSym],
      [`Asimmetrik cho'zilish (${isOct ? "t₁u" : "t₂"})`, complex.spectroscopy?.asymmetric || irAsym],
      [`Egilish tebranishi (${isOct ? "eg" : "e"})`, complex.spectroscopy?.bending || irBend],
      ["Raman spektr", complex.raman || complex.spectroscopy?.raman || "Faol polyarizatsiyalangan modlar"],
      ["NMR / Magnit ma'lumotlari", complex.magnetism || (isCoNH3 ? "⁵⁹Co: ≈ 8200 ppm  •  ¹H: 3.5 ppm" : isCoCl4 ? "Paramagnit kengayish" : "¹³C: 170 ppm  •  ¹⁴N: 270 ppm")]
    ]
