...
export async function generateFazoviyPDF({
  ...
  if (sections.spectra) {
    let specData = []
    if (complex.spectroscopy) {
      if (complex.spectroscopy.uvVis) specData.push(["UV-Vis Spektri", complex.spectroscopy.uvVis])
      if (complex.spectroscopy.ir) specData.push(["IR Tebranish Spektri", complex.spectroscopy.ir])
      if (complex.spectroscopy.raman) specData.push(["Raman Spektri", complex.spectroscopy.raman])
      if (complex.spectroscopy.nmr) specData.push(["YMR (NMR) Spektri", complex.spectroscopy.nmr])
      if (complex.spectroscopy.xray) specData.push(["Rentgen (X-ray) Tuzilishi", complex.spectroscopy.xray])
    } else if (complex.IR && Array.isArray(complex.IR)) {
      complex.IR.forEach((item) => {
        specData.push([item.mode, `${item.freq} cm⁻¹  (${item.intensity || "faol"})`])
      })
      if (complex.UV && Array.isArray(complex.UV)) {
        complex.UV.forEach((item) => {
          specData.push([`UV: ${item.assignment}`, `λ = ${item.wl} nm  (ε ≈ ${item.epsilon})`])
        })
      }
    }
