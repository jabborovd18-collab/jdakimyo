...
export async function hisobotPDFYuklab(event, attempts = []) {
  ...
  const jamiSavollar = sinovTestimi ? 40 : 30
  ...
  for (let pIdx = 0; pIdx < sahifalar.length; pIdx++) {
    ...
    if (isBirinchi) {
      ...
      // Hujjat nomi
      sahifa.drawText(sinovTestimi ? 'MILLIY SERTIFIKAT SINOV TESTI NATIJALARI HISOBOTI' : 'MAVSUMIY HAMKORLIK — DTM SINOV TESTI NATIJALARI HISOBOTI', {
      ...
      const kartochkalar = [
        ...
        { label: sinovTestimi ? 'Topshirganlar' : "O'tganlar (>= 75%)", qiymat: sinovTestimi ? `${jami} ta` : `${otganlar} ta (${otishFoizi}%)`, rang: rgb(0.10, 0.50, 0.25) },
        { label: 'Eng Yuqori Ball', qiymat: `${engYuqori} / ${jamiSavollar} ball`, rang: rgb(0.70, 0.45, 0.10) },
        { label: sinovTestimi ? 'Vaqt chegarasi' : "O'rtacha Ko'rsatkich", qiymat: sinovTestimi ? '100 daqiqa' : `${ortachaFoiz}%`, rang: rgb(0.40, 0.20, 0.60) },
      ]
      ...
      if (attempts.length >= 3) {
        ...
        sahifa.drawText(sinovTestimi ? 'TOP 3 REYTING' : "TOP 3 G'OLIBLAR SHOHSUPASI", {
      ...
      sahifa.drawText(sinovTestimi ? 'MILLIY SERTIFIKAT SINOV TESTI NATIJALARI (DAVOMI)' : 'MAVSUMIY HAMKORLIK — DTM SINOV TESTI NATIJALARI (DAVOMI)', {
  ...
  function chizJadval(sahifa, attemptsQatorlar, boshlangichIndeks, startY) {
    ...
    sahifa.drawText(sinovTestimi ? 'Holati' : 'Holati', { x: 405, y: thY - 12, size: 8, font: qalin, color: rgb(1, 1, 1) })
    if (!sinovTestimi) {
    ...
    attemptsQatorlar.forEach((att, idx) => {
      ...
      // Ball
      sahifa.drawText(`${att.score}/${jamiSavollar}`, { x: 255, y: rowY + 7, size: 8.5, font: oddiy, color: rgb(0.05, 0.05, 0.10) })

      // Foiz
      const foizRang = sinovTestimi || att.percentage >= 75 ? rgb(0.10, 0.55, 0.25) : rgb(0.75, 0.20, 0.20)
      ...
      // Holati
      const holatMatn = sinovTestimi ? 'Topshirildi' : (att.passed ? "O'tdi" : "O'tmadi")
      const holatRang = sinovTestimi || att.passed ? rgb(0.10, 0.55, 0.25) : rgb(0.70, 0.20, 0.20)
