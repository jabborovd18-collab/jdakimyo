...
export async function telegramYubor(chatId, matn, qoshimcha = {}) {
  ...
  } else if (qoshimcha.aiKlaviatura) {
    tana.reply_markup = aiKlaviatura()
  ...
  } else if (qoshimcha.reply_markup) {
    tana.reply_markup = qoshimcha.reply_markup
  }
...
export async function telegramRasmYubor(chatId, rasmUrl, izoh, qoshimcha = {}) {
  ...
  /**
   * Telegramdagi xabarni o'chiradi (masalan, vaqtinchalik "tahlil qilinmoqda..." xabari).
   */
  export async function telegramOchir(chatId, messageId) {
    if (!telegramSozlanganmi() || !messageId) return { ok: false, sabab: 'sozlanmagan' }
    return sora('deleteMessage', {
   ...
   */
  export async function telegramHujjatYubor(chatId, buffer, faylNomi = 'JDA-Kimyo-Yechim.pdf', izoh = '', qoshimcha = {}) {
    ...
    try {
      ...
      if (qoshimcha.aiKlaviatura) {
        formData.append('reply_markup', JSON.stringify(aiKlaviatura()))
      } else if (qoshimcha.klaviatura) {
        formData.append('reply_markup', JSON.stringify(doimiyKlaviatura()))
      }
