...
export default function TaroziUI({
  ...
  return (
    <div
      className="fixed top-20 left-4 z-40 flex w-80 flex-col rounded-2xl border p-4 shadow-2xl backdrop-blur-xl transition-all v3-panel-karta bg-[var(--v3-fon-2)]/95 border-[var(--v3-chiziq-2)] space-y-3 animate-in fade-in zoom-in-95 duration-150"
    >
      <div className="flex items-center justify-between pb-2 border-b border-[var(--v3-chiziq)]">
        <span className="flex items-center gap-2 text-xs font-bold text-[var(--v3-urgu)]">
          ...
          Raqamli Analitik Tarozi (0.001g)
        </span>
        <button
          type="button"
          onClick={onYop}
      ...
      >
        ...
        <span className="mt-1 text-[10px] text-[var(--v3-xira)] font-sans">
          {!data.idishBormi
            ? "Tarozi pallasida idish yo'q (0.000 g)"
            : taraMassa > 0
            ? `(Tara: ${taraMassa.toFixed(3)} g) [NET]`
            : "Sof brutto massa"}
        </span>
      </div>
      ...
      <div className="space-y-1 text-[11px] font-mono text-[var(--v3-xira)] pt-1">
        <div className="flex justify-between">
          <span>Pallasidagi idish:</span>
          <strong className="text-[var(--v3-matn)]">
            {data.idishBormi ? `${idishKaliti.toUpperCase()} (${data.boshMassa.toFixed(3)} g)` : "Bo'sh (idish yo'q)"}
          </strong>
        </div>
        <div className="flex justify-between">
