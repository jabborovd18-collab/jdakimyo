...
export default function FazoviyQuizPage() {
  ...
  const { questions, isLoading: bankLoading, error: bankError } = useQuizBank("fazoviy", 20)
  ...
  const submitQuizResult = async (score, percentage, timeSpent) => {
    ...
    try {
      const response = await fetch('/api/quiz/submit', {
        ...
        body: JSON.stringify({
          quizName: 'Koordinatsion birikmalarning fazoviy tuzilishi',
          score,
  ...
  const handleNext = async () => {
    if (currentQuestionIndex < questions.length - 1) {
      ...
      saveQuizHistory("fazoviy", questionIds)
    ...
    return (
      <main className="min-h-screen bg-gradient-to-b from-purple-950 via-blue-950/20 to-slate-950 flex items-center justify-center p-4">
        <div className="bg-purple-900/40 border border-purple-700/50 rounded-2xl p-8 max-w-2xl w-full text-center">
          <h2 className="text-3xl font-bold text-purple-300 mb-4">Fazoviy Tuzilishi Quiz</h2>
          ...
          <div className="bg-purple-950/50 rounded-lg p-4 mb-6 text-left">
            ...
            <ul className="text-purple-300 text-sm space-y-2">
              <li>✓ Geometriya turlari (tetraedr, oktaedr, kvadrat tekislik, chiziqli)</li>
              <li>✓ Gibridlanish (sp³, dsp², d²sp³)</li>
              <li>✓ VSEPR nazariyasi</li>
              <li>✓ Fazoviy izomeriya (cis/trans, fac/mer)</li>
              <li>✓ Kristall maydon nazariyasi asoslari</li>
            </ul>
            {isAuthenticated && (
