...
// ═══════════════════════════════════════════════════════════════════════════
const CPK = {
  Fe: 0xE06633, Ni: 0x50D050, Co: 0xF090A0, Cr: 0x8A99C7,
  Ru: 0x248F8F, Os: 0x266696,
  C:  0x909090, H:  0xFFFFFF, N:  0x3050F8, O:  0xFF0D0D,
  Cl: 0x1FF01F, K:  0x8F40D4, Na: 0xAB5CF2,
  V:  0xA6A6AB,
  bond: 0x8B9DC3, hbond: 0x66CCFF,
  cpRing:      0xffb066,  // Cp halqa aromatik chizig'i
  cpDelocal:   0xff9944,  // π-delokalizatsiya markaz–halqa chiziqlari
  metalRing:   0xffcc88   // Fe–Cp koordinatsion chiziq (η⁵)
}

// ═══════════════════════════════════════════════════════════════════════════
// KOMPLEKS DATABASE — SENDVICH (METALLOTSEN) uchun tanlangan birikmalar
// ═══════════════════════════════════════════════════════════════════════════
// Ferrotsen [Fe(η⁵-C₅H₅)₂]  — klassik sendvich birikma, 1951 y. Kealy & Pauson
//     tomonidan kashf etilgan. E.O. Fischer va G. Wilkinson uni to'g'ri
//     η⁵-tuzilma sifatida asosladilar (1973 y. Nobel mukofoti).
//     18-elektron qoidasiga to'liq mos keladi: Fe(II) d⁶ + 2×6 (Cp)¹⁻ = 18 e⁻
// Nikelosen [Ni(η⁵-C₅H₅)₂] — 20 elektronli sendvich, ikkita elektron antibog'lovchi
//     e₁g* orbitalda joylashadi → paramagnit (2 juftlashmagan e⁻),
//     ferrotsendan zaif barqarorlik. Yashil rangda, havoda tez oksidlanadi.
// ═══════════════════════════════════════════════════════════════════════════
const COMPLEXES = {
  Ferrocene: {
    id: "Ferrocene",
    ...
    center: { element: "Fe", color: CPK.Fe, radius: 0.42, charge: "+2" },
    // “Ligand” bu yerda C₅H₅⁻ halqasi — η⁵ koordinatsiyali
    ligand: { type: "Cp", donor: "C", donorColor: CPK.C, donorRadius: 0.22, label: "Cp⁻" },
    // Sahnada Fe–Cp(markaz) masofasi
    ringCentroidDist: 1.66,   // Fe–Cp(centroid): 1.66 Å (haqiqiy: 1.66 Å)
    ringRadius: 1.21,          // C₅H₅ halqa radiusi (M dan C ga = 2.04 Å)
    ccBond: 1.44,              // C–C halqa bog'i
    chBond: 1.09,              // C–H
    bondLength: 2.04,          // Fe–C (o'rtacha)
    bondLengthReal: "2.04 Å",  // Fe–C (5 ta bir xil)
    ...
    // Sendvich (D₅d/D₅h) da d-orbital ajralishi: e₂g < a₁g < e₁g*
    // Fe(II) d⁶ barcha 6 elektron pastki 3 ta MO ga to'ldiriladi
    dOrbital: { e2g: 4, a1g: 2, e1gStar: 0, type: "LS", delta1: 21000, delta2: 26000 },
    geometry: "Sendvich (metallotsen)", symmetry: "D₅d",
    dElectrons: 6,
    // Rotamer holati: eclipsed (D₅h, gaz) yoki staggered (D₅d, kristall)
    conformation: "staggered",  // Cp halqalar ustma-ust burilgan (36°)
    valenceElectrons: 18
  },
  Nickelocene: {
    id: "Nickelocene",
    ...
    ligand: { type: "Cp", donor: "C", donorColor: CPK.C, donorRadius: 0.22, label: "Cp⁻" },
    ringCentroidDist: 1.82,   // Fe (1.66) dan uzunroq — antibog'lovchi elektronlar
    ringRadius: 1.21,
    ccBond: 1.43,
    chBond: 1.09,
    bondLength: 2.20,
    ...
    // 20 elektronli — e₁g* orbitalga 2 elektron
...
// ═══════════════════════════════════════════════════════════════════════════
const ATOM_INFO = {
  Fe: { name: "Temir (Fe)", atomic: 26, mass: "55.85 u", config: "[Ar] 3d⁶ 4s²", oxidation: "+2 (ferrotsende)", role: "Markaziy metall — sendvich birikmada", hybridization: "d²sp³", color: "#E06633" },
  Ni: { name: "Nikel (Ni)", atomic: 28, mass: "58.69 u", config: "[Ar] 3d⁸ 4s²", oxidation: "+2 (nikelotsende)", role: "Markaziy metall", color: "#50D050" },
  Co: { name: "Kobalt (Co)", atomic: 27, mass: "58.93 u", config: "[Ar] 3d⁷ 4s²", oxidation: "+2", role: "Kobaltotsen [Co(Cp)₂]", color: "#F090A0" },
  Cr: { name: "Xrom (Cr)", atomic: 24, mass: "52.00 u", config: "[Ar] 3d⁵ 4s¹", oxidation: "+2", role: "Xromotsen", color: "#8A99C7" },
  Ru: { name: "Ruteniy (Ru)", atomic: 44, mass: "101.07 u", config: "[Kr] 4d⁷ 5s¹", oxidation: "+2", role: "Rutenotsen (Fe analogi)", color: "#248F8F" },
  Os: { name: "Osmiy (Os)", atomic: 76, mass: "190.23 u", config: "[Xe] 4f¹⁴ 5d⁶ 6s²", oxidation: "+2", role: "Osmotsen", color: "#266696" },
  C:  { name: "Uglerod (C)", atomic: 6, mass: "12.01 u", config: "[He] 2s² 2p²", role: "Cp halqa uglerodi (aromatik)", hybridization: "sp²", color: "#909090" },
  H:  { name: "Vodorod (H)", atomic: 1, mass: "1.008 u", config: "1s¹", role: "Cp halqadagi H atomi (radial yo'nalgan)", color: "#FFFFFF" },
  N:  { name: "Azot (N)", atomic: 7, mass: "14.01 u", config: "[He] 2s² 2p³", role: "—", color: "#3050F8" },
  O:  { name: "Kislorod (O)", atomic: 8, mass: "16.00 u", config: "[He] 2s² 2p⁴", role: "—", color: "#FF0D0D" },
  Cl: { name: "Xlor (Cl⁻)", atomic: 17, mass: "35.45 u", config: "[Ne] 3s² 3p⁶", charge: "-1", role: "—", color: "#1FF01F" },
  K:  { name: "Kaliy (K⁺)", atomic: 19, mass: "39.10 u", config: "[Ar]", charge: "+1", role: "—", color: "#8F40D4" },
  V:  { name: "Vanadiy (V)", atomic: 23, mass: "50.94 u", config: "[Ar] 3d³ 4s²", oxidation: "+4", role: "—", color: "#A6A6AB" }
}
...
// ASOSIY KOMPONENT
// ═══════════════════════════════════════════════════════════════════════════
export default function Ferrotsen3D() {
  ...
  const [currentComplex, setCurrentComplex] = useState("Ferrocene")
  ...
  // D₅d/D₅h uchun simmetriya elementlari: C₅ (asosiy), S₁₀, i (D₅d), σv (D₅h)
  const [symmetryElement, setSymmetryElement] = useState("C5") // C5 | S10 | sigma_h | sigma_v | inversion

  // ✨ FERROTSEN — rotamer holati (staggered = D₅d, eclipsed = D₅h)
  // Staggered (D₅d): kristallda barqaror, halqalar 36° burilgan
  // Eclipsed  (D₅h): gaz fazasida, halqalar bir-birining ustida
  const [conformation, setConformation] = useState("staggered")

  // Cp halqalarni interaktiv burish (0° = eclipsed, 36° = staggered)
  const [ringRotation, setRingRotation] = useState(36)
  ...
  useEffect(() => {
    ...
    // Simmetriya elementini yaroqli qilib saqlaymiz (D₅d/D₅h)
    if (!["C5", "S10", "sigma_h", "inversion"].includes(symmetryElement)) setSymmetryElement("C5")
  ...
  const buildSingleMolecule = useCallback((parent, complexData, centerPos = new THREE.Vector3(0, 0, 0), scale = 1) => {
    ...
    // ═══ SENDVICH (FERROTSEN) TUZILISHI ═══
    // Fe atomi markazda, ikkita C₅H₅⁻ (Cp) halqasi ±Y o'qi bo'ylab tekislikda joylashadi.
    // Cp halqalar bir-biriga nisbatan burilgan bo'ladi:
    //   • D₅d (staggered)  — kristallda, 36° burilish
    //   • D₅h (eclipsed)   — gaz fazasida, 0° burilish
    const centroidDist = complexData.ringCentroidDist   // Fe–Cp(centroid) masofasi
    const ringR        = complexData.ringRadius         // Cp halqa radiusi (M–C ≈ 2.04 Å)
    const bondMC       = complexData.bondLength         // M–C

    // Ikkita halqa markazi: yuqori (+Y) va pastki (−Y)
    const upperCentroid = new THREE.Vector3(0,  centroidDist, 0)
    const lowerCentroid = new THREE.Vector3(0, -centroidDist, 0)

    // Rotamer burchagi (yuqoridagi state):
    // conformationRef ni yaratamiz — real vaqtda o'zgartirish uchun
    // 0° = eclipsed, 36° = staggered
    const upperTwist =  0                                     // Yuqori halqa burilishi (base)
    const lowerTwist = (conformationRef.current === "eclipsed" ? 0 : 36) * Math.PI / 180

    // ═══ Cp HALQA — 5 ta C atomlari + 5 ta H atomlari + halqa bog'lari ═══
    const buildCpRing = (centroid, twist, ringIdx) => {
      const ringGroup = new THREE.Group()
      ringGroup.position.copy(centroid)
      ringGroup.userData = {
        type: 'cp_ring',
        ringIdx,                      // 0 = yuqori, 1 = pastki
      ...
      // Halqa tekisligi — XZ tekisligiga parallel; halqa normalisi Y bo'ylab
      // Yuqori halqa uchun H atomlari yuqoriga qaragan, pastki halqa uchun pastga
      const outUp = ringIdx === 0 ? 1 : -1

      const localAtoms = []
      const localHs = []

      // 5 ta C atomi — halqa aylanasi bo'ylab
      for (let i = 0; i < 5; i++) {
        const theta = (i * 2 * Math.PI / 5) + twist
        const cx = ringR * Math.cos(theta)
        const cz = ringR * Math.sin(theta)
        const cPos = new THREE.Vector3(cx, 0, cz)

        // C atomi
        const cGeo = new THREE.SphereGeometry(0.22, 40, 40)
        const cMat = new THREE.MeshStandardMaterial({
          color: CPK.C, roughness: 0.4, metalness: 0.15,
          emissive: 0x333333, emissiveIntensity: 0.05
        })
        const cMesh = new THREE.Mesh(cGeo, cMat)
        cMesh.position.copy(cPos)
        cMesh.castShadow = true
        cMesh.userData = {
          type: 'atom', element: 'C',
          info: ATOM_INFO.C, isDonor: true,
          ringIdx, atomIdx: i, ringPos: cPos.clone()
        }
        ringGroup.add(cMesh)
        atomsRef.current.push(cMesh)
        ligandAtomsRef.current.push(cMesh)
        localAtoms.push(cMesh)

        // C atom yorlig'i — C1–C5
        const cLabel = makeTextSprite(`C${i + 1}`, { color: "#d1d5db", scale: 0.28 })
        cLabel.position.copy(cPos).add(new THREE.Vector3(0, 0.35, 0))
        ringGroup.add(cLabel)
        labelsRef.current.push(cLabel)

        // H atomi — halqadan radial tashqi, tekislikda
        const hDir = cPos.clone().normalize()
        const hPos = cPos.clone().addScaledVector(hDir, 1.09)

        const hGeo = new THREE.SphereGeometry(0.14, 24, 24)
        const hMat = new THREE.MeshStandardMaterial({
          color: CPK.H, roughness: 0.6, metalness: 0.05
        })
        const hMesh = new THREE.Mesh(hGeo, hMat)
        hMesh.position.copy(hPos)
        hMesh.castShadow = true
        hMesh.userData = { type: 'atom', element: 'H', info: ATOM_INFO.H, ringIdx, atomIdx: i }
        ringGroup.add(hMesh)
        atomsRef.current.push(hMesh)
        localHs.push(hMesh)

        // C–H bog'
        const chBond = createBond(ringGroup, cPos, hPos, 0xbbbbbb, 0.04, 0.75)
        chBond.userData = { type: 'bond', bondType: 'C–H', length: complexData.chBondReal }
        bondsRef.current.push(chBond)
      }

      // C–C halqa bog'lari (5 ta, aromatik delokalizatsiyalangan)
      // Har bir bog' uchun ikkita chiziq — aromatik ikkilik ko'rsatilishi uchun
      for (let i = 0; i < 5; i++) {
        const a = localAtoms[i].position
        const b = localAtoms[(i + 1) % 5].position
        // Asosiy bog' (sp² skelet)
        const cc = createBond(ringGroup, a, b, CPK.cpRing, 0.07, 0.85)
        cc.userData = { type: 'bond', bondType: 'C–C (aromatik)', length: complexData.ccBondReal }
        bondsRef.current.push(cc)
      }

      // Aromatik markaz — nozik "delokalizatsiya doirasi" (torus)
      const torusGeo = new THREE.TorusGeometry(ringR * 0.72, 0.03, 8, 64)
      const torusMat = new THREE.MeshBasicMaterial({
        color: CPK.cpRing, transparent: true, opacity: 0.55
      })
      const aromRing = new THREE.Mesh(torusGeo, torusMat)
      aromRing.rotation.x = Math.PI / 2
      aromRing.userData = { type: 'aromatic_ring', ringIdx }
      ringGroup.add(aromRing)

      // Cp markaz atomi — kichik ko'rsatgich (centroid)
      const centGeo = new THREE.SphereGeometry(0.08, 24, 24)
      ...
      centMesh.userData = { type: 'centroid', ringIdx }
      ringGroup.add(centMesh)

      // Halqa yorlig'i
      const ringLabel = makeTextSprite(`Cp${ringIdx === 0 ? " (yuqori)" : " (pastki)"}`, {
        color: "#fed7aa", bgColor: "rgba(180, 83, 9, 0.85)",
        borderColor: "#fbbf24", scale: 0.35
      })
      ringLabel.position.set(0, outUp * 0.9, 0)
      ringGroup.add(ringLabel)
      labelsRef.current.push(ringLabel)

      ringGroup.userData.atoms = localAtoms
      ringGroup.userData.hAtoms = localHs

      return ringGroup
    }

    // Yuqori va pastki halqalarni yaratamiz
    const upperRing = buildCpRing(upperCentroid, upperTwist, 0)
    const lowerRing = buildCpRing(lowerCentroid, lowerTwist, 1)
    molGroup.add(upperRing)
    molGroup.add(lowerRing)

    // ═══ η⁵ KOORDINATSION BOG'LAR (Fe → 5 ta C) — har halqadan ═══
    const coPos = new THREE.Vector3(0, 0, 0)
    const ligandVectors = []

    ;[upperRing, lowerRing].forEach((ring, ringIdx) => {
      // Fe → halqa markazi — asosiy koordinatsion "o'q chizig'i"
      const centroidWorld = ring.position.clone()
      const centroidBond = createBond(molGroup, coPos, centroidWorld,
                                       CPK.metalRing, 0.13, 0.7)
      centroidBond.userData = {
        type: 'bond', bondType: `Fe–Cp (η⁵ markaz)`,
        length: complexData.ringCentroidReal, ringIdx, isCentroidBond: true
      }
      bondsRef.current.push(centroidBond)

      // Centroid uzunligi yorlig'i
      const midCent = new THREE.Vector3().addVectors(coPos, centroidWorld).multiplyScalar(0.5)
      const centLenLabel = makeTextSprite(complexData.ringCentroidReal, {
        color: "#fef3c7", bgColor: "rgba(180, 83, 9, 0.92)",
        borderColor: "#fbbf24", fontSize: 48, scale: 0.35
      })
      centLenLabel.position.copy(midCent).add(new THREE.Vector3(0.25, 0, 0))
      centLenLabel.visible = false
      molGroup.add(centLenLabel)
      bondLabelsRef.current.push(centLenLabel)

      // Fe → har bir C uchun mayin havolachi (5 ta, nozik)
      ring.userData.atoms.forEach((cAtom) => {
        const cWorldPos = cAtom.position.clone().add(centroidWorld)
        ligandVectors.push(cWorldPos.clone())

        const dashedGeo = new THREE.BufferGeometry().setFromPoints([coPos, cWorldPos])
        const dashedMat = new THREE.LineDashedMaterial({
          color: CPK.metalRing, dashSize: 0.12, gapSize: 0.08,
          transparent: true, opacity: 0.5
        })
        const dashedBond = new THREE.Line(dashedGeo, dashedMat)
        dashedBond.computeLineDistances()
        dashedBond.userData = { type: 'bond', bondType: 'Fe–C', length: complexData.bondLengthReal, ringIdx }
        molGroup.add(dashedBond)
        bondsRef.current.push(dashedBond)

        // M–C uzunlik yorlig'i (opsional — birinchi atomga)
        const midC = new THREE.Vector3().addVectors(coPos, cWorldPos).multiplyScalar(0.5)
        const mcLabel = makeTextSprite(complexData.bondLengthReal, {
          color: "#fed7aa", bgColor: "rgba(120, 53, 15, 0.85)",
          borderColor: "#fbbf24", fontSize: 48, scale: 0.28
        })
        mcLabel.position.copy(midC).add(new THREE.Vector3(0.1, 0.1, 0))
        mcLabel.visible = false
        molGroup.add(mcLabel)
        bondLabelsRef.current.push(mcLabel)
      })

      ring.userData.centroidBond = centroidBond
      localLigandGroups.push(ring)
    })
  ...
  const conformationRef = useRef(conformation)
  const ringRotationRef = useRef(ringRotation)

  useEffect(() => { conformationRef.current = conformation }, [conformation])
  ...
  useEffect(() => {
    ...
    setSelectedLigands([])
    setMeasuredAngle(null)
  }, [currentComplex, moleculeCount, ensembleMode, conformation, buildEnsemble])
  ...
  useEffect(() => {
    ...
    // ═══ D₅d/D₅h (SENDVICH FERROTSEN) uchun simmetriya elementlari ═══
    // D₅d (staggered): E, 2C₅, 2C₅², 5C₂, i (inversiya), 2S₁₀, 2S₁₀³, 5σd
    // D₅h (eclipsed):  E, 2C₅, 2C₅², 5C₂, σh,          2S₅, 2S₅³, 5σv
    if (symmetryElement === "C5") {
      // Asosiy C₅ o'qi — sendvich o'qi (Y bo'ylab, ikki halqa markazi orasidan)
      const dir = new THREE.Vector3(0, 1, 0)
      const points = [dir.clone().multiplyScalar(-len * 1.1), dir.clone().multiplyScalar(len * 1.1)]
      const geom = new THREE.BufferGeometry().setFromPoints(points)
      const mat = new THREE.LineBasicMaterial({ color: 0x44ff44, linewidth: 3, transparent: true, opacity: 0.85 })
      ...
      const label = makeTextSprite("C₅ (asosiy o'q)", {
        color: "#ffffff",
        bgColor: "rgba(68, 255, 68, 0.9)",
        borderColor: "#ffffff", scale: 0.45
      })
      label.position.copy(dir.clone().multiplyScalar(len * 1.2 + 0.3))
      label.userData = { type: 'symmetry' }
      scene.add(label)
      symmetryHelpersRef.current.push(label)

      // Yuqori va pastki halqa markazlarida aylanish yo'nalishi ko'rsatgichi
      ;[1.66, -1.66].forEach(yPos => {
        const ringGeo = new THREE.TorusGeometry(1.2, 0.04, 8, 60)
        const ringMat = new THREE.MeshBasicMaterial({
          color: 0x44ff44, transparent: true, opacity: 0.65
        })
    ...
    } else if (symmetryElement === "S10") {
      // S₁₀ = C₁₀ (36° aylanish) + σh (aks etish) — D₅d ga xos
      const dir = new THREE.Vector3(0, 1, 0)
      const points = [dir.clone().multiplyScalar(-len * 1.1), dir.clone().multiplyScalar(len * 1.1)]
      const geom = new THREE.BufferGeometry().setFromPoints(points)
      const mat = new THREE.LineDashedMaterial({
        color: 0xff44ff, dashSize: 0.2, gapSize: 0.15,
        transparent: true, opacity: 0.9
      })
      const line = new THREE.Line(geom, mat)
      line.computeLineDistances()
      ...
      const label = makeTextSprite("S₁₀ (D₅d)", {
        color: "#ffffff",
        bgColor: "rgba(255, 68, 255, 0.9)",
        borderColor: "#ffffff", scale: 0.45
      })
      label.position.copy(dir.clone().multiplyScalar(len * 1.2 + 0.3))
      label.userData = { type: 'symmetry' }
      ...
      const noteLabel = makeTextSprite("S₁₀ = C₁₀ + σh (36° + aks)", {
        color: "#fce7ff", bgColor: "rgba(120, 30, 120, 0.9)",
        borderColor: "#ff88ff", scale: 0.36
      })
      noteLabel.position.set(len * 1.4, 0, 0)
      noteLabel.userData = { type: 'symmetry' }
      scene.add(noteLabel)
      symmetryHelpersRef.current.push(noteLabel)
    } else if (symmetryElement === "sigma_h") {
      // σh (gorizontal tekislik) — D₅h holatida (eclipsed), Fe orqali o'tuvchi XZ tekislik
      ...
      const lbl = makeTextSprite("σh (Fe orqali)", {
        color: "#ffffff", bgColor: "rgba(0, 200, 200, 0.9)",
        borderColor: "#ffffff", scale: 0.42
      })
      lbl.position.set(len, 0.3, 0)
      lbl.userData = { type: 'symmetry' }
      scene.add(lbl)
      symmetryHelpersRef.current.push(lbl)

      const noteLbl = makeTextSprite("D₅h (eclipsed) ga xos", {
        color: "#e0ffff", bgColor: "rgba(0, 100, 100, 0.9)",
        borderColor: "#00ffff", scale: 0.34
      })
      noteLbl.position.set(len, -0.3, 0)
      noteLbl.userData = { type: 'symmetry' }
      scene.add(noteLbl)
      symmetryHelpersRef.current.push(noteLbl)
    } else if (symmetryElement === "inversion") {
      // Inversiya markazi (i) — faqat D₅d (staggered) da, Fe atomida
      ...
      const lbl = makeTextSprite("i (inversiya markazi)", {
        color: "#ffffff", bgColor: "rgba(255, 136, 68, 0.9)",
        borderColor: "#ffffff", scale: 0.4
      })
      lbl.position.set(0.5, 0.5, 0)
      lbl.userData = { type: 'symmetry' }
      scene.add(lbl)
      symmetryHelpersRef.current.push(lbl)

      const noteLbl = makeTextSprite("faqat D₅d (staggered) da bor", {
        color: "#ffe0cc", bgColor: "rgba(150, 80, 40, 0.9)",
        borderColor: "#ff8844", scale: 0.32
      })
      noteLbl.position.set(0.5, -0.2, 0)
      noteLbl.userData = { type: 'symmetry' }
      scene.add(noteLbl)
      symmetryHelpersRef.current.push(noteLbl)
    }
...
const generatePDF = async () => {
  ...
  try {
    ...
    const addFooter = () => {
      ...
      const leftText = truncate(
        `Sendvich (Ferrotsen) 3D Lab PRO  •  ${cleanText(complex.formula)}  •  ${new Date().toLocaleDateString("uz-UZ")}`,
    ...
    // D₅d sendvich uchun d-orbital konfiguratsiyasi:
    //   e₂g (dxy, dx²-y²) < a₁g (dz²) < e₁g* (dxz, dyz)
    const e2gSub     = String(complex.dOrbital.e2g     ?? 0).split("").map(d => subNum(+d)).join("")
    const a1gSub     = String(complex.dOrbital.a1g     ?? 0).split("").map(d => subNum(+d)).join("")
    const e1gStarSub = String(complex.dOrbital.e1gStar ?? 0).split("").map(d => subNum(+d)).join("")

    const abstract =
      `${cleanText(complex.formula)} — klassik sendvich (metallotsen) tipidagi metall-organik birikma. ` +
      `Markaziy ${cleanText(ATOM_INFO[complex.center.element].name.split(" ")[0])} atomi ikkita η⁵-koordinatsiyali ` +
      `siklopentadienil anioni (C₅H₅⁻, "Cp⁻") halqasi orasida joylashadi. ` +
      `Metall–halqa markaz masofasi ${cleanText(complex.ringCentroidReal)}, o'rtacha M–C bog' uzunligi ${cleanText(complex.bondLengthReal)}, ` +
      `Cp halqadagi C–C bog' uzunligi ${cleanText(complex.ccBondReal)} (aromatik, delokalizatsiyalangan). ` +
      `Kristallda ${cleanText(complex.symmetry)} nuqtaviy guruh ("staggered", 36° burilgan) simmetriyasi kuzatiladi; ` +
      `gaz fazasida esa D₅h ("eclipsed") holat barqaror — bu ikki rotamer orasidagi bar'er atigi ≈3.8 kJ/mol. ` +
      `D₅d simmetriyasida d-orbitallar 3 sathga ajraladi: e₂g (dxy, dx²-y²) < a₁g (dz²) < e₁g* (dxz, dyz); ` +
      `Δ1 (e₂g → a₁g) ≈ ${(complex.dOrbital.delta1||0).toLocaleString()} cm⁻¹, ` +
      `Δ2 (a₁g → e₁g*) ≈ ${(complex.dOrbital.delta2||0).toLocaleString()} cm⁻¹. ` +
      `Elektron konfiguratsiya: (e₂g)${e2gSub} (a₁g)${a1gSub} (e₁g*)${e1gStarSub}, jami valent elektronlar soni ${complex.valenceElectrons} ` +
      `(18-elektron qoidasi ${complex.valenceElectrons === 18 ? "to'liq bajariladi — diamagnit, favqulodda barqaror" : "buziladi — e₁g* antibog'lovchi orbitalda elektronlar bor, paramagnit"}). ` +
      `Ferrotsen 1951-yilda Kealy va Pauson tomonidan sintez qilingan, uning η⁵-sendvich tuzilishi esa 1952-yilda ` +
      `E. O. Fischer, G. Wilkinson va R. B. Woodward tomonidan mustaqil ravishda tushuntirilgan; ` +
      `bu ish 1973-yilda Nobel kimyo mukofotiga sazovor bo'lgan va zamonaviy metall-organik kimyoning boshlanishini belgilagan.`
    ...
    if (pdfSections.snapshot) {
      ...
      if (renderer && sceneRef.current && cameraRef.current) {
        ...
        const caption =
          ...
          `ko'rinishidagi 3D modeli. Sendvich (metallotsen) tuzilishi, ${cleanText(complex.symmetry)} simmetriyasida ${conformation === "staggered" ? "'staggered' (36° burilgan)" : "'eclipsed' (ustma-ust)"} rotamer. ${moleculeCount > 1 ? `${moleculeCount} ta molekula ${ensembleMode} ansamblida.` : "Bitta molekula."}`
    ...
    if (pdfSections.info) {
      ...
      const infoTable = [
        ...
        ["To'liq nomi (IUPAC)", complex.name],
        ["Sinf", "Metallotsen (sendvich birikmasi)"],
        ["Koordinatsiya", "η⁵ (har halqadan 5 ta uglerod — jami 10 e⁻ donor)"],
        ["Molekulyar simmetriya (kristall)", `${complex.symmetry}  (staggered, 36° burilgan)`],
        ["Molekulyar simmetriya (gaz fazasi)", "D₅h  (eclipsed)"],
        ["Rotamer bar'eri", "≈3.8 kJ/mol — xona t. da erkin aylanadi"],
        ["Fe–Cp centroid masofasi", complex.ringCentroidReal],
        [`${complex.center.element}–C bog' uzunligi (o'rt.)`, complex.bondLengthReal],
        ["C–C halqa bog'i (delok.)", complex.ccBondReal],
        ["C–H bog'i", complex.chBondReal],
        ["Gibridlanish", complex.hybridization],
        ["Metall oksidlanish darajasi", complex.center.charge],
        ["d-elektronlar", `d${complex.dElectrons}`],
        ["Valent elektronlar (jami)", `${complex.valenceElectrons} e⁻  ${complex.valenceElectrons === 18 ? "(18-e⁻ qoida bajariladi)" : "(qoida buziladi)"}`],
    ...
    if (pdfSections.dorbital) {
      drawSectionHeader(sectionNum++, "Kristall Maydon d-Orbital Ajralishi (D₅d sendvich)")
      checkPageBreak(200)

      // Diagramma joylashuvi — D₅d 3 sath: e₂g (past) < a₁g (o'rta) < e₁g* (yuqori)
      const diagX = MARGIN + 30
      const e1gStarY = y - 20
      const a1gY_D5  = y - 80
      const e2gY_D5  = y - 140
      // Eski nomlar bilan mos kelish uchun
      const b1Y  = e1gStarY
      const a1Y  = a1gY_D5
      const b2Y  = e2gY_D5
      const eY   = e2gY_D5 - 20
      ...
      // ═══ D₅d 3 sath: e₁g* (eng yuqori), a₁g, e₂g (eng past) ═══
      const orbLineW = 42
      const singleX  = diagX + 30
      // e₁g* (dxz, dyz) — eng yuqori, ikki orbital yonma-yon
      const eStar1X = diagX + 20
      const eStar2X = diagX + 70
      page.drawLine({ start: { x: eStar1X, y: e1gStarY }, end: { x: eStar1X + orbLineW, y: e1gStarY }, thickness: 2, color: C.red })
      page.drawLine({ start: { x: eStar2X, y: e1gStarY }, end: { x: eStar2X + orbLineW, y: e1gStarY }, thickness: 2, color: C.red })
      page.drawText("e₁g*", { x: diagX + 128, y: e1gStarY - 3, size: 11, font: boldFont, color: C.red })
      page.drawText("dxz", { x: eStar1X + 10, y: e1gStarY + 6, size: 7, font: regularFont, color: C.textMuted })
      page.drawText("dyz", { x: eStar2X + 10, y: e1gStarY + 6, size: 7, font: regularFont, color: C.textMuted })
      page.drawText("antibog'", { x: diagX + 155, y: e1gStarY + 6, size: 7, font: italicFont, color: C.textMuted })

      // a₁g (dz²) — o'rtada
      page.drawLine({ start: { x: singleX, y: a1gY_D5 }, end: { x: singleX + orbLineW, y: a1gY_D5 }, thickness: 2, color: C.orange })
      page.drawText("a₁g", { x: singleX + orbLineW + 8, y: a1gY_D5 - 3, size: 11, font: boldFont, color: C.orange })
      page.drawText("dz²", { x: singleX + 10, y: a1gY_D5 + 6, size: 7, font: regularFont, color: C.textMuted })
      page.drawText("kuchsiz σ*", { x: diagX + 155, y: a1gY_D5 - 3, size: 7, font: italicFont, color: C.textMuted })

      // e₂g (dxy, dx²-y²) — eng past, ikki orbital yonma-yon
      const e2g1X = diagX + 20
      const e2g2X = diagX + 70
      page.drawLine({ start: { x: e2g1X, y: e2gY_D5 }, end: { x: e2g1X + orbLineW, y: e2gY_D5 }, thickness: 2, color: C.blue })
      page.drawLine({ start: { x: e2g2X, y: e2gY_D5 }, end: { x: e2g2X + orbLineW, y: e2gY_D5 }, thickness: 2, color: C.blue })
      page.drawText("e₂g", { x: diagX + 128, y: e2gY_D5 - 3, size: 11, font: boldFont, color: C.blue })
      page.drawText("dxy", { x: e2g1X + 10, y: e2gY_D5 + 6, size: 7, font: regularFont, color: C.textMuted })
      page.drawText("dx²-y²", { x: e2g2X + 4, y: e2gY_D5 + 6, size: 7, font: regularFont, color: C.textMuted })
      page.drawText("bog'lanmagan", { x: diagX + 155, y: e2gY_D5 - 3, size: 7, font: italicFont, color: C.textMuted })
      ...
      // Elektronlarni to'ldirish (past dan yuqoriga)
      const e2gVal     = complex.dOrbital.e2g || 0
      const a1gVal_D5  = complex.dOrbital.a1g || 0
      const e1gStarVal = complex.dOrbital.e1gStar || 0
      // e₂g (4 slot)
      drawSlot(e2g1X, orbLineW, e2gY_D5, Math.min(2, e2gVal))
      drawSlot(e2g2X, orbLineW, e2gY_D5, Math.max(0, e2gVal - 2))
      // a₁g (2 slot)
      drawSlot(singleX, orbLineW, a1gY_D5, a1gVal_D5)
      // e₁g* (4 slot)
      drawSlot(eStar1X, orbLineW, e1gStarY, Math.min(2, e1gStarVal))
      drawSlot(eStar2X, orbLineW, e1gStarY, Math.max(0, e1gStarVal - 2))

      // ═══ Δ1 STRELKASI (e₂g → a₁g) ═══
      const arX = diagX + 220
      page.drawLine({ start: { x: arX, y: a1gY_D5 }, end: { x: arX, y: e2gY_D5 }, thickness: 1.5, color: C.orangeDeep })
      page.drawLine({ start: { x: arX - 3, y: a1gY_D5 - 5 }, end: { x: arX, y: a1gY_D5 }, thickness: 1.5, color: C.orangeDeep })
      page.drawLine({ start: { x: arX + 3, y: a1gY_D5 - 5 }, end: { x: arX, y: a1gY_D5 }, thickness: 1.5, color: C.orangeDeep })
      page.drawLine({ start: { x: arX - 3, y: e2gY_D5 + 5 }, end: { x: arX, y: e2gY_D5 }, thickness: 1.5, color: C.orangeDeep })
      page.drawLine({ start: { x: arX + 3, y: e2gY_D5 + 5 }, end: { x: arX, y: e2gY_D5 }, thickness: 1.5, color: C.orangeDeep })
      const mid1Y = (a1gY_D5 + e2gY_D5) / 2
      ...
      // ═══ Δ2 STRELKASI (a₁g → e₁g*) ═══
      page.drawLine({ start: { x: arX, y: e1gStarY }, end: { x: arX, y: a1gY_D5 }, thickness: 1.5, color: C.green })
      page.drawLine({ start: { x: arX - 3, y: e1gStarY - 5 }, end: { x: arX, y: e1gStarY }, thickness: 1.5, color: C.green })
      page.drawLine({ start: { x: arX + 3, y: e1gStarY - 5 }, end: { x: arX, y: e1gStarY }, thickness: 1.5, color: C.green })
      const mid2Y = (e1gStarY + a1gY_D5) / 2
      ...
      safeText(`Konfiguratsiya:`, {
        x: infoX, y: e1gStarY, size: 8.5, font: boldFont, color: C.textDark, maxWidth: infoMaxW,
      })
      safeText(`(e₂g)${e2gSub} (a₁g)${a1gSub} (e₁g*)${e1gStarSub}`, {
        x: infoX, y: e1gStarY - 13, size: 9, font: regularFont, color: C.textDark, maxWidth: infoMaxW,
      })

      safeText(`Valent e⁻:`, {
        x: infoX, y: a1gY_D5 + 5, size: 8.5, font: boldFont, color: C.textDark, maxWidth: infoMaxW,
      })
      safeText(`${complex.valenceElectrons} e⁻ ${complex.valenceElectrons === 18 ? "→ barqaror" : "→ antibog'ga"}`, {
        x: infoX, y: a1gY_D5 - 8, size: 9, font: regularFont, color: C.textDark, maxWidth: infoMaxW,
      })

      safeText(`Spin holati:`, {
        x: infoX, y: e2gY_D5 + 5, size: 8.5, font: boldFont, color: C.textDark, maxWidth: infoMaxW,
      })
      safeText(complex.dOrbital.type === "LS" ? "Past spin (LS) — diamagnit" :
               complex.dOrbital.type === "HS" ? "Yuqori spin (HS) — paramagnit" : "—", {
        x: infoX, y: e2gY_D5 - 8, size: 8.5, font: regularFont, color: C.textDark, maxWidth: infoMaxW,
      })

      // Umumiy y ni pastga tushiramiz
      y = e2gY_D5 - 45
      const caption = `2-rasm. ${cleanText(complex.formula)} uchun D₅d (sendvich) kristall maydon ajralish diagrammasi. Cp⁻ halqasining π-orbitallari bilan metall d-orbitallarining o'zaro ta'siri natijasida d-orbitallar 3 sathga ajraladi: e₂g (dxy, dx²-y² — bog'lanmagan) < a₁g (dz² — kuchsiz σ*) < e₁g* (dxz, dyz — antibog'lovchi π*). Ferrotsen uchun (e₂g)⁴(a₁g)² konfiguratsiya — barcha 6 valent d-elektron pastki 3 orbitalda joylashadi, e₁g* bo'sh qoladi (18-elektron qoidasi).`
      const capLines = wrapText(cleanText(caption), italicFont, 8.5, CONTENT_W)
      capLines.forEach((line, i) => {
        page.drawText(line, { x: MARGIN, y: y - i * 11, size: 8.5, font: italicFont, color: C.purpleSoft })
      })
    ...
    if (pdfSections.mo) {
      ...
      // D₅d sendvich (ferrotsen) uchun MO sxemasi — Ballhausen–Dahl (1961)
      // Cp⁻ (π-orbital simmetriya): a₁g, e₁g, e₂g (har halqadan 5 ta MO)
      // Metall d-orbitallar: a₁g (dz²), e₁g (dxz, dyz), e₂g (dxy, dx²-y²)
      const moLevels = [
        { label: "σ* (a₁g) — antibog' (4s + Cpπ)",          fill: 0 },
        { label: "σ* (e₁u) — antibog' (4p + Cpπ)",         fill: 0 },
        { label: "π* (e₁g) — antibog' (dxz, dyz)  ← LUMO",  fill: complex.dOrbital.e1gStar || 0 },
        { label: "σnb (a₁g) — kuchsiz σ* (dz²)  ← HOMO",   fill: complex.dOrbital.a1g || 0 },
        { label: "πnb (e₂g) — bog'lanmagan (dxy, dx²-y²)", fill: complex.dOrbital.e2g || 0 },
        { label: "π (e₁g) — bog'lovchi (Cpπ + d)",          fill: 4 },
        { label: "σ (e₁u) — bog'lovchi (Cpπ + p)",           fill: 4 },
        { label: "σ (a₁g) — bog'lovchi (Cpπ + s)",           fill: 2 },
      ]
      ...
      const caption = `3-rasm. Ferrotsen (D₅d) uchun Ballhausen–Dahl (1961) molekulyar orbital diagrammasi. Ikkita Cp⁻ halqasining aromatik π-orbitallari (jami 10 ta) metallning s, p va d-orbitallari bilan simmetriya bo'yicha mos keluvchi qismlarga birlashadi: a₁g, a₂u, e₁g, e₁u, e₂g, e₂u. Fe(II) d⁶ orbital elektronlar HOMO sathigacha to'ldiriladi: pastki 3 metall-markazlashgan orbital (e₂g⁴, a₁g²) to'la, e₁g* antibog'lovchi bo'sh. Jami 18 valent elektron — barqaror.`
      const capLines = wrapText(cleanText(caption), italicFont, 8.5, CONTENT_W)
      capLines.forEach((line, i) => {
        page.drawText(line, { x: MARGIN, y: y - i * 11, size: 8.5, font: italicFont, color: C.purpleSoft })
      })
    ...
    if (pdfSections.crystalField) {
      ...
      // D₅d sendvich uchun elektron barqarorlashuv energiyasi
      // Barotsentr atrofida taxminiy koeffitsientlar (Ballhausen–Dahl 1961)
      const e2gEl     = complex.dOrbital.e2g     || 0
      const a1gEl     = complex.dOrbital.a1g     || 0
      const e1gStarEl = complex.dOrbital.e1gStar || 0
      const cfse = -0.60 * e2gEl - 0.10 * a1gEl + 0.85 * e1gStarEl
      const d1  = complex.dOrbital.delta1 || 0
      const d2  = complex.dOrbital.delta2 || 0
      const cfData = [
        ["Ligand tipi", "Cp⁻ (siklopentadienil anioni) — η⁵, aromatik π-donor + π-akseptor"],
        ["Ligand maydon kuchi", "Juda kuchli (spektrxem. qatorda CN⁻ bilan taqqoslash mumkin)"],
        ["Δ1 (e₂g → a₁g)", `${d1.toLocaleString()} cm⁻¹  (≈ ${(d1 * 0.012).toFixed(1)} kJ/mol)`],
        ["Δ2 (a₁g → e₁g*)", `${d2.toLocaleString()} cm⁻¹  (≈ ${(d2 * 0.012).toFixed(1)} kJ/mol)`],
        ["KMBE (Δ1 birligida)", `${cfse.toFixed(2)} Δ1`],
        ["KMBE (energiya)", `${(cfse * d1 * 0.012).toFixed(1)} kJ/mol`],
        ["Konfiguratsiya", `(e₂g)${e2gSub} (a₁g)${a1gSub} (e₁g*)${e1gStarSub}`],
        ["Valent elektronlar", `${complex.valenceElectrons} e⁻  —  ${complex.valenceElectrons === 18 ? "18-elektron qoidasi bajariladi (barqaror)" : `qoida buziladi (${complex.valenceElectrons - 18} ortiqcha e⁻ antibog'da)`}`],
        ["Magnit xossa", complex.magnetism],
        ["Termik barqarorlik", "Ferrotsen: >500°C, havoda barqaror; sulf. kisl.da protonlanadi (FpH⁺)"],
      ]
    ...
    if (pdfSections.references) {
      ...
      const refs = [
        "1. Kealy, T. J.; Pauson, P. L. (1951). A New Type of Organo-Iron Compound. Nature, 168(4285), 1039–1040. [Ferrotsenning birinchi sintezi]",
        "2. Miller, S. A.; Tebboth, J. A.; Tremaine, J. F. (1952). Dicyclopentadienyliron. J. Chem. Soc., 632–635. [Mustaqil kashfiyot]",
        "3. Wilkinson, G.; Rosenblum, M.; Whiting, M. C.; Woodward, R. B. (1952). The Structure of Iron Bis-cyclopentadienyl. J. Am. Chem. Soc., 74(8), 2125–2126. [Sendvich tuzilmaning kashf etilishi]",
        "4. Fischer, E. O.; Pfab, W. (1952). Zur Kristallstruktur der Di-Cyclopentadienyl-Verbindungen. Z. Naturforsch. B, 7(7), 377–379. [Rentgen tuzilma]",
        "5. Dunitz, J. D.; Orgel, L. E.; Rich, A. (1956). The Crystal Structure of Ferrocene. Acta Cryst., 9, 373–375. [Staggered D₅d konformatsiyasining aniqlanishi]",
        "6. Ballhausen, C. J.; Dahl, J. P. (1961). Bonding in Ferrocene. Acta Chem. Scand., 15, 1333–1336. [Ferrotsen uchun MO diagramma]",
        "7. Haaland, A.; Nilsson, J. E. (1968). The Determination of Barriers to Internal Rotation. Acta Chem. Scand., 22, 2653–2670. [Gaz fazasida eclipsed D₅h]",
        "8. Seiler, P.; Dunitz, J. D. (1979). A New Interpretation of the Disordered Crystal Structure of Ferrocene. Acta Cryst. B, 35, 1068–1074.",
        "9. Nobel Prize in Chemistry (1973). E. O. Fischer & G. Wilkinson: 'Pioneering work on the chemistry of the organometallic sandwich compounds'.",
        "10. Elschenbroich, C. (2006). Organometallics, 3rd ed. Wiley-VCH. [Ferrotsen bo'limi]",
        "11. Crabtree, R. H. (2019). The Organometallic Chemistry of the Transition Metals, 7th ed. Wiley. [η⁵-koordinatsiya, 18-e⁻ qoidasi]",
        "12. Astruc, D. (2007). Organometallic Chemistry and Catalysis. Springer. [Ferrotsenning sanoat qo'llanilishi]",
        "13. IUPAC. (2005). Nomenclature of Inorganic Chemistry: Recommendations 2005. RSC Publishing.",
      ]
      refs.forEach(ref => {
    ...
    pdfDoc.setKeywords([complex.geometry, complex.symmetry, "sendvich", "ferrotsen", "metallotsen", "eta5", "18 elektron qoidasi", "metall-organik kimyo", "IUPAC"])
  ...
  url    = {https://jda-kimyo.uz/oquv/fazoviy/sendvich},
  urldate = {${accessDate}}
}`
    ...
    <main className="min-h-screen bg-gradient-to-b from-purple-950 via-indigo-950 to-blue-950 text-white flex flex-col">
      ...
      <header className="flex items-center justify-between px-4 sm:px-6 py-3 border-b border-purple-800/50 z-30 bg-purple-950/80 backdrop-blur-md">
        <div className="flex items-center gap-3 sm:gap-4 min-w-0">
          ...
          <div className="min-w-0">
            <h1 className="text-base sm:text-xl font-bold text-purple-300 flex items-center gap-2 truncate">
              <span>🧈</span>
              <span className="hidden sm:inline">Sendvich (Ferrotsen) — 3D Laboratoriya PRO</span>
              <span className="sm:hidden">Ferrotsen 3D Lab PRO</span>
            </h1>
        ...
        <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
          ...
          >
            <option value="Ferrocene">[Fe(η⁵-C₅H₅)₂] — Ferrotsen</option>
            <option value="Nickelocene">[Ni(η⁵-C₅H₅)₂] — Nikelotsen</option>
          </select>
      ...
      <div className="flex-1 flex flex-row relative overflow-hidden">
        ...
        >
          ...
          {/* ═══ FERROTSEN — KONFORMATSION BOSHQARUV ═══ */}
          <div className="bg-gradient-to-r from-orange-900/30 to-amber-900/30 rounded-lg p-2 border border-orange-700/40 mb-2">
            <h4 className="text-[10px] text-orange-400 uppercase mb-2 font-bold flex items-center gap-1">
              <span>🔄</span> Cp halqalar konformatsiyasi
            </h4>
            <div className="grid grid-cols-2 gap-1 mb-2">
              <button
                onClick={() => { setConformation("staggered"); setRingRotation(36) }}
                className={`p-1.5 rounded text-[10px] font-bold transition-all ${
                  conformation === "staggered"
                    ? 'bg-orange-600 text-white shadow-lg'
                    : 'bg-purple-900/50 text-purple-300 hover:bg-purple-800'
                }`}
                title="Kristallda barqaror: halqalar 36° burilgan"
              >Staggered <span className="text-[9px] opacity-80">D₅d</span></button>
              <button
                onClick={() => { setConformation("eclipsed"); setRingRotation(0) }}
                className={`p-1.5 rounded text-[10px] font-bold transition-all ${
                  conformation === "eclipsed"
                    ? 'bg-amber-500 text-white shadow-lg'
                    : 'bg-purple-900/50 text-purple-300 hover:bg-purple-800'
                }`}
                title="Gaz fazasida: halqalar ustma-ust"
              >Eclipsed <span className="text-[9px] opacity-80">D₅h</span></button>
            </div>
            <div className="text-[9px] text-orange-300 leading-snug">
              <div>• Kristall (X-ray): <b>Staggered</b> (36°)</div>
              <div>• Gaz elektron diff.: <b>Eclipsed</b> (0°)</div>
              <div>• Bar'er: ≈3.8 kJ/mol — xona t. da erkin aylanadi</div>
            </div>
          </div>
            ...
            <div className="space-y-2 mb-2 pl-1">
              ...
              {showLigandExchange && (
                <div className="ml-2 mt-1 space-y-1 bg-purple-900/30 p-2 rounded">
                  ...
                  >
                    <option value="H2O">Bir Cp⁻ → CO (1-tishli)</option>
                    <option value="Cl">Bir Cp⁻ → halid (η⁵→η¹)</option>
                  </select>
              ...
              {showRedox && (
                <div className="ml-2 mt-1 bg-purple-900/30 p-2 rounded">
                  ...
                  <div className="grid grid-cols-3 gap-1">
                    {(complex.center.element === "Fe" ? [2, 3, 4] :
                      complex.center.element === "Ni" ? [2, 3, 4] :
                      [2, 3, 4]).map(ox => (
                      <button
                        key={ox}
          ...
          {expandedSection === "scientific" && (
            <div className="space-y-2 pl-1">
              ...
              <ToggleRow label="🔀 Konformatsion izomerlar" value={showIsomers} onChange={setShowIsomers} />
              {showIsomers && (
                <div className="ml-2 mt-1 bg-purple-900/30 p-2 rounded">
                  <select
                    value={isomerType}
                  ...
                  >
                    ...
                    <option value="staggered">Staggered (D₅d) — kristallda</option>
                    <option value="eclipsed">Eclipsed (D₅h) — gaz fazasida</option>
                    <option value="tilt">Bent (egik) — [Cp₂M(L)₂] tipi</option>
                    <option value="h1">η⁵ → η³ → η¹ slippage</option>
                  </select>
                  <div className="mt-1 text-[9px] text-purple-400">
                    D₅d va D₅h orasidagi bar'er: ≈3.8 kJ/mol
                  </div>
                </div>
              )}
          ...
          <div className="absolute top-16 left-1/2 -translate-x-1/2 z-25 space-y-1 flex flex-col items-center">
            {showJahnTeller && (
              <div className="bg-pink-950/90 backdrop-blur-md px-3 py-1.5 rounded-lg text-xs text-pink-200 border border-pink-700/50">
                📐 Halqa slippage (η⁵ → η³): metallotsenlarda kuzatiladi
              </div>
            )}
            {showIsomers && isomerType !== "none" && (
              <div className="bg-indigo-950/90 backdrop-blur-md px-3 py-1.5 rounded-lg text-xs text-indigo-200 border border-indigo-700/50">
                🔀 {isomerType.toUpperCase()}: {
                  isomerType === "staggered" ? "Halqalar 36° burilgan (D₅d) — kristallda barqaror" :
                  isomerType === "eclipsed"  ? "Halqalar ustma-ust (D₅h) — gaz fazasida" :
                  isomerType === "tilt"      ? "Egik metallotsen: Cp'–M–Cp bur. < 180°" :
                  "η⁵–η³–η¹ slippage — halqa ‘sirg\u2019aladi’ (aromatik→allil→vinil)"
                }
              </div>
            )}
          </div>
          ...
          {!fullscreenMode && !selectedAtom && activePanel === "info" && (
            <div className="absolute top-3 right-3 bg-purple-950/95 backdrop-blur-md rounded-xl p-4 z-25 border border-purple-700/50 max-w-sm w-[300px] shadow-2xl animate-slide-in">
              ...
              <div className="space-y-2 text-xs">
                ...
                <InfoRow label="Simmetriya" value={`${complex.symmetry}  (${conformation === "staggered" ? "D₅d" : "D₅h"})`} mono />
                <InfoRow label="Konformatsiya" value={conformation === "staggered" ? "Staggered (36°)" : "Eclipsed (0°)"} />
                <InfoRow label="Koordinatsiya" value={`η⁵ (2 × 5 = 10 e⁻ donor)`} mono />
                <InfoRow label="Gibridlanish" value={complex.hybridization} small mono />
                <InfoRow label={`${complex.center.element}–Cp centroid`} value={complex.ringCentroidReal} mono />
                <InfoRow label={`${complex.center.element}–C bog'`} value={complex.bondLengthReal} mono />
                <InfoRow label="C–C (halqa)" value={complex.ccBondReal} mono />
                <InfoRow label="C–H" value={complex.chBondReal} mono />
                <InfoRow label="18-e⁻ qoida" value={`${complex.valenceElectrons} e⁻  ${complex.valenceElectrons === 18 ? "✅ barqaror" : "⚠️ antibog'da"}`} small mono />
                <InfoRow label="d-elektronlar" value={`d${complex.dElectrons}  —  ${complex.center.element}(${complex.center.charge})`} mono />
                <InfoRow label="Magnit xossa" value={complex.magnetism} small />
                <InfoRow label="Rangi" value={complex.color} small />
                <InfoRow label="Δ1 (e₂g→a₁g)" value={`${complex.dOrbital.delta1.toLocaleString()} cm⁻¹`} mono />
                <InfoRow label="Δ2 (a₁g→e₁g*)" value={`${complex.dOrbital.delta2.toLocaleString()} cm⁻¹`} mono />
              </div>
            </div>
          )}
            ...
            <div className="absolute top-3 right-3 bg-purple-950/95 backdrop-blur-md rounded-xl p-4 z-25 border border-purple-700/50 shadow-2xl w-[320px] animate-slide-in">
              <div className="flex justify-between items-start mb-3">
                <h3 className="text-sm font-bold text-purple-300 flex items-center gap-2">
                  <span>⚛️</span> d-orbital (D₅d sendvich)
                </h3>
                <button onClick={() => setActivePanel(null)} className="text-purple-400 hover:text-white text-xl leading-none">×</button>
              </div>

              {/* D₅d sendvich uchun 3 sathli d-orbital diagramma */}
              {/* Tartib past→yuqori: e₂g (nb) < a₁g (nb/kuchsiz σ*) < e₁g* (antibog') */}
              <div className="relative h-56 flex flex-col justify-between text-xs">
                {[
                  { key: 'e1gStar', label: 'e₁g* (dxz, dyz)', slots: 4, val: complex.dOrbital.e1gStar, color: 'text-red-300', tag: 'antibog' },
                  { key: 'a1g',     label: 'a₁g (dz²)',        slots: 2, val: complex.dOrbital.a1g,     color: 'text-orange-300', tag: 'kuchsiz' },
                  { key: 'e2g',     label: 'e₂g (dxy, x²-y²)', slots: 4, val: complex.dOrbital.e2g,   color: 'text-cyan-300', tag: 'bog-nomas' }
                ].map((row) => (
                ...
                <div className="text-center text-[10px] text-purple-500 mt-1">
                  ↕ Δ1 (e₂g → a₁g) ≈ {complex.dOrbital.delta1.toLocaleString()} cm⁻¹<br/>
                  ↕ Δ2 (a₁g → e₁g*) ≈ {complex.dOrbital.delta2.toLocaleString()} cm⁻¹
                </div>
              </div>
              ...
              <div className="mt-3 pt-2 border-t border-purple-800/50 text-[10px] text-purple-400 space-y-1">
                <div>Konfiguratsiya: <span className="text-white font-mono">
                  (e₂g)^{complex.dOrbital.e2g} (a₁g)^{complex.dOrbital.a1g} (e₁g*)^{complex.dOrbital.e1gStar}
                </span></div>
                <div>Valent e⁻: <span className="text-white font-bold">{complex.valenceElectrons} e⁻
                  {complex.valenceElectrons === 18 ? " → barqaror" : " → antibog'da bor"}
                </span></div>
                <div>Spin: <span className="text-white font-mono">
                  {complex.dOrbital.type === "LS" ? "Past spin (LS)" :
                   complex.dOrbital.type === "HS" ? "Yuqori spin (HS)" : "—"}
                </span></div>
                <div>Magnit: <span className="text-white">{complex.magnetism}</span></div>
                <div className="text-[9px] text-purple-500 italic mt-1">
                  Ferrotsen (18e⁻): e₂g⁴ a₁g² → diamagnit.<br/>
                  Nikelotsen (20e⁻): 2 e⁻ e₁g* ga → paramagnit.
                </div>
              </div>
            </div>
          )}
          ...
          {!fullscreenMode && !selectedAtom && activePanel === "mo" && (
            <div className="absolute top-3 right-3 bg-purple-950/95 backdrop-blur-md rounded-xl p-4 z-25 border border-pink-700/50 shadow-2xl w-[300px] animate-slide-in">
              ...
              <div className="space-y-1 text-[10px]">
                <div className="text-[9px] text-pink-400 italic pb-1">Ferrotsen MO tartibi (Ballhausen–Dahl 1961)</div>
                <div className="flex justify-between bg-purple-900/40 px-2 py-1 rounded"><span className="text-purple-400">σ* (a₁g)</span><span className="text-white">4s ligand</span></div>
                <div className="flex justify-between bg-purple-900/40 px-2 py-1 rounded"><span className="text-purple-400">π* (e₁u)</span><span className="text-white">4p ligand</span></div>
                <div className="flex justify-between bg-red-900/30 px-2 py-1 rounded border border-red-700/30"><span className="text-red-300">π* (e₁g)</span><span className="text-white font-mono">dxz, dyz {complex.dOrbital.e1gStar > 0 ? `↑↑ (${complex.dOrbital.e1gStar}e⁻)` : "—"}</span></div>
                ...
                <div className="flex justify-between bg-orange-900/30 px-2 py-1 rounded"><span className="text-orange-300">σnb (a₁g)</span><span className="text-white font-mono">dz² ↑↓ ({complex.dOrbital.a1g}e⁻)</span></div>
                ...
                <div className="flex justify-between bg-cyan-900/30 px-2 py-1 rounded"><span className="text-cyan-300">πnb (e₂g)</span><span className="text-white font-mono">dxy, dx²-y² ↑↓↑↓ ({complex.dOrbital.e2g}e⁻)</span></div>
                <div className="flex justify-between bg-purple-900/40 px-2 py-1 rounded"><span className="text-purple-400">σ (e₁u)</span><span className="text-white">Cpπ + M(p)</span></div>
                <div className="flex justify-between bg-purple-900/40 px-2 py-1 rounded"><span className="text-purple-400">σ (a₁g)</span><span className="text-white">Cpπ(a₁g) + s</span></div>
                <div className="border-t border-pink-700/30 mt-2 pt-2 text-pink-200 text-center">{complex.formula} — {complex.symmetry} — {complex.valenceElectrons}e⁻</div>
              </div>
            </div>
          )}
            ...
            <div className="absolute top-3 right-3 bg-purple-950/95 backdrop-blur-md rounded-xl p-4 z-25 border border-cyan-700/50 shadow-2xl w-[320px] animate-slide-in">
              ...
              <div className="bg-purple-900/50 rounded p-3">
                ...
                <div className="text-[10px] text-purple-300 mt-2 leading-relaxed">
                  {spectrumType === "uv-vis" && (currentComplex === "Ferrocene"
                    ? "Ferrotsen: λ₁ ≈ 440 nm (e₂g → e₁g*)  •  λ₂ ≈ 325 nm (a₁g → e₁g*)  •  λ₃ ≈ 262 nm (CT)"
                    : "Nikelotsen: λ₁ ≈ 715 nm  •  λ₂ ≈ 520 nm  •  yashil rang kelib chiqishi")}
                  {spectrumType === "ir" && (currentComplex === "Ferrocene"
                    ? "ν(C–H): 3100 cm⁻¹  •  ν(C=C): 1410, 1108 cm⁻¹  •  γ(CH): 1005 cm⁻¹  •  ν(Fe–Cp): 478, 492 cm⁻¹"
                    : "ν(C–H): 3080 cm⁻¹  •  ν(Ni–Cp): 355 cm⁻¹ (Fe dan kalta)")}
                  {spectrumType === "nmr" && (currentComplex === "Ferrocene"
                    ? "¹H NMR: singl. 4.16 ppm (10H)  •  ¹³C: 68.2 ppm  •  ⁵⁷Fe: ∕1600 ppm (aromatik shielding)"
                    : "¹H NMR: kengaygan, paramagnit (Ni²⁺)  •  μFₑff ≈ 2.86 μB")}
                </div>
              </div>
            </div>
          )}
              ...
              {showCrystalField && (
                <div className="bg-purple-950/95 backdrop-blur-md rounded-xl p-3 border border-purple-700/50 shadow-2xl animate-slide-in">
                  ...
                  <div className="bg-purple-900/50 rounded p-2 space-y-1 text-[11px]">
                    <div className="text-[10px] text-orange-300 italic">Sendvich (D₅d) uchun 3 sathli sxema:</div>
                    <div>Δ1 (e₂g → a₁g): <span className="text-white font-mono">
                      {`≈ ${complex.dOrbital.delta1.toLocaleString()} cm⁻¹`}
                    </span></div>
                    <div>Δ2 (a₁g → e₁g*): <span className="text-white font-mono">
                      {`≈ ${complex.dOrbital.delta2.toLocaleString()} cm⁻¹`}
                    </span></div>
                    <div>Ligand: <span className="text-white">C₅H₅⁻ (Cp⁻)</span></div>
                    <div>Ligand kuchi: <span className="text-white">Juda kuchli π-donor + akseptor</span></div>
                    <div>Spin: <span className="text-white">
                      {complex.dOrbital.type === "LS" ? "Past spin" : "Yuqori spin (paramagnit)"}
                    </span></div>
                    <div className="text-[9px] text-orange-300 italic mt-1">
                      Cp⁻ halqa 6 π-elektron (aromatik, 4n+2).<br/>
                      Har halqa metallga 5 orbital + 6e⁻ beradi.<br/>
                      Fe(II) d⁶ + 2 × 6 (Cp) = <b>18 e⁻</b> (barqaror).
                    </div>
                  </div>
                </div>
              )}
                ...
                <div className="bg-purple-950/95 backdrop-blur-md rounded-xl p-3 border border-orange-700/50 shadow-2xl animate-slide-in">
                  ...
                  <div className="bg-purple-900/50 rounded p-2 space-y-1 text-[11px]">
                    ...
                    <div className="text-[10px] text-purple-400">
                      {complex.center.element === "Fe" && (
                        oxidationState === 2 ? "Fe(II) d⁶ — ferrotsen (to'q-sariq, diamagnit)" :
                        oxidationState === 3 ? "Fe(III) d⁵ — ferritsenium [Fp]⁺ (ko'k, paramagnit)" :
                        "Kamdan-kam holat"
                      )}
                      {complex.center.element === "Ni" && (
                        oxidationState === 2 ? "Ni(II) d⁸ — nikelotsen (yashil, paramagnit, 20e⁻)" :
                        oxidationState === 3 ? "Ni(III) d⁷ — [Ni(Cp)₂]⁺ (19e⁻)" :
                        "Ni(IV) d⁶ — [Ni(Cp)₂]²⁺ (18e⁻, barqaror!)"
                      )}
                    </div>
                  </div>
                </div>
      ...
      <div className="bg-purple-950/90 backdrop-blur-md border-t border-purple-800/50 z-10">
        ...
        <div className="flex justify-center gap-3 sm:gap-6 py-3 px-3 sm:px-6 flex-wrap">
          <Stat label="Koord. son (η⁵)" value="10  (2 × 5)" />
          <Stat label={`${complex.center.element}–Cp(centroid)`} value={complex.ringCentroidReal} mono />
          <Stat label={`${complex.center.element}–C`} value={complex.bondLengthReal} mono />
          <Stat label="C–C (halqada)" value={complex.ccBondReal} mono />
          <Stat label="C–H" value={complex.chBondReal} mono />
          <Stat label="Simmetriya" value={complex.symmetry} mono />
          <Stat label="Konformatsiya" value={conformation === "staggered" ? "D₅d" : "D₅h"} mono />
          <Stat label="Valent e⁻" value={`${complex.valenceElectrons} e⁻`} mono />
