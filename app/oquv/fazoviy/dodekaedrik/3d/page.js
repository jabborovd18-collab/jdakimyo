...
// ═══════════════════════════════════════════════════════════════════════════
// CPK RANGLARI (IUPAC standartlari) — DODEKAEDRIK KOMPLEKSLAR uchun
// ═══════════════════════════════════════════════════════════════════════════
const CPK = {
  Mo: 0x54B5B5, W:  0x2194D6, V:  0xA6A6AB, Ni: 0x50D050,
  Co: 0xF090A0, Fe: 0xE06633, N:  0x3050F8, C:  0x909090,
  H:  0xFFFFFF, O:  0xFF0D0D, Cl: 0x1FF01F, K:  0x8F40D4,
  Na: 0xAB5CF2,
  bond: 0x8B9DC3, hbond: 0x66CCFF,
  bondA: 0xff9ec7,   // A-pozitsiya bog'lari (o'tkir uchli — pushti)
  bondB: 0x9ecfff    // B-pozitsiya bog'lari (yoyilgan — ochiq ko'k)
}

// ═══════════════════════════════════════════════════════════════════════════
// KOMPLEKS DATABASE — DODEKAEDRIK (D₂d) uchun tanlangan birikmalar
// ═══════════════════════════════════════════════════════════════════════════
// [Mo(CN)₈]⁴⁻  — Oktatsianomolibdat(IV), K₄[Mo(CN)₈]·2H₂O.
//                 Klassik dodekaedrik (D₂d) geometriya, d², diamagnit,
//                 sariq rang. Hoard-Nordsieck (1939) tomonidan rentgen bilan
//                 tasdiqlangan birinchi dodekaedr.
// [W(CN)₈]⁴⁻   — Oktatsianovolframat(IV), K₄[W(CN)₈]·2H₂O.
//                 Mo analogi. Eritmada D₂d, qattiq holatda kvadrat antiprizma
//                 ham bo'lishi mumkin. Sariq-yashil.
// ═══════════════════════════════════════════════════════════════════════════
const COMPLEXES = {
  MoCN8: {
    id: "MoCN8",
    ...
    center: { element: "Mo", color: CPK.Mo, radius: 0.50, charge: "+4" },
    ligand: { type: "CN", donor: "C", donorColor: CPK.C, donorRadius: 0.25 },
    // A pozitsiyalari qisqaroq, B pozitsiyalari uzunroq
    bondLength: 2.15,           // A ligand sahna masofasi
    bondLengthB: 2.20,          // B ligand sahna masofasi
    bondLengthReal: "2.15 Å",   // Mo-C(A) haqiqiy
    bondLengthBReal: "2.20 Å",  // Mo-C(B) haqiqiy
    outerIon: { element: "K", color: CPK.K, radius: 0.40, charge: "+1", count: 4 },
    hybridization: "d⁴sp³ (yoki sp³d⁴)",
    magnetism: "Diamagnit",
    color: "Och sariq kristall",
    // D₂d da d-orbital ajralishi: d(z²)=a₁ < d(x²-y²)=b₁ < d(xy)=b₂ < d(xz,yz)=e
    // (aslida a₁ eng past bo'ladi, chunki elektron-donor ligand a₁ ni destabilizatsiya qilmaydi)
    dOrbital: { a1: 2, b1: 0, b2: 0, e: 0, type: "LS", delta1: 15000, delta2: 8000, delta3: 6000 },
    geometry: "Dodekaedrik", symmetry: "D₂d",
    dElectrons: 2
  },
  WCN8: {
    id: "WCN8",
    ...
    center: { element: "W", color: CPK.W, radius: 0.52, charge: "+4" },
    ligand: { type: "CN", donor: "C", donorColor: CPK.C, donorRadius: 0.25 },
    bondLength: 2.16,
    bondLengthB: 2.22,
...
// ═══════════════════════════════════════════════════════════════════════════
const ATOM_INFO = {
  Mo: { name: "Molibden (Mo)", atomic: 42, mass: "95.95 u", config: "[Kr] 4d⁵ 5s¹", oxidation: "+4", role: "Markaziy ion (dodekaedrik)", color: "#54B5B5" },
  W:  { name: "Volfram (W)", atomic: 74, mass: "183.84 u", config: "[Xe] 4f¹⁴ 5d⁴ 6s²", oxidation: "+4", role: "Markaziy ion (dodekaedrik)", color: "#2194D6" },
  V:  { name: "Vanadiy (V)", atomic: 23, mass: "50.94 u", config: "[Ar] 3d³ 4s²", oxidation: "+4", role: "Markaziy ion", color: "#A6A6AB" },
  ...
  N:  { name: "Azot (N)", atomic: 7, mass: "14.01 u", config: "[He] 2s² 2p³", role: "CN⁻ lig. N-uchi", hybridization: "sp", color: "#3050F8" },
  C:  { name: "Uglerod (C)", atomic: 6, mass: "12.01 u", config: "[He] 2s² 2p²", role: "CN⁻ donor atomi", hybridization: "sp", color: "#909090" },
  H:  { name: "Vodorod (H)", atomic: 1, mass: "1.008 u", config: "1s¹", role: "H₂O tarkibi", color: "#FFFFFF" },
...
export default function Dodekaedrik3D() {
  ...
  const [currentComplex, setCurrentComplex] = useState("MoCN8")
  ...
  const [oxidationState, setOxidationState] = useState(4)  // Mo/W(IV) uchun
  ...
  // D₂d uchun simmetriya elementlari: E, 2S₄, C₂, 2C₂', 2σd
  const [symmetryElement, setSymmetryElement] = useState("S4") // S4 | C2 | sigma_d
  ...
  useEffect(() => {
    ...
    if (!["S4", "C2", "sigma_d"].includes(symmetryElement)) setSymmetryElement("S4")
  ...
  const buildSingleMolecule = useCallback((parent, complexData, centerPos = new THREE.Vector3(0, 0, 0), scale = 1) => {
    ...
    // ═══ DODEKAEDRIK D₂d — 8 ta ligand pozitsiyasi ═══
    // Hoard-Silverton koordinatalar tizimida ideal dodekaedr:
    //   A (o'tkir uchli, 4 ta):  θ_A ≈ 36.9°  — XZ va −XZ tekisliklarda,
    //                             y = ±z_A (chi qattiq bog'langan)
    //   B (yoyilgan, 4 ta):      θ_B ≈ 73.1°  — YZ va −YZ tekisliklarda,
    //                             y = ±z_B (uzunroq bog')
    // Ideal nisbatlar (bog' uzunligiga normalangan):
    //   sin(θ_A) ≈ 0.598,  cos(θ_A) ≈ 0.802  — A pozitsiya
    //   sin(θ_B) ≈ 0.960,  cos(θ_B) ≈ 0.283  — B pozitsiya
    const dA = complexData.bondLength                              // M–A masofa
    const dB = complexData.bondLengthB ?? complexData.bondLength   // M–B masofa
    // A atomlari uchun (sin/cos)
    const sA = 0.598, cA = 0.802
    // B atomlari uchun
    const sB = 0.960, cB = 0.283

    // A-atomlari XZ tekisligining ustki va pastki qismida (Y = ±cA*dA)
    // B-atomlari YZ tekisligining o'ng va chap qismida (Y = 0, X = ±sB*dB) — aslida
    // Hoard-Silverton da B lar boshqa koord. tizimida joylashadi, biz vizual
    // aniqlikni saqlash uchun quyidagi klassik dodekaedr atom pozitsiyalarini
    // ishlatamiz (S₄ o'qi Y bo'ylab):
    const ligandPositions = [
      // A atomlari: XZ tekislikda, Y = ±cA*dA
      [ sA * dA,  cA * dA,  0 ],   // 0 — A1 (+x, +y)
      [-sA * dA,  cA * dA,  0 ],   // 1 — A2 (−x, +y)
      [ sA * dA, -cA * dA,  0 ],   // 2 — A3 (+x, −y)
      [-sA * dA, -cA * dA,  0 ],   // 3 — A4 (−x, −y)
      // B atomlari: YZ tekislikda, X = 0
      [ 0,  cB * dB,  sB * dB ],   // 4 — B1 (+z, +y)
      [ 0,  cB * dB, -sB * dB ],   // 5 — B2 (−z, +y)
      [ 0, -cB * dB,  sB * dB ],   // 6 — B3 (+z, −y)
      [ 0, -cB * dB, -sB * dB ]    // 7 — B4 (−z, −y)
    ]
    ...
    ligandPositions.forEach(([x, y, z], idx) => {
      ...
      // DODEKAEDRIK D₂d: idx 0–3 = A (o'tkir), idx 4–7 = B (yoyilgan)
      const posType = idx < 4 ? "A" : "B"          // A yoki B pozitsiya
      const bondRadius = 0.09
      const bondColor  = posType === "A" ? CPK.bondA : CPK.bondB

      const bond = createBond(molGroup, coPos, donorPos, bondColor, bondRadius, 0.78)
      const activeLig = complexData.ligand
      const activeLen = posType === "A"
        ? complexData.bondLengthReal
        : (complexData.bondLengthBReal || complexData.bondLengthReal)
      bond.userData = {
        type: 'bond',
        bondType: `${center.element}–${activeLig.donor}(${posType})`,
        length: activeLen, ligandIdx: idx, posType
      }
      bondsRef.current.push(bond)

      const midpoint = new THREE.Vector3().addVectors(coPos, donorPos).multiplyScalar(0.5)
      const lengthLabel = makeTextSprite(`${activeLen} (${posType})`, {
        color: "#fef3c7",
        bgColor: posType === "A" ? "rgba(180, 60, 90, 0.92)" : "rgba(60, 100, 180, 0.92)",
    ...
    // Tashqi sfera — dodekaedrik komplekslar 4− zaryadli bo'lganligi uchun
    // odatda K⁴⁺ yoki Cs⁴⁺ bilan muvozanatlashadi (K₄[Mo(CN)₈], K₄[W(CN)₈])
  ...
  const buildEnsemble = useCallback((scene, complexData, count, mode) => {
    ...
    // ═══ DODEKAEDRIK QIRRALARI (edge chiziqlari) ═══
    // Dodekaedr geometriyasini vizual aniq ko'rsatish uchun 3 xil qirra to'plami:
    //   1) A–A qirralari (o'tkir uchli — pushti):    4 ta
    //   2) B–B qirralari (yoyilgan — ochiq ko'k):   4 ta
    //   3) A–B qirralari (aralash — binafsha):    8 ta
    if (count === 1 && moleculeGroupsRef.current[0]) {
      const mol = moleculeGroupsRef.current[0]
      const ligVecs = mol.userData.ligandVectors
      if (ligVecs && ligVecs.length === 8) {
        const A = [ligVecs[0], ligVecs[1], ligVecs[2], ligVecs[3]]
        const B = [ligVecs[4], ligVecs[5], ligVecs[6], ligVecs[7]]

        const makeEdge = (color, opacity) => new THREE.LineDashedMaterial({
          color, dashSize: 0.14, gapSize: 0.09,
          transparent: true, opacity
        })

        const matAA = makeEdge(0xff9ec7, 0.55)   // A–A pushti
        const matBB = makeEdge(0x9ecfff, 0.55)   // B–B ochiq ko'k
        const matAB = makeEdge(0xc084fc, 0.45)   // A–B binafsha
        ...
        // A–A: 0–1 va 2–3 (yuqori va pastki juftlik)
        drawLine(A[0], A[1], matAA)
        drawLine(A[2], A[3], matAA)
        // B–B: 4–5 va 6–7
        drawLine(B[0], B[1], matBB)
        drawLine(B[2], B[3], matBB)

        // A–B: har bir A o'ziga eng yaqin 2 ta B ga bog'lanadi (dodekaedr qirrasi)
        A.forEach(a => {
          const dists = B.map((b, k) => ({ k, d: a.distanceTo(b) }))
            .sort((p, q) => p.d - q.d)
          drawLine(a, B[dists[0].k], matAB)
          drawLine(a, B[dists[1].k], matAB)
        })
      }
    }
  }, [buildSingleMolecule])
  ...
  useEffect(() => {
    ...
    // ═══ D₂d (DODEKAEDRIK) uchun simmetriya elementlari ═══
    // E, 2S₄ (Y-o'q), C₂ (Y-o'q = S₄²), 2C₂' (X va Z o'qlari), 2σd (XY, ZY)
    if (symmetryElement === "S4") {
      // S₄ o'qi — Y (apeks o'qi), aylanish + gorizontal aks etish
      const dir = new THREE.Vector3(0, 1, 0)
      const points = [dir.clone().multiplyScalar(-len * 0.9), dir.clone().multiplyScalar(len * 1.1)]
      const geom = new THREE.BufferGeometry().setFromPoints(points)
      const mat = new THREE.LineBasicMaterial({ color: 0xff44ff, linewidth: 3, transparent: true, opacity: 0.85 })
      const line = new THREE.Line(geom, mat)
      line.userData = { type: 'symmetry' }
      scene.add(line)
      symmetryHelpersRef.current.push(line)

      const label = makeTextSprite("S₄ (Y-o'qi)", {
        color: "#ffffff",
        bgColor: "rgba(255, 68, 255, 0.85)",
      ...
      // Aylanma yo'nalish + aks tekisligi (gorizontal aylana Y=0)
      const ringGeo = new THREE.TorusGeometry(1.6, 0.03, 8, 48)
      const ringMat = new THREE.MeshBasicMaterial({ color: 0xff44ff, transparent: true, opacity: 0.55 })
      ...
      const noteLabel = makeTextSprite("Improper rotation (S₄) = C₄ × σh", {
        color: "#fce7ff", bgColor: "rgba(120, 30, 120, 0.85)",
        borderColor: "#ff88ff", scale: 0.35
      })
      noteLabel.position.set(len * 1.5, 0, 0)
      noteLabel.userData = { type: 'symmetry' }
      scene.add(noteLabel)
      symmetryHelpersRef.current.push(noteLabel)
    } else if (symmetryElement === "C2") {
      // 3 ta C₂ o'qi: 1 ta asosiy (Y = S₄²) + 2 ta perpendikulyar (X va Z)
      const axes = [
        { dir: [0, 1, 0], color: 0x44ff44, label: "C₂ (asosiy)", thick: 3 },
        { dir: [1, 0, 0], color: 0xff8844, label: "C₂' (X)",    thick: 2 },
        { dir: [0, 0, 1], color: 0x44aaff, label: "C₂' (Z)",    thick: 2 }
      ]
      axes.forEach(a => {
        ...
        const mat = new THREE.LineBasicMaterial({
          color: a.color, linewidth: a.thick,
        ...
        const lbl = makeTextSprite(a.label, {
          ...
          borderColor: "#ffffff", scale: 0.38
        })
        lbl.position.copy(dir.clone().multiplyScalar(len + 0.4))
        lbl.userData = { type: 'symmetry' }
        scene.add(lbl)
        symmetryHelpersRef.current.push(lbl)
      })
    } else if (symmetryElement === "sigma_d") {
      // 2σd tekisliklari — XY (Z = 0) va ZY (X = 0) tekisliklari
      // (bular A va B ligand juftlarini almashtiradi)
      const planes = [
        { rot: [0, 0, 0],          color: 0x00ffff, name: "σd (XY)" },  // XY tekislik (Z = 0)
        { rot: [0, Math.PI / 2, 0], color: 0xffff00, name: "σd (ZY)" }   // ZY tekislik (X = 0)
      ]
...
const generatePDF = async () => {
  ...
  try {
    ...
    const addFooter = () => {
      ...
      const leftText = truncate(
        `Dodekaedrik 3D Lab PRO  •  ${cleanText(complex.formula)}  •  ${new Date().toLocaleDateString("uz-UZ")}`,
    ...
    const abstract =
      ...
      `Bu natija Hoard–Nordsieck (1939), Hoard–Silverton (1963) va Muetterties (1974) tomonidan taklif etilgan 8-koordinatsion polytopal ideal shakllar nazariyasi asosida tahlil qilingan.`
    ...
    if (pdfSections.snapshot) {
      ...
      if (renderer && sceneRef.current && cameraRef.current) {
        ...
        const caption =
          ...
          `ko'rinishidagi 3D modeli. Dodekaedrik (${cleanText(complex.symmetry)}) simmetriya, 4 ta A (o'tkir uchli) + 4 ta B (yoyilgan) ligand pozitsiyasi. ${moleculeCount > 1 ? `${moleculeCount} ta molekula ${ensembleMode} ansamblida.` : "Bitta molekula."}`
    ...
    if (pdfSections.info) {
      ...
      const infoTable = [
        ...
        ["Koordinatsion son", "8  (4 ta A + 4 ta B)"],
    ...
    if (pdfSections.dorbital) {
      drawSectionHeader(sectionNum++, "Kristall Maydon d-Orbital Ajralishi (D₂d)")
      checkPageBreak(240)

      // Diagramma joylashuvi — D₂d 4 sath, tartib: a₁ (eng past) < b₁ < b₂ < e (eng yuqori)
      const diagX  = MARGIN + 30
      const eY_D   = y - 15    // e   — eng yuqori
      const b2Y_D  = y - 55    // b₂
      const b1Y_D  = y - 100   // b₁
      const a1Y_D  = y - 140   // a₁  — eng past

      // E o'qi
      page.drawLine({
        start: { x: diagX, y: y }, end: { x: diagX, y: a1Y_D - 25 },
      ...
      // ═══ 4 sath (past dan yuqoriga): a₁ < b₁ < b₂ < e ═══
      const orbLineW = 42
      const singleX  = diagX + 30
      // e (dxz, dyz) — eng yuqori, dubl
      const e1X = diagX + 20
      const e2X = diagX + 70
      page.drawLine({ start: { x: e1X, y: eY_D }, end: { x: e1X + orbLineW, y: eY_D }, thickness: 2, color: C.blue })
      page.drawLine({ start: { x: e2X, y: eY_D }, end: { x: e2X + orbLineW, y: eY_D }, thickness: 2, color: C.blue })
      page.drawText("e", { x: diagX + 128, y: eY_D - 3, size: 11, font: boldFont, color: C.blue })
      page.drawText("dxz", { x: e1X + 10, y: eY_D + 6, size: 7, font: regularFont, color: C.textMuted })
      page.drawText("dyz", { x: e2X + 10, y: eY_D + 6, size: 7, font: regularFont, color: C.textMuted })
      // b₂ (d_{xy})
      page.drawLine({ start: { x: singleX, y: b2Y_D }, end: { x: singleX + orbLineW, y: b2Y_D }, thickness: 2, color: C.purpleSoft })
      page.drawText("b₂", { x: singleX + orbLineW + 8, y: b2Y_D - 3, size: 11, font: boldFont, color: C.purpleSoft })
      page.drawText("dxy", { x: singleX + 10, y: b2Y_D + 6, size: 7, font: regularFont, color: C.textMuted })
      // b₁ (d_{x²-y²})
      page.drawLine({ start: { x: singleX, y: b1Y_D }, end: { x: singleX + orbLineW, y: b1Y_D }, thickness: 2, color: C.orange })
      page.drawText("b₁", { x: singleX + orbLineW + 8, y: b1Y_D - 3, size: 11, font: boldFont, color: C.orange })
      page.drawText("dx²-y²", { x: singleX + 4, y: b1Y_D + 6, size: 7, font: regularFont, color: C.textMuted })
      // a₁ (d_{z²}) — ENG PAST
      page.drawLine({ start: { x: singleX, y: a1Y_D }, end: { x: singleX + orbLineW, y: a1Y_D }, thickness: 2, color: C.red })
      page.drawText("a₁", { x: singleX + orbLineW + 8, y: a1Y_D - 3, size: 11, font: boldFont, color: C.red })
      page.drawText("dz²", { x: singleX + 10, y: a1Y_D + 6, size: 7, font: regularFont, color: C.textMuted })
      ...
      drawSlot(e1X, orbLineW, eY_D, Math.min(2, eVal))
      drawSlot(e2X, orbLineW, eY_D, Math.max(0, eVal - 2))
      drawSlot(singleX, orbLineW, b2Y_D, complex.dOrbital.b2 || 0)
      drawSlot(singleX, orbLineW, b1Y_D, complex.dOrbital.b1 || 0)
      drawSlot(singleX, orbLineW, a1Y_D, complex.dOrbital.a1 || 0)

      // ═══ Δ1 STRELKASI (a₁ → b₁) — eng katta ajralish ═══
      const arX = diagX + 200
      page.drawLine({ start: { x: arX, y: b1Y_D }, end: { x: arX, y: a1Y_D }, thickness: 1.5, color: C.orangeDeep })
      page.drawLine({ start: { x: arX - 3, y: b1Y_D - 5 }, end: { x: arX, y: b1Y_D }, thickness: 1.5, color: C.orangeDeep })
      page.drawLine({ start: { x: arX + 3, y: b1Y_D - 5 }, end: { x: arX, y: b1Y_D }, thickness: 1.5, color: C.orangeDeep })
      page.drawLine({ start: { x: arX - 3, y: a1Y_D + 5 }, end: { x: arX, y: a1Y_D }, thickness: 1.5, color: C.orangeDeep })
      page.drawLine({ start: { x: arX + 3, y: a1Y_D + 5 }, end: { x: arX, y: a1Y_D }, thickness: 1.5, color: C.orangeDeep })
      const mid1Y = (b1Y_D + a1Y_D) / 2
      ...
      // ═══ Δ2 STRELKASI (b₁ → b₂) ═══
      page.drawLine({ start: { x: arX, y: b1Y_D }, end: { x: arX, y: b2Y_D }, thickness: 1.5, color: C.green })
      page.drawLine({ start: { x: arX - 3, y: b2Y_D + 5 }, end: { x: arX, y: b2Y_D }, thickness: 1.5, color: C.green })
      page.drawLine({ start: { x: arX + 3, y: b2Y_D + 5 }, end: { x: arX, y: b2Y_D }, thickness: 1.5, color: C.green })
      const mid2Y = (b1Y_D + b2Y_D) / 2
      ...
      // ═══ Δ3 STRELKASI (b₂ → e) ═══
      page.drawLine({ start: { x: arX, y: b2Y_D }, end: { x: arX, y: eY_D }, thickness: 1.5, color: C.blue })
      page.drawLine({ start: { x: arX - 3, y: eY_D + 5 }, end: { x: arX, y: eY_D }, thickness: 1.5, color: C.blue })
      page.drawLine({ start: { x: arX + 3, y: eY_D + 5 }, end: { x: arX, y: eY_D }, thickness: 1.5, color: C.blue })
      const mid3Y = (b2Y_D + eY_D) / 2
      page.drawText("Δ3", { x: arX + 6, y: mid3Y + 4, size: 11, font: boldFont, color: C.blue })
      page.drawText(`≈ ${(complex.dOrbital.delta3||0).toLocaleString()} cm⁻¹`, {
        x: arX + 6, y: mid3Y - 8, size: 8, font: regularFont, color: C.blue,
      })
      ...
      safeText(`Konfiguratsiya:`, {
        x: infoX, y: eY_D, size: 8.5, font: boldFont, color: C.textDark, maxWidth: infoMaxW,
      })
      safeText(`a₁${a1Sub} b₁${b1Sub} b₂${b2Sub} e${eSub}`, {
        x: infoX, y: eY_D - 13, size: 9, font: regularFont, color: C.textDark, maxWidth: infoMaxW,
      })

      safeText(`Spin:`, {
        x: infoX, y: b2Y_D + 5, size: 8.5, font: boldFont, color: C.textDark, maxWidth: infoMaxW,
      })
      safeText(complex.dOrbital.type === "LS" ? "Past spin (LS)" :
               complex.dOrbital.type === "HS" ? "Yuqori spin (HS)" : "d² juftlashgan", {
        x: infoX, y: b2Y_D - 8, size: 9, font: regularFont, color: C.textDark, maxWidth: infoMaxW,
      })

      // D₂d uchun KMBE (soddalashtirilgan, a₁ eng past sath sifatida)
      // Barotsentr atrofida taxminiy koeffitsientlar:
      const cfse = (-0.60 * (complex.dOrbital.a1||0) + 0.10 * (complex.dOrbital.b1||0)
                    + 0.30 * (complex.dOrbital.b2||0) + 0.60 * (complex.dOrbital.e||0))
      safeText(`KMBE (Δ1 birligida):`, {
        x: infoX, y: b1Y_D + 5, size: 8.5, font: boldFont, color: C.textDark, maxWidth: infoMaxW,
      })
      safeText(`≈ ${cfse.toFixed(2)} Δ1`, {
        x: infoX, y: b1Y_D - 8, size: 9, font: regularFont, color: C.textDark, maxWidth: infoMaxW,
      })

      safeText(`d–elektronlar:`, {
        x: infoX, y: a1Y_D + 5, size: 8.5, font: boldFont, color: C.textDark, maxWidth: infoMaxW,
      })
      safeText(`d${complex.dElectrons}  —  ${complex.center.element}(${complex.center.charge})`, {
        x: infoX, y: a1Y_D - 8, size: 9, font: regularFont, color: C.textDark, maxWidth: infoMaxW,
      })

      // Umumiy y ni bir necha sath tashqarisiga tushiramiz
      y = a1Y_D - 45
      const caption = `2-rasm. ${cleanText(complex.formula)} uchun D₂d (dodekaedrik) kristall maydon ajralish diagrammasi. d-orbitallar to'rt sathga (a₁, b₁, b₂, e) ajraladi. a₁ (d_{z²}) eng past sathda joylashadi — 8 ligandning fazoviy taqsimoti d_{z²} orbital bilan minimal to'qnashuvga ega.`
      const capLines = wrapText(cleanText(caption), italicFont, 8.5, CONTENT_W)
      capLines.forEach((line, i) => {
        page.drawText(line, { x: MARGIN, y: y - i * 11, size: 8.5, font: italicFont, color: C.purpleSoft })
      })
    ...
    if (pdfSections.mo) {
      ...
      // Dodekaedrik (D₂d) uchun MO sxemasi — 8 ta ligand σ-guruh orbital:
      // 2a₁ + b₁ + b₂ + 2e (D₂d dekompozitsiya)
      const moLevels = [
        { label: "σ* (5p, 5s) — antibog'lovchi", fill: 0 },
        { label: "σ* (e, d_{xz,yz}) — antibog'",  fill: complex.dOrbital.e  || 0 },
        { label: "πnb (b₂, d_{xy}) — bog'lanmagan", fill: complex.dOrbital.b2 || 0 },
        { label: "σ* (b₁, d_{x²-y²}) — antibog'", fill: complex.dOrbital.b1 || 0 },
        { label: "HOMO (a₁, d_{z²}) — to'liq",     fill: complex.dOrbital.a1 || 0 },
        { label: "σ (2a₁ + b₁ + b₂ + 2e) — ligand", fill: 16 },
      ]
    ...
    if (pdfSections.spectra) {
      ...
      const specData = currentComplex === "MoCN8" ? [
        ["UV-Vis (d–d 1)", "λ₁ ≈ 367 nm  (¹A₁ → ¹E)"],
        ["UV-Vis (d–d 2)", "λ₂ ≈ 240 nm  (¹A₁ → ¹B₂)"],
        ["UV-Vis (LMCT)", "λmax < 220 nm"],
        ["ν(C≡N)", "2110–2135 cm⁻¹  (bir necha bo'lingan)"],
        ["ν(Mo–C) A pozit.", "≈ 490 cm⁻¹"],
        ["ν(Mo–C) B pozit.", "≈ 460 cm⁻¹"],
        ["δ(C–Mo–C)", "≈ 380 cm⁻¹"],
        ["NMR (⁹⁵Mo)", "≈ −1850 ppm (diamagnit)"],
        ["NMR (¹³C)", "CN(A): 138 ppm  •  CN(B): 141 ppm"],
      ] : [
        ["UV-Vis (d–d 1)", "λ₁ ≈ 358 nm  (¹A₁ → ¹E)"],
        ["UV-Vis (d–d 2)", "λ₂ ≈ 250 nm  (¹A₁ → ¹B₂)"],
        ["UV-Vis (LMCT)", "λmax < 220 nm"],
        ["ν(C≡N)", "2115–2140 cm⁻¹  (bir necha bo'lingan)"],
        ["ν(W–C) A pozit.", "≈ 495 cm⁻¹"],
        ["ν(W–C) B pozit.", "≈ 465 cm⁻¹"],
        ["NMR (¹⁸³W)", "≈ −2200 ppm (diamagnit)"],
        ["NMR (¹³C)", "CN(A): 145 ppm  •  CN(B): 148 ppm"],
      ]
      ...
      // IR cho'qqilari — dodekaedrik komplekslar uchun
      const irPeaks = currentComplex === "MoCN8"
        ? [
            { wn: 380, rel: 0.50, label: "δ(C–Mo–C)" },
            { wn: 460, rel: 0.75, label: "ν(Mo–C) B" },
            { wn: 490, rel: 0.90, label: "ν(Mo–C) A" },
            { wn: 620, rel: 0.35, label: "kombinat. mod" },
          ]
        : [
            { wn: 385, rel: 0.50, label: "δ(C–W–C)" },
            { wn: 465, rel: 0.80, label: "ν(W–C) B" },
            { wn: 495, rel: 0.95, label: "ν(W–C) A" },
            { wn: 630, rel: 0.35, label: "kombinat. mod" },
          ]
    ...
    if (pdfSections.crystalField) {
      ...
      // D₂d uchun KMBE hisobi: a₁ (-0.60), b₁ (+0.10), b₂ (+0.30), e (+0.60) — Δ1 birligida
      // Barotsentr atrofida taxminiy koeffitsientlar (Ballhausen "Ligand Field Theory")
      ...
      const cfse = -0.60 * a1El + 0.10 * b1El + 0.30 * b2El + 0.60 * eEl
      const d1  = complex.dOrbital.delta1 || 0
      const d2  = complex.dOrbital.delta2 || 0
      const d3  = complex.dOrbital.delta3 || 0
      const cfData = [
        ["Ligand maydon kuchi", showCrystalField ? ligandFieldStrength : "kuchli (CN⁻)"],
        ["Δ1 (a₁ → b₁)", `${d1.toLocaleString()} cm⁻¹  (≈ ${(d1 * 0.012).toFixed(1)} kJ/mol)`],
        ["Δ2 (b₁ → b₂)", `${d2.toLocaleString()} cm⁻¹  (≈ ${(d2 * 0.012).toFixed(1)} kJ/mol)`],
        ["Δ3 (b₂ → e)", `${d3.toLocaleString()} cm⁻¹  (≈ ${(d3 * 0.012).toFixed(1)} kJ/mol)`],
        ["KMBE (Δ1 birligida)", `${cfse.toFixed(2)} Δ1`],
        ["KMBE (energiya)", `${(cfse * d1 * 0.012).toFixed(1)} kJ/mol`],
        ["Juftlashuv energiyasi (P)", "≈ 20 000–25 000 cm⁻¹ (4d/5d)"],
        ["Konfiguratsiya", `a₁${a1Sub} b₁${b1Sub} b₂${b2Sub} e${eSub}`],
        ["Bashorat qilingan spin", complex.dOrbital.type === "LS" ? "Past spin (LS — 4d/5d + CN⁻ uchun tipik)" : "Yuqori spin"],
      ]
    ...
    if (pdfSections.references) {
      ...
      const refs = [
        "1. Hoard, J. L.; Nordsieck, H. H. (1939). The Structure of Potassium Molybdocyanide Dihydrate. J. Am. Chem. Soc., 61(10), 2853–2863. [Birinchi dodekaedrik struktura]",
        "2. Hoard, J. L.; Silverton, J. V. (1963). Stereochemistry of Discrete Eight-Coordination. I. Basic Analysis. Inorg. Chem., 2(2), 235–243. [Klassik dodekaedrik koordinatalar]",
        "3. Muetterties, E. L.; Guggenberger, L. J. (1974). Idealized polytopal forms. J. Am. Chem. Soc., 96(6), 1748–1756. [8-koord. geometriyalar taqqoslash]",
        "4. Corden, B. J.; Cunningham, J. A.; Eisenberg, R. (1970). Crystal and Molecular Structure of K₄[W(CN)₈]·2H₂O. Inorg. Chem., 9(2), 356–362. [W analogining tuzilishi]",
        "5. Werner, A. (1893). Beitrag zur Konstitution anorganischer Verbindungen. Z. Anorg. Chem., 3, 267–330.",
        ...
        "8. Miessler, G. L.; Fischer, P. J.; Tarr, D. A. (2014). Inorganic Chemistry, 5th ed. Pearson. [8-koord. — 8-bob]",
        "9. IUPAC. (2005). Nomenclature of Inorganic Chemistry: Recommendations 2005. RSC Publishing.",
        "10. Bethe, H. (1929). Termaufspaltung in Kristallen. Ann. Phys., 395(2), 133–208.",
      ]
      refs.forEach(ref => {
    ...
    pdfDoc.setKeywords([complex.geometry, complex.symmetry, "dodekaedrik", "D2d", "8-koordinatsion", "oktatsianometalat", "koordinatsion kimyo", "IUPAC"])
  ...
  url    = {https://jda-kimyo.uz/oquv/fazoviy/dodekaedrik},
  urldate = {${accessDate}}
}`
    ...
    <main className="min-h-screen bg-gradient-to-b from-purple-950 via-indigo-950 to-blue-950 text-white flex flex-col">
      ...
      <header className="flex items-center justify-between px-4 sm:px-6 py-3 border-b border-purple-800/50 z-30 bg-purple-950/80 backdrop-blur-md">
        <div className="flex items-center gap-3 sm:gap-4 min-w-0">
          ...
          <div className="min-w-0">
            <h1 className="text-base sm:text-xl font-bold text-purple-300 flex items-center gap-2 truncate">
              <span>🔷</span>
              <span className="hidden sm:inline">Dodekaedrik — 3D Laboratoriya PRO</span>
              <span className="sm:hidden">Dodekaedr 3D Lab PRO</span>
            </h1>
        ...
        <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
          ...
          >
            <option value="MoCN8">[Mo(CN)₈]⁴⁻</option>
            <option value="WCN8">[W(CN)₈]⁴⁻</option>
          </select>
        ...
        >
            ...
            <div className="space-y-2 mb-2 pl-1">
              ...
              {showLigandExchange && (
                <div className="ml-2 mt-1 space-y-1 bg-purple-900/30 p-2 rounded">
                  ...
                  >
                    <option value="H2O">A pozitsiya → H₂O (1 ta)</option>
                    <option value="Cl">B pozitsiya → Cl⁻ (1 ta)</option>
                  </select>
              ...
              {showRedox && (
                <div className="ml-2 mt-1 bg-purple-900/30 p-2 rounded">
                  ...
                  <div className="grid grid-cols-3 gap-1">
                    {(complex.center.element === "Mo" ? [3, 4, 5] :
                      complex.center.element === "W"  ? [4, 5, 6] :
                      [3, 4, 5]).map(ox => (
                      <button
                        key={ox}
          ...
          {expandedSection === "scientific" && (
            <div className="space-y-2 pl-1">
              ...
              <ToggleRow label="🔀 Izomerlar (D₂d)" value={showIsomers} onChange={setShowIsomers} />
              {showIsomers && (
                <div className="ml-2 mt-1 bg-purple-900/30 p-2 rounded">
                  <select
                    value={isomerType}
                  ...
                  >
                    ...
                    <option value="a_sub">A pozitsiya almashinuvi</option>
                    <option value="b_sub">B pozitsiya almashinuvi</option>
                    <option value="sap">Kvadrat antiprizmaga o'tish (D₄d)</option>
                    <option value="bcs">Ikki qopqoqli trigonal prizma (C₂ᵥ)</option>
                  </select>
                </div>
              )}
          ...
          <div className="absolute top-16 left-1/2 -translate-x-1/2 z-25 space-y-1 flex flex-col items-center">
            {showJahnTeller && (
              <div className="bg-pink-950/90 backdrop-blur-md px-3 py-1.5 rounded-lg text-xs text-pink-200 border border-pink-700/50">
                📐 Dodekaedr buzilishi: A va B bog'lari nomutanosib o'zgarishi
              </div>
            )}
            {showIsomers && isomerType !== "none" && (
              <div className="bg-indigo-950/90 backdrop-blur-md px-3 py-1.5 rounded-lg text-xs text-indigo-200 border border-indigo-700/50">
                🔀 {isomerType.toUpperCase()}: {
                  isomerType === "a_sub" ? "Ligand A (o'tkir) pozitsiyasida almashadi — M–A kalta bog'" :
                  isomerType === "b_sub" ? "Ligand B (yoyilgan) pozitsiyasida almashadi — M–B uzun bog'" :
                  isomerType === "sap"  ? "D₂d dodekaedr → D₄d kvadrat antiprizma o'zaro o'tishi" :
                  "Ikki qopqoqli trigonal prizma (BCTP, C₂ᵥ) alternativ 8-koord. shakl"
                }
              </div>
            )}
          </div>
          ...
          {!fullscreenMode && !selectedAtom && activePanel === "info" && (
            <div className="absolute top-3 right-3 bg-purple-950/95 backdrop-blur-md rounded-xl p-4 z-25 border border-purple-700/50 max-w-sm w-[300px] shadow-2xl animate-slide-in">
              ...
              <div className="space-y-2 text-xs">
                ...
                <InfoRow label="Bog' uzunligi (A)" value={complex.bondLengthReal} mono />
                {complex.bondLengthBReal && (
                  <InfoRow label="Bog' uzunligi (B)" value={complex.bondLengthBReal} mono />
                )}
                <InfoRow label="Magnit xossa" value={complex.magnetism} />
                <InfoRow label="Rangi" value={complex.color} small />
                <InfoRow label="Δ1 (a₁→b₁)" value={`${complex.dOrbital.delta1.toLocaleString()} cm⁻¹`} mono />
                <InfoRow label="Δ2 (b₁→b₂)" value={`${complex.dOrbital.delta2.toLocaleString()} cm⁻¹`} mono />
                <InfoRow label="Δ3 (b₂→e)" value={`${(complex.dOrbital.delta3||0).toLocaleString()} cm⁻¹`} mono />
                ...
                <InfoRow label="d-elektron soni" value={`d${complex.dElectrons}`} mono />
              </div>
            </div>
          )}

          {!fullscreenMode && !selectedAtom && activePanel === "dorbital" && (
            <div className="absolute top-3 right-3 bg-purple-950/95 backdrop-blur-md rounded-xl p-4 z-25 border border-purple-700/50 shadow-2xl w-[330px] animate-slide-in">
              <div className="flex justify-between items-start mb-3">
                <h3 className="text-sm font-bold text-purple-300 flex items-center gap-2">
                  <span>⚛️</span> d-orbital (D₂d ajralishi)
                </h3>
                <button onClick={() => setActivePanel(null)} className="text-purple-400 hover:text-white text-xl leading-none">×</button>
              </div>

              {/* D₂d dodekaedr uchun 4 sathli diagramma */}
              {/* Tartib (past dan yuqoriga): a₁ (d_{z²}) < b₁ (d_{x²-y²}) < b₂ (d_{xy}) < e (d_{xz,yz}) */}
              <div className="relative h-56 flex flex-col justify-between text-xs">
                {[
                  { key: 'e',  label: 'e (d_{xz,yz})',    slots: 4, val: complex.dOrbital.e,  color: 'text-cyan-300' },
                  { key: 'b2', label: 'b₂ (d_{xy})',      slots: 2, val: complex.dOrbital.b2, color: 'text-yellow-300' },
                  { key: 'b1', label: 'b₁ (d_{x²-y²})',   slots: 2, val: complex.dOrbital.b1, color: 'text-orange-300' },
                  { key: 'a1', label: 'a₁ (d_{z²})',     slots: 2, val: complex.dOrbital.a1, color: 'text-red-300' }
                ].map((row) => (
                ...
                <div className="text-center text-[10px] text-purple-500 mt-1">
                  ↕ Δ1 (a₁→b₁) ≈ {complex.dOrbital.delta1.toLocaleString()} cm⁻¹<br/>
                  ↕ Δ2 (b₁→b₂) ≈ {complex.dOrbital.delta2.toLocaleString()} cm⁻¹<br/>
                  ↕ Δ3 (b₂→e) ≈ {(complex.dOrbital.delta3||0).toLocaleString()} cm⁻¹
                </div>
              </div>
              ...
              <div className="mt-3 pt-2 border-t border-purple-800/50 text-[10px] text-purple-400 space-y-1">
                <div>Konfiguratsiya: <span className="text-white font-mono">
                  a₁^{complex.dOrbital.a1} b₁^{complex.dOrbital.b1} b₂^{complex.dOrbital.b2} e^{complex.dOrbital.e}
                </span></div>
                ...
                <div className="text-[9px] text-purple-500 italic mt-1">
                  D₂d: 8 ligand → d-orbitallar 4 sathga; a₁ eng past bo'lishi —
                  d_{`{z²}`} orbital ligand tekisliklariga proyeksiyalanmaydi.
                </div>
              </div>
            </div>
          )}
          ...
          {!fullscreenMode && !selectedAtom && activePanel === "mo" && (
            <div className="absolute top-3 right-3 bg-purple-950/95 backdrop-blur-md rounded-xl p-4 z-25 border border-pink-700/50 shadow-2xl w-[300px] animate-slide-in">
              ...
              <div className="space-y-1 text-[11px]">
                <div className="flex justify-between bg-purple-900/40 px-2 py-1 rounded"><span className="text-purple-400">σ* (e, 5p)</span><span className="text-white">— — (bo'sh)</span></div>
                <div className="flex justify-between bg-purple-900/40 px-2 py-1 rounded"><span className="text-purple-400">σ* (e, d_{`{xz,yz}`})</span><span className="text-white">— —</span></div>
                <div className="flex justify-between bg-yellow-900/30 px-2 py-1 rounded border border-yellow-700/30"><span className="text-yellow-400">Δ3</span><span className="text-yellow-300">{(complex.dOrbital.delta3||0).toLocaleString()} cm⁻¹</span></div>
                <div className="flex justify-between bg-purple-900/40 px-2 py-1 rounded"><span className="text-purple-400">πnb (b₂, d_{`{xy}`})</span><span className="text-white">—</span></div>
                ...
                <div className="flex justify-between bg-purple-900/40 px-2 py-1 rounded"><span className="text-purple-400">σ* (b₁, d_{`{x²-y²}`})</span><span className="text-white">—</span></div>
                ...
                <div className="flex justify-between bg-purple-900/40 px-2 py-1 rounded"><span className="text-purple-400">HOMO (a₁, d_{`{z²}`})</span><span className="text-white font-bold">↑↓</span></div>
                <div className="flex justify-between bg-purple-900/40 px-2 py-1 rounded"><span className="text-purple-400">σ (a₁, b₁, b₂, e)</span><span className="text-white">ligand σ-guruh</span></div>
                <div className="border-t border-pink-700/30 mt-2 pt-2 text-pink-200 text-center">{complex.formula} — D₂d</div>
              </div>
            </div>
          )}
            ...
            <div className="absolute top-3 right-3 bg-purple-950/95 backdrop-blur-md rounded-xl p-4 z-25 border border-cyan-700/50 shadow-2xl w-[320px] animate-slide-in">
              ...
              <div className="bg-purple-900/50 rounded p-3">
                ...
                <div className="text-[10px] text-purple-300 mt-2">
                  {spectrumType === "uv-vis" && (currentComplex === "MoCN8"
                    ? "d–d o'tish: λ₁ ≈ 367 nm, λ₂ ≈ 240 nm"
                    : "d–d o'tish: λ₁ ≈ 358 nm, λ₂ ≈ 250 nm")}
                  {spectrumType === "ir" && (currentComplex === "MoCN8"
                    ? "ν(C≡N): 2110–2135 cm⁻¹  •  ν(Mo–C): 460–500 cm⁻¹"
                    : "ν(C≡N): 2115–2140 cm⁻¹  •  ν(W–C): 465–495 cm⁻¹")}
                  {spectrumType === "nmr" && (currentComplex === "MoCN8"
                    ? "⁹⁵Mo: ≈ −1850 ppm  •  ¹³C(CN): ≈ 138 ppm"
                    : "¹⁸³W: ≈ −2200 ppm  •  ¹³C(CN): ≈ 145 ppm")}
                </div>
              </div>
            </div>
          )}
              ...
              {showCrystalField && (
                <div className="bg-purple-950/95 backdrop-blur-md rounded-xl p-3 border border-purple-700/50 shadow-2xl animate-slide-in">
                  ...
                  <div className="bg-purple-900/50 rounded p-2 space-y-1 text-[11px]">
                    ...
                    <div className="text-[10px] text-purple-400 italic">D₂d dodekaedr uchun taxminiy ajralish:</div>
                    <div>Δ1 (a₁→b₁): <span className="text-white font-mono">
                      {ligandFieldStrength === "weak" && "≈ 8,000 cm⁻¹"}
                      {ligandFieldStrength === "medium" && "≈ 15,000 cm⁻¹"}
                      {ligandFieldStrength === "strong" && "≈ 22,000 cm⁻¹"}
                    </span></div>
                    <div>Δ2 (b₁→b₂): <span className="text-white font-mono">
                      {ligandFieldStrength === "weak" && "≈ 4,000 cm⁻¹"}
                      {ligandFieldStrength === "medium" && "≈ 8,000 cm⁻¹"}
                      {ligandFieldStrength === "strong" && "≈ 12,000 cm⁻¹"}
                    </span></div>
                    <div>Δ3 (b₂→e): <span className="text-white font-mono">
                      {ligandFieldStrength === "weak" && "≈ 3,000 cm⁻¹"}
                      {ligandFieldStrength === "medium" && "≈ 6,000 cm⁻¹"}
                      {ligandFieldStrength === "strong" && "≈ 10,000 cm⁻¹"}
                    </span></div>
                    <div>Spin: <span className="text-white">
                      Past spin — 4d/5d metallar CN⁻ bilan doim LS
                    </span></div>
                    <div className="text-[9px] text-purple-400 italic">
                      D₂d: 8 ligand → 4 orbital sath;<br/>
                      a₁ (d_{`{z²}`}) juda past sabab — ligand tekisliklari
                      d_{`{z²}`} orbitalga to'g'ri kelmaydi.
                    </div>
                  </div>
                </div>
              )}
                ...
                <div className="bg-purple-950/95 backdrop-blur-md rounded-xl p-3 border border-orange-700/50 shadow-2xl animate-slide-in">
                  ...
                  <div className="bg-purple-900/50 rounded p-2 space-y-1 text-[11px]">
                    ...
                    <div className="text-[10px] text-purple-400">
                      {complex.center.element === "Mo" && (
                        oxidationState === 3 ? "Mo(III) — qora-jigar, boshqa geometriya" :
                        oxidationState === 4 ? "Mo(IV) — sariq, [Mo(CN)₈]⁴⁻ klassik" :
                        oxidationState === 5 ? "Mo(V) — qizil, [Mo(CN)₈]³⁻ oksidlangan" :
                        "Boshqa oksidlanish darajasi"
                      )}
                      {complex.center.element === "W" && (
                        oxidationState === 4 ? "W(IV) — sariq-yashil, klassik" :
                        oxidationState === 5 ? "W(V) — qizg'ish, [W(CN)₈]³⁻" :
                        oxidationState === 6 ? "W(VI) — rangsiz, WO₄²⁻ tipida" :
                        "Boshqa oksidlanish darajasi"
                      )}
                    </div>
                  </div>
      ...
      <div className="bg-purple-950/90 backdrop-blur-md border-t border-purple-800/50 z-10">
        ...
        <div className="flex justify-center gap-3 sm:gap-6 py-3 px-3 sm:px-6 flex-wrap">
          <Stat label="A–M–A burchak" value="~73.7°" />
          <Stat label="B–M–B burchak" value="~145.8°" />
          <Stat label="A–M–B burchak" value="~76–95°" />
          <Stat label="Koord. son" value="8" />
          <Stat label="Gibridlanish" value={complex.hybridization} mono />
          <Stat label="Simmetriya" value={complex.symmetry} mono />
          <Stat label={`${complex.center.element}–${complex.ligand.donor} (A)`} value={complex.bondLengthReal} mono />
          {complex.bondLengthBReal && (
            <Stat label={`${complex.center.element}–${complex.ligand.donor} (B)`} value={complex.bondLengthBReal} mono />
          )}
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {[
              ...
              { key: "references", icon: "📚", label: "Foydalanilgan adabiyotlar", desc: "10 ta ilmiy manba (Hoard, Silverton, Muetterties, IUPAC...)", highlight: true }
            ].map(item => (
              <label
                key={item.key}
