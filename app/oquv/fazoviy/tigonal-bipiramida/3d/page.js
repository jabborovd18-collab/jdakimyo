...
export default function TrigonalBipiramida3D() {
  ...
  useEffect(() => {
    ...
    const glow = new THREE.Mesh(glowGeo, new THREE.MeshBasicMaterial({ color: 0x808090, transparent: true, opacity: 0.1 })); scene.add(glow)

    const distEq = 1.8, distAx = 2.0
    // Ekvatorial — C (qora #1A1A1A) + O (qizil #FF0D0D)
    ...
    eqAngles.forEach(angle => {
      ...
      const cMesh = new THREE.Mesh(cGeo, cMat); cMesh.position.set(x * 0.7, 0, z * 0.7); scene.add(cMesh)
      // O
      ...
      const oMesh = new THREE.Mesh(oGeo, oMat); oMesh.position.set(x, 0, z); scene.add(oMesh)
      // C—O bog'
    ...
    // Aksial — C + O
    [[0, distAx, 0], [0, -distAx, 0]].forEach(([ax, ay, az]) => {
      const dir = ay > 0 ? 1 : -1
      const cGeo = new THREE.SphereGeometry(0.22, 32, 32)
      const cMat = new THREE.MeshStandardMaterial({ color: 0x1A1A1A, roughness: 0.4, metalness: 0.3 })
      const cMesh = new THREE.Mesh(cGeo, cMat); cMesh.position.set(ax, ay * 0.7, az); scene.add(cMesh)
      const oGeo = new THREE.SphereGeometry(0.2, 32, 32)
      const oMat = new THREE.MeshStandardMaterial({ color: 0xFF0D0D, roughness: 0.4, metalness: 0.2 })
      const oMesh = new THREE.Mesh(oGeo, oMat); oMesh.position.set(ax, ay, az); scene.add(oMesh)
      createBond(new THREE.Vector3(ax, ay * 0.7, az), new THREE.Vector3(ax, ay, az), 0x663333)
      createBond(new THREE.Vector3(0, 0, 0), new THREE.Vector3(ax, ay * 0.7, az), 0x886644)
    })
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-purple-950 to-blue-950 text-white flex flex-col">
      <header className="flex items-center gap-4 px-6 py-4 border-b border-purple-800/50 z-10">
        ...
        <div><h1 className="text-xl font-bold text-orange-400">🔷 Trigonal bipiramida — 3D</h1><p className="text-purple-400 text-sm">[Fe(CO)₅] • CPK ranglarda</p></div>
      </header>
