...
import { qobiqlar } from "../data/fe-co5-data"

export default function EXAFSSimulator() {
  const canvasRef = useRef(null)
  const [mode, setMode] = useState("chi")
  const [params, setParams] = useState({ r1: 1.807, s1: 0.0015, r2: 1.827, s2: 0.0016, r3: 2.950, s3: 0.0030 })
  const [activeShells, setActiveShells] = useState([true, true, true])
  const [showControls, setShowControls] = useState(false)

  const S02 = 0.82
  const N = [2, 3, 5]
  const colors = ["#F97316", "#4F8EF7", "#2DC78F"]
  const shellNames = ["Fe−C(aks)", "Fe−C(ekv)", "Fe−O"]

  const fkeff = (k, shell) => {
    const f0 = [0.52, 0.52, 0.30][shell]
    const decay = [0.05, 0.05, 0.08][shell]
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-white font-semibold">📊 EXAFS χ(k) va FT simulyatori — [Fe(CO)₅]</h3>
      ...
      {showControls && (
        <div className="bg-purple-800/20 rounded-lg p-4 border border-purple-700/30 space-y-3">
          {[
            { key: "r1", label: "Fe−C(aks) R₁", min: 1.70, max: 1.95, step: 0.001, unit: "Å", value: params.r1 },
            { key: "r2", label: "Fe−C(ekv) R₂", min: 1.70, max: 1.95, step: 0.001, unit: "Å", value: params.r2 },
          ].map((p) => (
      ...
      <p className="text-purple-500 text-xs">[Fe(CO)₅] — Fe⁰ (d⁸), trigonal bipiramida. Fe−C(aks)=1.807, Fe−C(ekv)=1.827 Å. Berry psevdorotatsiyasi.</p>
    </div>
  )
}