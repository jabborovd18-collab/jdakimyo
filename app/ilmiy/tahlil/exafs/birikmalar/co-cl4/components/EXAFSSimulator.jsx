...
import { qobiqlar } from "../data/co-cl4-data"

export default function EXAFSSimulator() {
  const canvasRef = useRef(null)
  const [mode, setMode] = useState("chi")
  const [params, setParams] = useState({ r1: 2.252, s1: 0.0025, r2: 5.000, s2: 0.0060 })
  ...
  const shellNames = ["Co−Cl (1-qobiq)", "Co−K/NR₄ (2-qobiq)"]

  const fkeff = (k, shell) => {
    const f0 = [0.55, 0.20][shell]
    const decay = [0.07, 0.10][shell]
  ...
  const chiShell = (k, R, sig2, shell) => {
    ...
    return S02 * N[shell] * f / (keff * R * R) *
      Math.exp(-2 * sig2 * keff * keff) * Math.exp(-2 * R / lam) * Math.sin(phase)
  }
  ...
  const draw = () => {
    ...
    const kmin = 2.5, kmax = 13, pts = 300
    ...
    for (let i = 0; i <= pts; i++) {
      ...
      shData[0].push(c1); shData[1].push(c2)
      tots.push((activeShells[0]?c1:0) + (activeShells[1]?c2:0))
    }
    ...
    if (mode === "ft") {
      ...
      for (let si = 0; si < 2; si++) {
        ...
        ft(shData[si]).forEach((v, i) => {
          const x = pad.l + i / Rpts * gw; const y = pad.t + gh - v / ymax * gh
      ...
      const allVals = [...tots]
      const ymin = Math.min(...allVals) * 1.2; const ymax = Math.max(...allVals) * 1.2
      const yr = ymax - ymin || 1

      for (let gx = 3; gx <= 13; gx += 2) {
      ...
      for (let si = 0; si < 2; si++) {
        ...
        shData[si].forEach((v, i) => {
          const x = pad.l + i / pts * gw; const y = pad.t + gh - (v - ymin) / yr * gh
          i === 0 ? ctx.moveTo(x, y) : ctx.lineTo(x, y)
        })
        ctx.stroke()
      }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-white font-semibold">📊 EXAFS χ(k) va FT simulyatori — [CoCl₄]²⁻</h3>
      ...
      {showControls && (
        <div className="bg-purple-800/20 rounded-lg p-4 border border-purple-700/30 space-y-3">
          {[
            { key: "r1", label: "Co−Cl masofasi R₁", min: 2.10, max: 2.40, step: 0.001, unit: "Å", value: params.r1 },
            { key: "s1", label: "σ²₁ (Co−Cl)", min: 0.0005, max: 0.012, step: 0.0001, unit: "Å²", value: params.s1 },
          ].map((p) => (
      ...
      <p className="text-purple-500 text-xs">[CoCl₄]²⁻ — Co²⁺ (d⁷, HS), tetraedrik. Co−Cl = 2.252 Å. Ko'k rang. Pre-edge kuchli (T_d).</p>
    </div>
  )
}