...
import { qobiqlar } from "../data/ni-cn4-data"
...
export default function EXAFSSimulator() {
  ...
  const S02 = 0.82
  const N = [4, 4, 4]
  const colors = ["#4F8EF7", "#2DC78F", "#EC4899"]
  const shellNames = ["Ni−C (1-qobiq)", "Ni−N (2-qobiq)", "Ni−K (3-qobiq)"]

  const fkeff = (k, shell) => {
    const f0 = [0.55, 0.35, 0.20][shell]
    const decay = [0.05, 0.07, 0.10][shell]
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-white font-semibold">📊 EXAFS χ(k) va FT simulyatori — [Ni(CN)₄]²⁻</h3>
      ...
      {showControls && (
        <div className="bg-purple-800/20 rounded-lg p-4 border border-purple-700/30 space-y-3">
          {[
            { key: "r1", label: "Ni−C masofasi R₁", min: 1.75, max: 1.95, step: 0.001, unit: "Å", value: params.r1 },
            { key: "s1", label: "σ²₁ (Ni−C)", min: 0.0005, max: 0.008, step: 0.0001, unit: "Å²", value: params.s1 },
            { key: "r2", label: "Ni−N masofasi R₂", min: 2.85, max: 3.20, step: 0.001, unit: "Å", value: params.r2 },
            { key: "s2", label: "σ²₂ (Ni−N)", min: 0.0005, max: 0.010, step: 0.0001, unit: "Å²", value: params.s2 },
          ].map((p) => (
      ...
      <p className="text-purple-500 text-xs">[Ni(CN)₄]²⁻ — Ni²⁺ (d⁸), kvadrat tekislik. Ni−C = 1.858 Å, 4 ta CN⁻. Sisplatin bilan izoelektronik.</p>
    </div>
  )
}