...
import { qobiqlar } from "../data/ferrosen-data"
...
export default function EXAFSSimulator() {
  ...
  const S02 = 0.80
  const N = [10, 10, 2]
  const colors = ["#F97316", "#4F8EF7", "#EC4899"]
  const shellNames = ["Fe−C (1-qobiq)", "Fe−H (2-qobiq)", "Fe−Fe (3-qobiq)"]

  const fkeff = (k, shell) => {
    const f0 = [0.50, 0.12, 0.25][shell]
    const decay = [0.05, 0.12, 0.09][shell]
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-white font-semibold">📊 EXAFS χ(k) va FT simulyatori — Ferrosen</h3>
      ...
      <div className="flex gap-2">
        <button onClick={() => setMode("chi")} className={`px-4 py-2 rounded-lg text-xs font-semibold ${mode === "chi" ? "bg-orange-600/80 text-white" : "bg-purple-800/40 text-purple-300"}`}>χ(k)·k²</button>
        <button onClick={() => setMode("ft")} className={`px-4 py-2 rounded-lg text-xs font-semibold ${mode === "ft" ? "bg-orange-600/80 text-white" : "bg-purple-800/40 text-purple-300"}`}>|FT(χ)|</button>
      </div>
      ...
      {showControls && (
        <div className="bg-purple-800/20 rounded-lg p-4 border border-purple-700/30 space-y-3">
          {[
            { key: "r1", label: "Fe−C masofasi R₁", min: 1.95, max: 2.20, step: 0.001, unit: "Å", value: params.r1 },
            { key: "s1", label: "σ²₁ (Fe−C)", min: 0.0005, max: 0.010, step: 0.0001, unit: "Å²", value: params.s1 },
            { key: "r2", label: "Fe−H masofasi R₂", min: 2.60, max: 3.10, step: 0.001, unit: "Å", value: params.r2 },
            { key: "s2", label: "σ²₂ (Fe−H)", min: 0.001, max: 0.015, step: 0.0001, unit: "Å²", value: params.s2 },
          ].map((p) => (
      ...
      <p className="text-purple-500 text-xs">Ferrosen — Fe²⁺ (d⁶, LS), 18 e⁻. Fe−C = 2.064 Å, 10 ta C. Birinchi sendvich kompleks — Nobel 1973.</p>
    </div>
  )
}