...
import { qobiqlar } from "../data/ag-nh3-2-data"
...
export default function EXAFSSimulator() {
  ...
  const S02 = 0.88
  const N = [2, 6, 4]
  const colors = ["#C0C0C0", "#4F8EF7", "#EC4899"]
  const shellNames = ["Ag−N (1-qobiq)", "Ag−H (2-qobiq)", "Ag−O/N (3-qobiq)"]

  const fkeff = (k, shell) => {
    const f0 = [0.50, 0.12, 0.22][shell]
    const decay = [0.06, 0.12, 0.09][shell]
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-white font-semibold">📊 EXAFS χ(k) va FT simulyatori — [Ag(NH₃)₂]⁺</h3>
      ...
      <div className="flex gap-2">
        <button onClick={() => setMode("chi")} className={`px-4 py-2 rounded-lg text-xs font-semibold ${mode === "chi" ? "bg-gray-400/80 text-white" : "bg-purple-800/40 text-purple-300"}`}>χ(k)·k²</button>
        <button onClick={() => setMode("ft")} className={`px-4 py-2 rounded-lg text-xs font-semibold ${mode === "ft" ? "bg-gray-400/80 text-white" : "bg-purple-800/40 text-purple-300"}`}>|FT(χ)|</button>
      </div>
      ...
      {showControls && (
        <div className="bg-purple-800/20 rounded-lg p-4 border border-purple-700/30 space-y-3">
          {[
            { key: "r1", label: "Ag−N masofasi R₁", min: 2.00, max: 2.30, step: 0.001, unit: "Å", value: params.r1 },
            { key: "s1", label: "σ²₁ (Ag−N)", min: 0.0005, max: 0.010, step: 0.0001, unit: "Å²", value: params.s1 },
            { key: "r2", label: "Ag−H masofasi R₂", min: 2.60, max: 3.10, step: 0.001, unit: "Å", value: params.r2 },
            { key: "s2", label: "σ²₂ (Ag−H)", min: 0.001, max: 0.015, step: 0.0001, unit: "Å²", value: params.s2 },
          ].map((p) => (
      ...
      <p className="text-purple-500 text-xs">[Ag(NH₃)₂]⁺ — Ag⁺ (d¹⁰), chiziqli. Ag−N = 2.115 Å, 2 ta NH₃. Tollens reaktivi.</p>
    </div>
  )
}