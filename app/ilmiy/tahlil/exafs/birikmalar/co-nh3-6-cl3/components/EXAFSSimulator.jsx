...
import { qobiqlar } from "../data/co-nh3-6-cl3-data"
...
export default function EXAFSSimulator() {
  ...
  const S02 = 0.80
  const N = [6, 18, 8]
  const colors = ["#4F8EF7", "#F0A030", "#EC4899"]
  const shellNames = ["Co−N (1-qobiq)", "Co−H (2-qobiq)", "Co−Cl (3-qobiq)"]

  const fkeff = (k, shell) => {
    const f0 = [0.52, 0.18, 0.25][shell]
    const decay = [0.05, 0.10, 0.08][shell]
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-white font-semibold">📊 EXAFS χ(k) va FT simulyatori — [Co(NH₃)₆]Cl₃</h3>
      ...
      {showControls && (
        <div className="bg-purple-800/20 rounded-lg p-4 border border-purple-700/30 space-y-3">
          {[
            { key: "r1", label: "Co−N masofasi R₁", min: 1.85, max: 2.05, step: 0.001, unit: "Å", value: params.r1 },
            { key: "s1", label: "σ²₁ (Co−N)", min: 0.0005, max: 0.008, step: 0.0001, unit: "Å²", value: params.s1 },
            { key: "r2", label: "Co−H masofasi R₂", min: 2.40, max: 2.90, step: 0.001, unit: "Å", value: params.r2 },
            { key: "s2", label: "σ²₂ (Co−H)", min: 0.001, max: 0.015, step: 0.0001, unit: "Å²", value: params.s2 },
          ].map((p) => (
      ...
      <p className="text-purple-500 text-xs">[Co(NH₃)₆]Cl₃ — Co³⁺ (LS, d⁶). Co−N = 1.961 Å, 6 ta NH₃. Verner klassikasi — Nobel 1913.</p>
    </div>
  )
}