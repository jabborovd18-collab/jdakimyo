...
const CRYSTAL = {
  ...
  independentReflections: 2105,
  completeness: "99.5%",
  rInt: 0.025,
  r1Obs: 0.032, r1All: 0.041,
  wR2Obs: 0.078, wR2All: 0.085,
  gof: 1.05,
  
  bondLengths: [
    { bond: "Fe−C (o'rtacha)", length: "1.918(3)", note: "6 ta ekvivalent bog'" },
    { bond: "C≡N (o'rtacha)", length: "1.152(4)", note: "6 ta ekvivalent bog'" },
    { bond: "K1···N", length: "2.78−3.25", note: "koordinatsion muhit" },
    { bond: "K2···N", length: "2.82−3.18", note: "koordinatsion muhit" },
  ],
  bondAngles: [
    { angle: "C−Fe−C (sis)", value: "89.6(2)°" },
    { angle: "C−Fe−C (trans)", value: "179.5(2)°" },
    { angle: "Fe−C≡N (o'rtacha)", value: "179.1(3)°" },
  ],
  
  phases: [
    { temp: "293 K", spaceGroup: "C2/c", a: 9.42, b: 12.88, c: 14.15, beta: 95.2, volume: 1709.5, Z: 4 },
    { temp: "130 K", spaceGroup: "C2/c", a: 9.35, b: 12.80, c: 14.05, beta: 95.0, volume: 1674.0, Z: 4 },
  ],
  phaseTransition: "K₄[Fe(CN)₆] xona haroratidan past haroratgacha C2/c fazoviy guruhini saqlaydi. ~130 K da panjara parametrlari biroz qisqaradi, lekin strukturaviy o'tish kuzatilmaydi. Hajm ~2% ga kamayadi.",
  
  safety: "K₄[Fe(CN)₆] nisbatan xavfsiz — E535 ozuqa qo'shimchasi sifatida ro'yxatga olingan. Kislotalar bilan reaksiyada HCN ajralishi mumkin. Kuchli kislotalardan uzoq tuting.",
  
  diffractionPeaks: [
    { h: 0, k: 2, l: 0, twotheta: 8.25, d: 5.36, intensity: 35 },
    { h: 1, k: 1, l: 0, twotheta: 10.42, d: 4.24, intensity: 100 },
    { h: 0, k: 0, l: 2, twotheta: 12.58, d: 3.52, intensity: 45 },
    { h: 1, k: 3, l: 0, twotheta: 14.87, d: 2.98, intensity: 28 },
    { h: 0, k: 4, l: 0, twotheta: 16.54, d: 2.68, intensity: 55 },
    { h: 1, k: 1, l: 2, twotheta: 18.25, d: 2.43, intensity: 40 },
    { h: 2, k: 0, l: 0, twotheta: 20.31, d: 2.18, intensity: 32 },
    { h: 1, k: 5, l: 0, twotheta: 22.68, d: 1.96, intensity: 18 },
    { h: 0, k: 6, l: 0, twotheta: 24.95, d: 1.78, intensity: 25 },
    { h: 2, k: 2, l: 2, twotheta: 27.12, d: 1.64, intensity: 15 },
    { h: 1, k: 5, l: 2, twotheta: 29.45, d: 1.52, intensity: 20 },
    { h: 3, k: 1, l: 0, twotheta: 31.88, d: 1.40, intensity: 12 },
    { h: 2, k: 6, l: 0, twotheta: 34.52, d: 1.30, intensity: 10 },
    { h: 0, k: 8, l: 0, twotheta: 37.25, d: 1.21, intensity: 8 },
    { h: 3, k: 5, l: 0, twotheta: 39.88, d: 1.13, intensity: 6 },
  ],
}
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2 mb-4">
        <button onClick={() => setMode("bragg")} className={`px-4 py-2 rounded-xl text-sm font-semibold ${mode === "bragg" ? "bg-amber-600/40 text-white border border-amber-400/50" : "bg-purple-800/30 text-purple-400"}`}>Bragg qonuni</button>
        <button onClick={() => setMode("scherrer")} className={`px-4 py-2 rounded-xl text-sm font-semibold ${mode === "scherrer" ? "bg-amber-600/40 text-white border border-amber-400/50" : "bg-purple-800/30 text-purple-400"}`}>Scherrer tenglamasi</button>
      </div>
      ...
      {result && !result.error && (
        <div className="bg-amber-600/10 border border-amber-500/30 rounded-xl p-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-center">
            <div><p className="text-amber-400 font-bold text-lg">{result.dVal.toFixed(3)}</p><p className="text-purple-400 text-xs">d (Å)</p></div>
            <div><p className="text-amber-400 font-bold text-lg">{result.theta.toFixed(2)}°</p><p className="text-purple-400 text-xs">θ</p></div>
            <div><p className="text-amber-400 font-bold text-lg">{result.twotheta.toFixed(2)}°</p><p className="text-purple-400 text-xs">2θ</p></div>
            {mode === "scherrer" && <div><p className="text-amber-400 font-bold text-lg">{result.D.toFixed(1)}</p><p className="text-purple-400 text-xs">D (Å)</p></div>}
          </div>
        </div>
      )}
      ...
      <div className="relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end gap-4 mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            <span className="bg-amber-600/20 text-amber-400 border border-amber-600/30 rounded-full px-3 py-1 text-xs font-semibold">{CRYSTAL.tarixiy}</span>
        ...
        <div className="bg-green-600/10 border border-green-500/30 rounded-xl p-4">
          <p className="text-green-400 text-sm"><strong>⚠ Xavfsizlik:</strong> {CRYSTAL.safety}</p>
        </div>
      </div>
    </div>
  )
...
export default function K4FeCN6RentgenPage() {
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-wrap gap-1 bg-purple-900/40 border border-purple-700/50 rounded-2xl p-1.5 mb-6">
          {TABS.map(tab => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all ${activeTab === tab.id ? "bg-amber-600/40 text-white border border-amber-400/50" : "text-purple-400 hover:text-white hover:bg-purple-800/30"}`}>
        ...
        <div className="bg-purple-900/40 border border-purple-700/50 rounded-2xl p-6 md:p-8">
          {activeTab === "diffraction" && (
            <div className="space-y-4">
              <h2 className="text-xl font-bold text-amber-400">Hisoblangan difraktogramma</h2>
              <p className="text-purple-400 text-sm">K₄[Fe(CN)₆] ning room-temperature monoklinik fazasi (C2/c) uchun strukturaviy modeldan hisoblangan PXRD pattern. Piklar 2θ bo'yicha indekslangan, intensivliklar nisbiy. <strong className="text-amber-400">Bu xom eksperimental difraktogramma emas.</strong></p>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>{CRYSTAL.diffractionPeaks.map((p, i) => (
                      <tr key={i} className="border-b border-purple-800/30 hover:bg-purple-800/20">
                        <td className="py-2 px-3 font-mono text-amber-400">({p.h}{p.k}{p.l})</td>
          ...
          {activeTab === "calculator" && (
            <div className="space-y-4">
              <h2 className="text-xl font-bold text-amber-400">Bragg + Scherrer kalkulyatori</h2>
              <p className="text-purple-400 text-sm">Monoklinik C2/c birlik hujayra uchun d(hkl) ni hisoblang. Miller indekslarini kiriting — K₄[Fe(CN)₆] ning haqiqiy panjara parametrlari asosida avtomatik hisoblanadi.</p>
          ...
          {activeTab === "structure" && (
            <div className="space-y-4">
              <h2 className="text-xl font-bold text-amber-400">Kristall struktura ma'lumotlari</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-purple-800/30 rounded-xl p-4">
                  ...
                  <table className="w-full text-sm"><thead><tr className="border-b border-purple-700/50"><th className="text-left py-2 px-3 text-purple-400">Bog'</th><th className="text-left py-2 px-3 text-purple-400">Uzunlik (Å)</th></tr></thead>
                    <tbody>{CRYSTAL.bondLengths.map((r, i) => (<tr key={i} className="border-b border-purple-800/30"><td className="py-2 px-3 text-amber-400 font-mono text-xs">{r.bond}</td><td className="py-2 px-3 text-purple-200">{r.length}</td></tr>))}</tbody></table>
                </div>
                <div className="bg-purple-800/30 rounded-xl p-4">
                  ...
                  <table className="w-full text-sm"><thead><tr className="border-b border-purple-700/50"><th className="text-left py-2 px-3 text-purple-400">Burchak</th><th className="text-left py-2 px-3 text-purple-400">Qiymat</th></tr></thead>
                    <tbody>{CRYSTAL.bondAngles.map((r, i) => (<tr key={i} className="border-b border-purple-800/30"><td className="py-2 px-3 text-amber-400 font-mono text-xs">{r.angle}</td><td className="py-2 px-3 text-purple-200">{r.value}</td></tr>))}</tbody></table>
                </div>
              </div>
              <div className="bg-blue-600/10 border border-blue-500/30 rounded-xl p-4">
                <h3 className="text-blue-400 font-semibold text-sm mb-2">🔬 Fe−C≡N lineerlik — K₄ vs K₃ taqqoslash</h3>
                <p className="text-purple-200 text-sm">K₄[Fe(CN)₆] da Fe−C≡N burchagi <strong className="text-amber-400">179.1°</strong> — K₃[Fe(CN)₆] dagi 178.1° ga nisbatan ideal 180° ga yaqinroq. Sababi: Fe(II) (d⁶) Fe(III) (d⁵) ga qaraganda kuchliroq π-back-donation beradi, bu CN ligandlarini yadro atrofida simmetrikroq joylashtiradi. K₄[Fe(CN)₆] diamagnit (S=0), K₃[Fe(CN)₆] paramagnit (S=1/2).</p>
              </div>
            </div>
          )}
          ...
          {activeTab === "refinement" && (
            <div className="space-y-4">
              <h2 className="text-xl font-bold text-amber-400">Ishonchlilik parametrlari</h2>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {[
                      ...
                      <tr key={i} className="border-b border-purple-800/30"><td className="py-3 px-4 text-amber-400 font-mono">{r[0]}</td><td className="py-3 px-4 text-purple-200">{r[1]}</td><td className="py-3 px-4 text-purple-400">{r[2]}</td></tr>
                    ))}
                  </tbody>
                </table>
              </div>
          ...
          {activeTab === "phases" && (
            <div className="space-y-4">
              <h2 className="text-xl font-bold text-amber-400">Fazaviy o'tishlar</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {CRYSTAL.phases.map((phase, i) => (
                  <div key={i} className={`rounded-xl p-5 border ${i === 0 ? "bg-amber-600/10 border-amber-500/30" : "bg-blue-600/10 border-blue-500/30"}`}>
                    <h3 className="font-semibold mb-3" style={{ color: i === 0 ? "#f59e0b" : "#60a5fa" }}>{phase.temp} — {phase.spaceGroup}</h3>
        ...
        <div className="flex justify-between mt-8 pt-6 border-t border-purple-800/50">
          <Link href="/ilmiy/tahlil/rentgen/birikmalar/k3-fe-cn6" className="text-purple-400 hover:text-purple-200 text-sm flex items-center gap-2"><span>←</span> K₃[Fe(CN)₆]</Link>
          <Link href="/ilmiy/tahlil/rentgen/birikmalar/co-nh3-6-cl3" className="text-purple-400 hover:text-purple-200 text-sm">[Co(NH₃)₆]Cl₃ →</Link>
        </div>
      </div>
    </div>
  )
