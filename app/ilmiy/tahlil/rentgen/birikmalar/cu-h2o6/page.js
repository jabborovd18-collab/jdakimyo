...
const CRYSTAL = {
  ...
  tarixiy: "",
  molarMass: "171.66 g·mol⁻¹",
  oxidationState: "Cu(II), d⁹ (S = 1/2)",
  coordinationGeom: "Cho'zilgan oktaedr (D₄h)",
  
  crystalSystem: "Monoklinik",
  spaceGroup: "P2₁/c (№14)",
  a: 6.12, b: 10.69, c: 13.88,
  alpha: 90, beta: 95.7, gamma: 90,
  volume: 903.5, Z: 4,
  densityCalc: 2.05,
  ...
  reflectionsCollected: 4100,
  independentReflections: 1950,
  completeness: "99.0%",
  rInt: 0.029,
  r1Obs: 0.033, r1All: 0.045,
  wR2Obs: 0.079, wR2All: 0.088,
  gof: 1.06,
  
  bondLengths: [
    { bond: "Cu−O(ekv) — 4 ta", length: "1.968(4)", note: "Ekvatorial, qisqa bog'lar" },
    { bond: "Cu−O(aks) — 2 ta", length: "2.275(5)", note: "Aksial, uzun bog'lar" },
    { bond: "Δ(Cu−O)", length: "0.307", note: "Yahn-Teller cho'zilishi" },
    { bond: "O−H···O / O−H···anion", length: "2.68−2.95", note: "Vodorod bog'lari" },
  ],
  bondAngles: [
    { angle: "O(ekv)−Cu−O(ekv) sis", value: "89.8(2)°" },
    { angle: "O(ekv)−Cu−O(ekv) trans", value: "179.2(2)°" },
    { angle: "O(aks)−Cu−O(aks)", value: "178.5(3)°" },
  ],
  
  jahnTeller: {
    title: "Yahn-Teller cho'zilishi",
    explanation: "Cu²⁺ (d⁹) da eg orbitallarda 3 ta elektron — degeneratsiya bartaraf qilinishi kerak. Natijada oktaedr z o'qi bo'yicha cho'ziladi: 4 ta ekvatorial Cu−O bog'i qisqa (1.968 Å), 2 ta aksial Cu−O bog'i uzun (2.275 Å). Farq Δ = 0.307 Å.",
    equatorial: "1.968 Å — qisqa, mustahkam",
    axial: "2.275 Å — uzun, kuchsiz",
    delta: "0.307 Å — Yahn-Teller cho'zilish kattaligi",
  },
  
  safety: "Cu(II) tuzlari — teri va shilliq qavatlar uchun tirnash xususiyati. Ko'z bilan kontaktidan saqlaning. Ortiqcha miqdorda zaharli.",
  
  diffractionPeaks: [
    { h: 0, k: 1, l: 1, twotheta: 7.95, d: 5.56, intensity: 40 },
    { h: 1, k: 0, l: 0, twotheta: 10.25, d: 4.32, intensity: 100 },
    { h: 0, k: 2, l: 0, twotheta: 13.55, d: 3.27, intensity: 55 },
    { h: 1, k: 1, l: 1, twotheta: 16.05, d: 2.76, intensity: 35 },
    { h: 0, k: 0, l: 2, twotheta: 18.42, d: 2.41, intensity: 28 },
    { h: 1, k: 2, l: 1, twotheta: 21.15, d: 2.10, intensity: 22 },
    { h: 2, k: 0, l: 0, twotheta: 23.68, d: 1.88, intensity: 18 },
    { h: 0, k: 3, l: 1, twotheta: 26.25, d: 1.70, intensity: 15 },
    { h: 2, k: 1, l: 1, twotheta: 28.85, d: 1.55, intensity: 12 },
    { h: 1, k: 3, l: 2, twotheta: 31.52, d: 1.42, intensity: 10 },
    { h: 2, k: 2, l: 2, twotheta: 34.15, d: 1.31, intensity: 8 },
    { h: 0, k: 4, l: 0, twotheta: 36.85, d: 1.22, intensity: 6 },
    { h: 3, k: 1, l: 0, twotheta: 39.55, d: 1.14, intensity: 5 },
    { h: 2, k: 3, l: 2, twotheta: 42.15, d: 1.07, intensity: 4 },
  ],
}

function DifraktogrammaGrafik({ lineColor = "#4ade80" }) {
...
const TABS = [
  ...
  { id: "jahnTeller", label: "Yahn-Teller effekti", icon: "🔷" },
  { id: "refinement", label: "Ishonchlilik", icon: "📋" },
]

export default function CuH2O6RentgenPage() {
    ...
    <div className="min-h-screen bg-gradient-to-b from-purple-950 to-blue-950 text-white">
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="flex items-center gap-4 px-6 py-4 mb-6 border-b border-purple-800/50">
          ...
          <h1 className="text-xl font-bold text-white">[Cu(H₂O)₆]²⁺ — Rentgen difraksiyasi tahlili</h1>
        </div>
        <HeroSection />
        ...
        <div className="bg-purple-900/40 border border-purple-700/50 rounded-2xl p-6 md:p-8">
          {activeTab==="diffraction"&&(<div className="space-y-4"><h2 className="text-xl font-bold text-green-400">Hisoblangan difraktogramma</h2><p className="text-purple-400 text-sm">[Cu(H₂O)₆]²⁺ ning monoklinik fazasi (P2₁/c) uchun hisoblangan PXRD pattern. <strong className="text-green-400">Bu xom eksperimental difraktogramma emas.</strong></p><DifraktogrammaGrafik /><div className="bg-purple-800/30 rounded-xl p-4"><h3 className="text-white font-semibold mb-3">Piklar ro'yxati</h3><div className="overflow-x-auto max-h-64"><table className="w-full text-sm"><thead><tr className="border-b border-purple-700/50"><th className="text-left py-2 px-3 text-purple-400">hkl</th><th className="text-left py-2 px-3 text-purple-400">2θ (°)</th><th className="text-left py-2 px-3 text-purple-400">d (Å)</th><th className="text-left py-2 px-3 text-purple-400">I (%)</th></tr></thead><tbody>{CRYSTAL.diffractionPeaks.map((p,i)=>(<tr key={i} className="border-b border-purple-800/30 hover:bg-purple-800/20"><td className="py-2 px-3 font-mono text-green-400">({p.h}{p.k}{p.l})</td><td className="py-2 px-3 text-purple-200">{p.twotheta.toFixed(2)}</td><td className="py-2 px-3 text-purple-200">{p.d.toFixed(2)}</td><td className="py-2 px-3 text-purple-200">{p.intensity}</td></tr>))}</tbody></table></div></div></div>)}
          {activeTab==="calculator"&&(<div className="space-y-4"><h2 className="text-xl font-bold text-green-400">Bragg + Scherrer kalkulyatori</h2><p className="text-purple-400 text-sm">Monoklinik P2₁/c birlik hujayra uchun d(hkl) ni hisoblang.</p><BraggScherrerKalkulyator /></div>)}
          {activeTab==="structure"&&(<div className="space-y-4"><h2 className="text-xl font-bold text-green-400">Kristall struktura ma'lumotlari</h2><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div className="bg-purple-800/30 rounded-xl p-4"><h3 className="text-white font-semibold mb-3">Bog' uzunliklari</h3><table className="w-full text-sm"><thead><tr className="border-b border-purple-700/50"><th className="text-left py-2 px-3 text-purple-400">Bog'</th><th className="text-left py-2 px-3 text-purple-400">Uzunlik (Å)</th></tr></thead><tbody>{CRYSTAL.bondLengths.map((r,i)=>(<tr key={i} className="border-b border-purple-800/30"><td className="py-2 px-3 text-green-400 font-mono text-xs">{r.bond}</td><td className="py-2 px-3 text-purple-200">{r.length}</td></tr>))}</tbody></table></div><div className="bg-purple-800/30 rounded-xl p-4"><h3 className="text-white font-semibold mb-3">Bog' burchaklari</h3><table className="w-full text-sm"><thead><tr className="border-b border-purple-700/50"><th className="text-left py-2 px-3 text-purple-400">Burchak</th><th className="text-left py-2 px-3 text-purple-400">Qiymat</th></tr></thead><tbody>{CRYSTAL.bondAngles.map((r,i)=>(<tr key={i} className="border-b border-purple-800/30"><td className="py-2 px-3 text-green-400 font-mono text-xs">{r.angle}</td><td className="py-2 px-3 text-purple-200">{r.value}</td></tr>))}</tbody></table></div></div><div className="bg-blue-600/10 border border-blue-500/30 rounded-xl p-4"><h3 className="text-blue-400 font-semibold text-sm mb-2">🔬 Nima uchun Cu−O bog'lari teng emas?</h3><p className="text-purple-200 text-sm">Cu²⁺ (d⁹) — Yahn-Teller teoremasi bo'yicha degenerat eg orbitallar (dz², dx²−y²) da elektronlar notekis taqsimlanishi kerak. dx²−y² orbitalda bitta bo'sh o'rin — bu ekvatorial ligandlar bilan kuchliroq bog'lanishga olib keladi. Natijada 4 ta ekvatorial Cu−O bog'i <strong className="text-green-400">1.968 Å</strong>, 2 ta aksial Cu−O bog'i <strong className="text-green-400">2.275 Å</strong> — farq Δ = 0.307 Å.</p></div></div>)}
          {activeTab==="jahnTeller"&&(<div className="space-y-4"><h2 className="text-xl font-bold text-green-400">{CRYSTAL.jahnTeller.title}</h2><p className="text-purple-400 text-sm">{CRYSTAL.jahnTeller.explanation}</p><div className="grid grid-cols-3 gap-4 text-center"><div className="bg-green-600/10 border border-green-500/30 rounded-xl p-4"><p className="text-green-400 font-bold text-lg">{CRYSTAL.jahnTeller.equatorial}</p><p className="text-purple-400 text-xs mt-1">Ekvatorial Cu−O (4 ta)</p></div><div className="bg-red-600/10 border border-red-500/30 rounded-xl p-4"><p className="text-red-400 font-bold text-lg">{CRYSTAL.jahnTeller.axial}</p><p className="text-purple-400 text-xs mt-1">Aksial Cu−O (2 ta)</p></div><div className="bg-yellow-600/10 border border-yellow-500/30 rounded-xl p-4"><p className="text-yellow-400 font-bold text-lg">{CRYSTAL.jahnTeller.delta}</p><p className="text-purple-400 text-xs mt-1">Cho'zilish kattaligi Δ</p></div></div></div>)}
          {activeTab==="refinement"&&(<div className="space-y-4"><h2 className="text-xl font-bold text-green-400">Ishonchlilik parametrlari</h2><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b border-purple-700/50"><th className="text-left py-3 px-4 text-purple-400">Parametr</th><th className="text-left py-3 px-4 text-purple-400">Qiymat</th><th className="text-left py-3 px-4 text-purple-400">Maqbul diapazon</th></tr></thead><tbody>{[["R₁",CRYSTAL.r1Obs.toFixed(3),"< 0.05"],["wR₂",CRYSTAL.wR2Obs.toFixed(3),"< 0.10"],["GoF",CRYSTAL.gof.toFixed(2),"0.8−1.3"],["Rint",CRYSTAL.rInt.toFixed(3),"< 0.05"],["To'liqlik",CRYSTAL.completeness,"> 95%"]].map((r,i)=>(<tr key={i} className="border-b border-purple-800/30"><td className="py-3 px-4 text-green-400 font-mono">{r[0]}</td><td className="py-3 px-4 text-purple-200">{r[1]}</td><td className="py-3 px-4 text-purple-400">{r[2]}</td></tr>))}</tbody></table></div></div>)}
        </div>
        <div className="flex justify-between mt-8 pt-6 border-t border-purple-800/50">
          <Link href="/ilmiy/tahlil/rentgen/birikmalar/ni-cn4" className="text-purple-400 hover:text-purple-200 text-sm flex items-center gap-2"><span>←</span> [Ni(CN)₄]²⁻</Link>
          <Link href="/ilmiy/tahlil/rentgen/birikmalar/ag-nh3-2" className="text-purple-400 hover:text-purple-200 text-sm">[Ag(NH₃)₂]⁺ →</Link>
        </div>
      </div>
    </div>
  )
