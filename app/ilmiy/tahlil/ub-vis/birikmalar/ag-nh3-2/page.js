...
export default function AgNH32_UBVis() {
  ...
  const tabs = [
    ...
    { id: "taqqos",   label: " Boshqa d¹⁰ komplekslar" },
  ]
    ...
    <div data-fon={fonKaliti} className="v3 min-h-screen flex flex-col text-[var(--v3-matn)] bg-[var(--v3-fon)] transition-colors duration-200">
      ...
      <header className="flex items-center gap-4 px-6 py-4 border-b border-[var(--v3-chiziq)]">
        ...
        <div>
          ...
          <p className="text-[var(--v3-xira)] text-sm">diamminkumush(I) ioni • RANGSIZ • d¹⁰ • Tollens reaktivi</p>
        </div>
      </header>
      ...
      <section className="max-w-4xl mx-auto px-6 py-12 space-y-8">
        ...
        <div className="v3-panel-karta p-8 relative overflow-hidden">
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            <span className="bg-purple-600/20 text-[var(--v3-xira)] border border-purple-600/30 px-3 py-1 rounded-full text-xs">Chiziqli</span>
          ...
          <div className="flex items-baseline gap-4 mb-4">
            ...
            <span className="text-[var(--v3-xira)] text-lg">141.94 g/mol</span>
          </div>
          ...
          <p className="text-[var(--v3-matn)] leading-relaxed mb-6">
            <strong className="text-amber-400 font-bold">[Ag(NH₃)₂]⁺ — RANGSIZ.</strong> Sababi: Ag⁺ (d¹⁰) — 
            ...
            <strong className="text-amber-400 font-bold"> UB sohada</strong> (~200−250 nm) LMCT va ligand ichidagi o'tishlar kuzatiladi,
          ...
          <div className="v3-panel-karta p-8 space-y-6">
            ...
            <div className="space-y-4">
              <div className="p-5 rounded-xl bg-[var(--v3-yuza)] border border-[var(--v3-chiziq)] border border-[var(--v3-chiziq)]">
                ...
                <p className="text-[var(--v3-matn)] text-sm">Ag⁺: [Kr] 4d¹⁰. Barcha 10 ta d-elektron juftlashgan, d-orbitallar to'liq to'lgan. d-d o'tish uchun elektron o'tadigan bo'sh orbital yo'q.</p>
              </div>
              <div className="p-5 rounded-xl bg-[var(--v3-yuza)] border border-[var(--v3-chiziq)] border border-[var(--v3-chiziq)]">
                <h3 className="text-amber-400 font-bold font-bold mb-2">2. KMBE = 0</h3>
                <p className="text-[var(--v3-matn)] text-sm">Barcha d-orbitallar to'lgan — kristall maydonda barqarorlashish energiyasi nolga teng. Geometriya sterik omillar bilan belgilanadi.</p>
              </div>
              <div className="p-5 rounded-xl bg-[var(--v3-yuza)] border border-[var(--v3-chiziq)] border border-[var(--v3-chiziq)]">
                <h3 className="text-amber-400 font-bold font-bold mb-2">3. UB sohada yutilish</h3>
                <p className="text-[var(--v3-matn)] text-sm">LMCT (NH₃→Ag⁺) va ligand ichidagi o'tishlar UB sohada (~200−250 nm). Ko'rinadigan sohaga (400−800 nm) tushmaydi — rangsiz.</p>
              </div>
            </div>
        ...
        {activeTab === "spektr" && (
          <div className="v3-panel-karta p-8 space-y-6">
            ...
            <p className="text-[var(--v3-matn)] leading-relaxed">UB-Vis spektrida <strong className="text-amber-400 font-bold">ko'rinadigan sohada (400−800 nm) hech qanday yutilish yo'q</strong>. Barcha yutilishlar UB sohada.</p>
            <div className="overflow-x-auto"><table className="w-full text-left"><thead><tr className="border-b border-[var(--v3-chiziq)]"><th className="py-3 px-4">λ (nm)</th><th className="py-3 px-4">O'tish turi</th><th className="py-3 px-4">Izoh</th></tr></thead><tbody className="text-[var(--v3-matn)]">{[["~250","LMCT (NH₃→Ag⁺)","UB sohada"],["~210","π→σ* (NH₃ ichki)","Chuqur UB"]].map((r,i)=>(<tr key={i} className="border-b border-[var(--v3-chiziq)] hover:bg-purple-800/20"><td className="py-3 px-4 font-mono font-bold text-amber-400 font-bold">{r[0]}</td><td className="py-3 px-4 text-sm">{r[1]}</td><td className="py-3 px-4 text-sm">{r[2]}</td></tr>))}</tbody></table></div>
          </div>
        )}

        {activeTab === "taqqos" && (
          <div className="v3-panel-karta p-8 space-y-6">
            <h2 className="text-xl font-bold text-white"> Barcha d¹⁰ komplekslar rangsiz</h2>
            <div className="overflow-x-auto"><table className="w-full text-left"><thead><tr className="border-b border-[var(--v3-chiziq)]"><th className="py-3 px-4">Kompleks</th><th className="py-3 px-4">dⁿ</th><th className="py-3 px-4">Rang</th><th className="py-3 px-4">Sabab</th></tr></thead><tbody className="text-[var(--v3-matn)]">{[["[Ag(NH₃)₂]⁺","d¹⁰","Rangsiz","d-d yo'q"],["[Zn(H₂O)₆]²⁺","d¹⁰","Rangsiz","d-d yo'q"],["[Cu(NH₃)₂]⁺","d¹⁰","Rangsiz","d-d yo'q"],["[Cd(CN)₄]²⁻","d¹⁰","Rangsiz","d-d yo'q"],["[HgI₄]²⁻","d¹⁰","Rangsiz","d-d yo'q"]].map((r,i)=>(<tr key={i} className="border-b border-[var(--v3-chiziq)] hover:bg-purple-800/20"><td className="py-3 px-4 font-bold text-amber-400 font-bold">{r[0]}</td><td className="py-3 px-4">{r[1]}</td><td className="py-3 px-4">{r[2]}</td><td className="py-3 px-4 text-sm">{r[3]}</td></tr>))}</tbody></table></div>
          </div>
        )}
        ...
        <div className="bg-gradient-to-r from-cyan-600/10 to-purple-600/10 border border-cyan-500/20 rounded-2xl p-8">
          ...
          <ol className="space-y-2 text-[var(--v3-matn)] list-decimal list-inside">
            <li><strong className="text-amber-400 font-bold">d¹⁰ — d-d o'tishlar YO'Q</strong></li>
            <li><strong className="text-amber-400 font-bold">KMBE=0</strong></li>
            <li><strong className="text-amber-400 font-bold">Rangsiz</strong> — yutilish faqat UB sohada</li>
          </ol>
        </div>

        <div className="flex justify-between pt-6">
          <Link href="/ilmiy/tahlil/ub-vis/birikmalar/cr-h2o6" className="px-6 py-3 border border-purple-500 rounded-xl hover:bg-purple-800/50 text-[var(--v3-matn)]">← [Cr(H₂O)₆]³⁺</Link>
