...
export default function Lambda3D() {
  ...
  useEffect(() => {
    ...
    // Λ (M-helis) — chap vint: Δ ning ko'zgudagi aksi
    const nPositions = [
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-purple-950 to-blue-950 text-white flex flex-col">
      <header className="flex items-center gap-4 px-6 py-4 border-b border-purple-800/50 z-10">
        ...
        <div><h1 className="text-xl font-bold text-purple-400">🟣 Λ-[Co(en)₃]³⁺ — 3D</h1><p className="text-purple-400 text-sm">Chap vint • M-helis • Tris(etilendiamin)kobalt(III)</p></div>
      </header>
      <div ref={containerRef} className="flex-1 w-full" />
      <div className="flex justify-center gap-8 py-4 px-6 bg-purple-950/80 border-t border-purple-800/50 z-10 flex-wrap">
        <div className="text-center"><div className="text-xs text-purple-400">Konfiguratsiya</div><div className="text-lg font-bold text-white">Λ (M-helis)</div></div>
        <div className="text-center"><div className="text-xs text-purple-400">CD signali</div><div className="text-lg font-bold text-white">Manfiy (−)</div></div>
        ...
        <div className="text-center"><div className="text-xs text-purple-400">Xelat</div><div className="text-lg font-bold text-white">λλλ (lel)</div></div>
      </div>
