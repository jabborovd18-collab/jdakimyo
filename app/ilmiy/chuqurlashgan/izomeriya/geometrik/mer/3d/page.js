...
export default function Mer3D() {
  ...
  useEffect(() => {
    ...
    const d = 1.5
    // mer — Cl lar meridian bo'ylab: ikkitasi qarama-qarshi (trans), bittasi perpendikulyar
    // Cl: (+x, 0, 0), (-x, 0, 0), (0, +y, 0) — meridian bo'ylab
    // N: (0, -y, 0), (0, 0, +z), (0, 0, -z) — qolgan pozitsiyalar

    const ligands = [
      { x: d, y: 0, z: 0, color: 0x00CC00, type: "Cl" },     // Cl — meridian
      { x: -d, y: 0, z: 0, color: 0x00CC00, type: "Cl" },    // Cl — meridian (trans)
      { x: 0, y: d, z: 0, color: 0x00CC00, type: "Cl" },     // Cl — meridian (perpendikulyar)
      { x: 0, y: -d, z: 0, color: 0x3050F0, type: "N" },     // N — meridian
      { x: 0, y: 0, z: d, color: 0x3050F0, type: "N" },      // N — aksial
      { x: 0, y: 0, z: -d, color: 0x3050F0, type: "N" },     // N — aksial
    ]
    ...
    // Meridian chiziq — Cl lar orasidagi yo'l
    const merPts = [new THREE.Vector3(d, 0, 0), new THREE.Vector3(0, 0, 0), new THREE.Vector3(-d, 0, 0), new THREE.Vector3(0, 0, 0), new THREE.Vector3(0, d, 0)]
    const merGeo = new THREE.BufferGeometry().setFromPoints(merPts)
    scene.add(new THREE.Line(merGeo, new THREE.LineBasicMaterial({ color: 0x00CC00, transparent: true, opacity: 0.5 })))

    // Trans Cl−Co−Cl chizig'i (x o'qi)
    const transPts = [new THREE.Vector3(-d, 0, 0), new THREE.Vector3(d, 0, 0)]
    const transGeo = new THREE.BufferGeometry().setFromPoints(transPts)
    scene.add(new THREE.Line(transGeo, new THREE.LineBasicMaterial({ color: 0x00CC00, transparent: true, opacity: 0.35 })))
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-purple-950 to-blue-950 text-white flex flex-col">
      <header className="flex items-center gap-4 px-6 py-4 border-b border-purple-800/50 z-10">
        ...
        <div><h1 className="text-xl font-bold text-purple-400">🟣 mer-[CoCl₃(NH₃)₃] — 3D</h1><p className="text-purple-400 text-sm">Oktaedrik • C₂v simmetriya • Meridional izomer</p></div>
      </header>
      ...
      <div className="flex justify-center gap-8 py-4 px-6 bg-purple-950/80 border-t border-purple-800/50 z-10 flex-wrap">
        ...
        <div className="text-center"><div className="text-xs text-purple-400">Cl joylashuvi</div><div className="text-lg font-bold text-white">Meridian bo'ylab</div></div>
        <div className="text-center"><div className="text-xs text-purple-400">Dipol momenti</div><div className="text-lg font-bold text-white">μ ≠ 0 (kuchsiz)</div></div>
      </div>
