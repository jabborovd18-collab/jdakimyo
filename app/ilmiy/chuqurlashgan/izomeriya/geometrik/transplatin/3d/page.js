...
export default function Transplatin3D() {
  ...
  useEffect(() => {
    ...
    const d = 1.5
    // trans — Cl lar qarama-qarshi (x o'qi), N lar qarama-qarshi (y o'qi)
    const positions = [
      { x: d, y: 0, z: 0, color: 0x00CC00, label: "Cl", type: "Cl" },    // +x — Cl
      { x: -d, y: 0, z: 0, color: 0x00CC00, label: "Cl", type: "Cl" },   // -x — Cl (qarama-qarshi — trans!)
      { x: 0, y: d, z: 0, color: 0x3050F0, label: "N", type: "N" },      // +y — N
      { x: 0, y: -d, z: 0, color: 0x3050F0, label: "N", type: "N" },     // -y — N (qarama-qarshi — trans!)
    ]
    ...
    const ringMat = new THREE.MeshBasicMaterial({ color: 0xaa4444, transparent: true, opacity: 0.3 })
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-purple-950 to-blue-950 text-white flex flex-col">
      <header className="flex items-center gap-4 px-6 py-4 border-b border-purple-800/50 z-10">
        ...
        <div><h1 className="text-xl font-bold text-red-400">🔶 trans-[PtCl₂(NH₃)₂] — 3D</h1><p className="text-purple-400 text-sm">Kvadrat planar • D₂h simmetriya • Trans-izomer</p></div>
      </header>
      ...
      <div className="flex justify-center gap-8 py-4 px-6 bg-purple-950/80 border-t border-purple-800/50 z-10 flex-wrap">
        ...
        <div className="text-center"><div className="text-xs text-purple-400">Cl−Pt−Cl</div><div className="text-lg font-bold text-white">180°</div></div>
